               ...
               struct functions *functions)
{
    struct Option *group, *seeds, *bounds, *output, *method, *similarity,
        *threshold, *min_segment_size,
        *endt; /* Establish an Option pointer for each option */
    struct Option *radio_weight, *smooth_weight;
    struct Flag *estimate_threshold, *diagonal, *weighted, *limited,
        *final;             /* Establish a Flag pointer for each option */
    struct Option *outband; /* optional saving of segment data, until a seperate
                               module is written */
    ...
    min_segment_size->answer = "1"; /* default: no merges, a minimum of 1 pixel
                                       is allowed in a segment. */
    ...
    min_segment_size->description =
        _("The final iteration will ignore the threshold for any segments with "
          "fewer pixels.");
    ...
    very_close->description =
        _("Neighbors similarity lower then this fraction of the threshold will "
          "be merged without regard to any other processing rules.");
#endif

    /* for the weights of bands values vs. shape parameters, and weight of
    ...
    bounds->description = _("Pixels with the same integer value will be "
    ...
    endt->required = NO;
    endt->answer = "1000";
    endt->description = _("Maximum number of passes (time steps) to complete.");
    ...
    /*
       ...
       path->key = 'p';
       path->description =
       _("temporary option, pathflag, select to use Rk as next Ri if not
       mutually best neighbors.");
     */
    ...
    outband->required = NO;
    outband->description = _(
        "Save the final mean values for the first band in the imagery group.");
    /* enhancement: save mean values for all bands.  Multiple rasters or switch
     * to polygons? */
    ...
    if (G_legal_filename(output->answer) == TRUE)
        files->out_name =
            output->answer; /* name of output (segment ID) raster map */
    else
        G_fatal_error(_("Invalid output raster name."));

    functions->threshold =
        atof(threshold->answer); /* Note: this threshold is scaled after we know
                                    more at the beginning of create_isegs() */
    ...
    if (strncmp(method->answer, "region_growing", 10) == 0)
        ...
        G_fatal_error(
            _("Couldn't assign segmentation method.")); /*shouldn't be able to
                                                           get here */
    ...
    if (strncmp(similarity->answer, "euclidean", 5) == 0)
        ...
        G_fatal_error(
            _("Couldn't assign similarity method.")); /*shouldn't be able to get
                                                         here */
    ...
    /* check if optional seeds map was given, if not, use all pixels as starting
     * seeds. */
    if (seeds->answer == NULL) {
        ...
        if ((files->seeds_mapset = G_find_raster2(files->seeds_map, "")) ==
            NULL) {
    ...
    if (bounds->answer == NULL) { /*no processing constraints */
        ...
        if ((files->bounds_mapset = G_find_raster2(files->bounds_map, "")) ==
            NULL) {
        ...
        if (Rast_map_type(files->bounds_map, files->bounds_mapset) !=
            CELL_TYPE) {
            G_fatal_error(
                _("Segmentation constraint map must be CELL type (integers)"));
        }
    }
