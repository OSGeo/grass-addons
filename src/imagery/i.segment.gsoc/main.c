 ...
 *
 *
 * NOTE:         the names "segment" and "SEG" are already used by the
 *               Segmentation Library for the data files/tiling, so "iseg"
 *               (image segmentation) will be used to refer to the image
 *               segmentation.
 *
 *               First developed for GSoC 2012 with mentor:
 *                  Markus Metz
 *
 *****************************************************************************/
...
{
    struct files
        files; /* input and output file descriptors, data structure, buffers */
    struct functions
        functions; /* function pointers and parameters for the calculations */
    ...
    module->description = _("Outputs a single segmented map (raster) based on "
                            "input values in an image group.");

    if (parse_args(argc, argv, &files, &functions) != TRUE)
        G_fatal_error(_("Error in parse_args()"));

    /* check if we are doing normal processing or if the estimate threshold and
     * exit flag has been selected */
