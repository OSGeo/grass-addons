...
int create_isegs(struct files *files, struct functions *functions)
{
    ...
    if (files->bounds_map == NULL) { /*normal processing */
        lower_bound = upper_bound =
            0; /* so run the segmentation algorithm just one time */
    }
    else {
        if (Rast_read_range(files->bounds_map, files->bounds_mapset, &range) !=
            1) { /* returns -1 on error, 2 on empty range, quiting either way.
                  */
            G_fatal_error(_("No min/max found in boundary raster map <%s>"),
                          files->bounds_map);
        }
        Rast_get_range_min_max(&range, &lower_bound, &upper_bound);
        /* speed enhancement, when processing with bounds: get the unique
         * values. As is, we will iterate at one time over the entire raster for
    ...
    if (files->bounds_map != NULL)
        G_message(_("Starting image segmentation within boundary constraints, "
                    "the percent complete is based the range of values in the "
                    "boundary constraints map"));
        ...
        if (files->bounds_map == NULL) {
            ...
            for (row = 0; row < files->nrows; row++) {
                for (col = 0; col < files->ncols; col++) {
                    ...
                    else /* pixel is outside the current boundary or was null in
                            the input bands */
    ...
    } /* end outer loop for processing bounds constraints (will just run once if
         not provided) */
...
int region_growing(struct files *files, struct functions *functions)
{
    ...
    int endflag;  /* =TRUE if there were no merges on that processing iteration
                   */
    int pathflag; /* =TRUE if we didn't find mutual neighbors, and should
                     continue with Rk */
    struct pixels *Ri_head, *Rk_head, *Rin_head, *Rkn_head, *current, *newpixel,
        *Ri_bestn;
    ...
    start = clock();
#endif
    /* files->token has the "link_head" for linkm: linked list memory
     * allocation.
     *
     ...
     * */
    if (files->bounds_map == NULL)
        G_message(_("Running region growing algorithm, the percent completed "
                    "is based on %d max iterations, but the process will end "
                    "earlier if no further merges can be made."),
                  functions->end_t);
     ...
     * if this is implemented, move this assignment inside the do loop and make
     * it a function of t. */
    threshold = functions->threshold;

    /* user has option to skip the normal growing steps and skip to the final
     * merge. */
    if (!functions->final_merge_only) {

        /* do while loop until no merges are made, or until t reaches maximum
        ...
        do {
...
#endif
            G_debug(3, "#######   Starting outer do loop! t = %d    #######",
                    t);
            if (files->bounds_map == NULL)
            ...
            /*check each pixel, start the processing only if it is a candidate
             * pixel */
            ...
            for (row = files->minrow; row < files->maxrow; row++) {
                for (col = files->mincol; col < files->maxcol; col++) {
                    ...
                    if (FLAG_GET(files->candidate_flag, row, col) &&
                        ...
                        while (pathflag ==
                               TRUE) { /*if don't find mutual neighbors on first
                                          try, will use Rk as next Ri. */
                            G_debug(4, "Next starting pixel: row, %d, col, %d",
                                    Ri_head->row, Ri_head->col);

                            pathflag = FALSE;

                            /* find segment neighbors, if we don't already have
                             * them */
                            if (Rin_head == NULL) {
#ifdef PROFILE
                                fn_start = clock();
                                ...
                                find_segment_neighbors(&Ri_head, &Rin_head,
                                                       &Ri_count, files,
                                                       functions);
#ifdef PROFILE
                                fn_end = clock();
                                fn_lap = ((double)(fn_end - fn_start)) /
                                         CLOCKS_PER_SEC;
                                fn_accum += fn_lap;
                            ...
                            if (Rin_head != NULL) { /*found neighbors, find best
                                                       neighbor then see if is
                                                       mutually best neighbor */
                                ...
                                Ri_similarity =
                                    threshold +
                                    1; /* set current similarity to max value */
                                Segment_get(
                                    &files->bands_seg, (void *)files->bands_val,
                                    Ri_head->row,
                                    Ri_head->col); /* current segment values */
                                ...
                                for (current = Rin_head; current != NULL;
...
#ifdef VCLOSE
                                    /* if very close, will merge, but continue
                                     * checking other neighbors */
                                    if (tempsim <
                                        functions->very_close * threshold) {
                                        /* add to Rclose list */
                                        newpixel = (struct pixels *)link_new(
                                            files->token);
                                    ...
                                    /* If "sort of" close, merge only if it is
                                     * the mutually most similar */
                                    else
#endif
                                ...
                                } /* finished similiarity check for all
                                     neighbors */

                                /* *** merge all the "very close"
                                 * pixels/segments *** */
                                /* doing this after checking all Rin, so we
                                 * don't change the bands_val between similarity
                                 * comparisons
                                 * ... but that leaves the possibility that we
                                 * have the wrong best Neighbor after doing
                                 * these merges... but it seems we can't put
                                 * this merge after the Rk/Rkn portion of the
                                 * loop, because we are changing the available
                                 * neighbors
                                 * ...maybe this extra "very close" idea has to
                                 * be done completely differently or dropped???
                                 */
#ifdef VCLOSE
                                for (current = Rclose_head; current != NULL;
                                    ...
                                    find_segment_neighbors(
                                        &Rc_head, &Rcn_head, &Rc_count, files,
                                        functions); /* just to get members, not
                                                       looking at neighbors now
                                                     */
                                    ...
                                    /*to consider, recurse?  Check all Rcn
                                     * neighbors if they are very close? */
                                ...
                                /* check if we have a bestn that is valid to use
                                 * to look at Rk */
                                if (Ri_bestn != NULL) {
                                    if ((functions->limited == TRUE) &&
                                        !(FLAG_GET(files->candidate_flag,
                                                   Ri_bestn->row,
                                                   Ri_bestn->col))) {
                                        /* this check is important:
                                         * best neighbor is not a valid
                                         * candidate, was already merged earlier
                                         * in this time step */
                                        Ri_bestn = NULL;
                                    }
                                }
                                if (Ri_bestn != NULL &&
                                    Ri_similarity <
                                        threshold) { /* small TODO: should this
                                                        be < or <= for
                                                        threshold? */
                                     ...
                                     * ******** */
                                    Rk_similarity =
                                        Ri_similarity; /*Ri gets first priority
                                                          - ties won't change
                                                          anything, so we'll
                                                          accept Ri and Rk as
                                                          mutually best
                                    ...
                                    Segment_get(
                                        &files->bands_seg,
                                        (void *)files->bands_val, Rk_head->row,
                                        Rk_head
                                            ->col); /* current segment values */

                                    /* check similarity for each of Rk's
                                     * neighbors */
                                    for (current = Rkn_head; current != NULL;
                                         current = current->next) {
                                        tempsim =
                                            functions->calculate_similarity(
                                                Rk_head, current, files,
                                                functions);

                                        if (tempsim < Rk_similarity) {
                                            Rk_similarity = tempsim;
                                            break; /* exit for Rk's neighbors
                                                      loop here, we know that Ri
                                                      and Rk aren't mutually
                                                      best neighbors */
                                        }
                                    } /* have checked all of Rk's neighbors */

                                    if (Rk_similarity ==
                                        Ri_similarity) { /* mutually most
                                                            similar neighbors */
...
#endif
                                        endflag =
                                            FALSE; /* we've made at least one
                                                      merge, so want another t
                                                      iteration */
                                    }
                                    else { /* they weren't mutually best
                                              neighbors */

                                        /* checked Ri once, didn't find a
                                         * mutually best neighbor, so remove all
                                         * members of Ri from candidate pixels
                                         * for this iteration */
                                        set_candidate_flag(Ri_head, FALSE,
                                                           files);

                                        if (FLAG_GET(files->candidate_flag,
                                                     Rk_head->row,
                                                     Rk_head->col) &&
                                ...
                                } /* end if (Ri_bestn != NULL && Ri_similarity <
                                     threshold) */
                                else {
                                    /* no valid best neighbor for this Ri
                                     * exclude this Ri from further comparisons
                                     * because we checked already Ri for a
                                     * mutually best neighbor with all valid
                                     * candidates thus Ri can not be the
                                     * mutually best neighbor later on during
                                     * this pass unfortunately this does happen
                                     * sometimes */
                            ...
                            if (pathflag) { /*initialize Ri, Rin, using Rk as Ri
                                               and Rkn as Rin. */

                                /* For the next iteration, lets start with Rk as
                                 * the focus segment */
    ...
    if (t == 2 && files->bounds_map == NULL)
        G_warning(_(
            "No segments were created. Verify threshold and region settings."));
    /* future enhancement, remove the "&& bounds_map == NULL" if we check for
     * unique bounds values. */

    if (endflag == FALSE)
        G_message(_("Merging processes stopped due to reaching max iteration "
                    "limit, more merges may be possible"));

    /* ******************************************************************************************
     */
    /* speed enhancement: after < 3 (?) merges are made in one pass, switch
     * processing modes. It seems a significant portion of the time is spent
     * merging small pixels into the largest segments.  So consider allowing
     * multiple merges after finding one large Ri.
     *
     * Maybe related to this, and/or integrated into the first loops:  If a
     * segment has only one neighbor go ahead and merge it if similarity is <
     * threshold.
     *
     * ******************************************************************************************
     */
    ...
    if ((functions->min_segment_size > 1 && t > 2) ||
        functions->final_merge_only) {
        ...
        if (files->bounds_map == NULL) {
            G_message(_("Final iteration, forcing merges for small segments, "
                        "percent complete based on rows."));
        }
        /* for the final forced merge, the candidate flag is just to keep track
         * if we have confirmed if: a. the segment size is >= to the minimum
         * allowed size  or b. we have merged it with its best neighbor
         */
        ...
        for (row = files->minrow; row < files->maxrow; row++) {
            ...
            for (col = files->mincol; col < files->maxcol; col++) {
                ...
                if (FLAG_GET(files->candidate_flag, row, col)) {
                    ...
                    /* First pixel in Ri is current row/col pixel.  We may add
                     * more later if it is part of a segment */
                    Ri_count = 1;
                    newpixel = (struct pixels *)link_new(files->token);
                    ...
                    find_segment_neighbors(&Ri_head, &Rin_head, &Ri_count,
                                           files, functions);

                    if (Rin_head != NULL) { /*found neighbors */
                        if (Ri_count >=
                            functions
                                ->min_segment_size) /* don't force a merge */
                            set_candidate_flag(Ri_head, FALSE, files);
                            ...
                            /* find Ri's most similar neighbor */
                            Ri_bestn = NULL;
                            Ri_similarity = DBL_MAX; /* set current similarity
                                                        to max value */
                            Segment_get(
                                &files->bands_seg, (void *)files->bands_val,
                                Ri_head->row,
                            ...
                            if (Ri_bestn != NULL) {
                                ...
                                /* get the full pixel/cell membership list for
                                 * Rk */
                                /* speed enhancement: a seperate function for
                                 * this, since we don't need the neighbors */
                                find_segment_neighbors(&Rk_head, &Rkn_head,
                                                       &Rk_count, files,
                                                       functions);
                                ...
                                /* merge_values sets Ri and Rk candidate flag to
                                 * FALSE.  Put Rk back to TRUE if the size is
                                 * too small. */
                                ...
                                G_warning(_(
                                    "No best neighbor found in final merge for "
...
/* perimeter todo, for now will return borderPixels instead of passing a
 * pointer, I saw mentioned that each parameter slows down the function call? */
...
{
    int n, borderPixels, current_seg_ID, temp_ID,
        R_iseg = -1; /* borderPixels is just used for open_files to get a
                        starting perimeter with seeded segmentation. */
    struct pixels *newpixel, *current, *to_check, tree_pix,
        *pixel_iter; /* need to check the pixel neighbors of to_check */
    int pixel_neighbors[8][2];
    struct RB_TREE *no_check_tree; /* pixels that should no longer be checked on
                                      this current find_neighbors() run */
    ...
    /* speed enhancement, any time savings to move any variables to files (mem
     * allocation once in open_files) */

    /* neighbor list will be a listing of pixels that are neighbors, but will be
     * limited to just one pixel from each neighboring segment.
     * */

    /* parameter: R, current segment membership, could be single pixel
     * (incomplete list) or list of pixels. parameter: neighbors/Rin/Rik,
     * neighbor pixels, could have a list already, or could be empty
    ...
    if (R_iseg ==
        0) { /* if seeds were provided, this is just a single non-seed pixel,
                only return neighbors that are segments or seeds */
        ...
        for (n = 0; n < functions->num_pn; n++) {
            ...
            if (current_seg_ID > 0) {
                newpixel = (struct pixels *)link_new(files->token);
                newpixel->next = *neighbors_head; /*point the new pixel to the
                                                     current first pixel */
                newpixel->row = pixel_neighbors[n][0];
                newpixel->col = pixel_neighbors[n][1];
                *neighbors_head =
                    newpixel; /*change the first pixel to be the new pixel. */
            }
            borderPixels++;
            /* increment for all non null pixels */ /* TODO perimeter: OK to
                                                       ignore NULL cells? */
        }
    }
        ...
        /* Copy R in to_check and no_check data structures (don't expand them if
         * we find them again) */
        ...
        for (current = *R_head; current != NULL; current = current->next) {
            ...
            newpixel = (struct pixels *)link_new(files->token);
            newpixel->next =
                to_check; /*point the new pixel to the current first pixel */
            newpixel->row = current->row;
            newpixel->col = current->col;
            to_check =
                newpixel; /*change the first pixel to be the new pixel. */
            ...
            if (rbtree_insert(no_check_tree, &tree_pix) ==
                0) /* don't check it again */
                G_warning(_("could not insert data into tree, out of memory?"));
        }

        while (to_check != NULL) { /* removing from to_check list as we go, NOT
                                      iterating over the list. */

            functions->find_pixel_neighbors(to_check->row, to_check->col,
                                            pixel_neighbors, files);
            ...
            to_check =
                to_check
                    ->next; /*head now points to the next element in the list */
            link_dispose(files->token, (VOID_T *)current);

            /* for each pixel neighbors, check if they should be processed,
             * check segment ID, and add to appropriate lists */
            for (n = 0; n < functions->num_pn; n++) {
                ...
                if (rbtree_find(no_check_tree, &tree_pix) ==
                    FALSE) { /* want to check this neighbor */
                    Segment_get(&files->iseg_seg, &current_seg_ID,
                                pixel_neighbors[n][0], pixel_neighbors[n][1]);

                    rbtree_insert(no_check_tree,
                                  &tree_pix); /* don't check it again */

                    if (current_seg_ID ==
                        R_iseg) { /* pixel is member of current segment, add to
                                     R */
                        /* put pixel_neighbor[n] in Ri */
                        newpixel = (struct pixels *)link_new(files->token);
                        newpixel->next = *R_head; /*point the new pixel to the
                                                     current first pixel */
                        newpixel->row = pixel_neighbors[n][0];
                        newpixel->col = pixel_neighbors[n][1];
                        newpixel->count_shared =
                            (*R_head)
                                ->count_shared; /* Needed for Rk - to have the
                                                   head remember how many pixels
                                                   were shared with Ri. Might be
                                                   a better way to do this... */
                        *R_head = newpixel; /*change the first pixel to be the
                                               new pixel. */
                        *seg_count =
                            *seg_count +
                            1; /* zero index... Ri[0] had first pixel and set
                                  count =1.  increment after save data. */

                        /* put pixel_neighbor[n] in to_check -- want to check
                         * this pixels neighbors */
                        newpixel = (struct pixels *)link_new(files->token);
                        newpixel->next = to_check; /*point the new pixel to the
                                                      current first pixel */
                        newpixel->row = pixel_neighbors[n][0];
                        newpixel->col = pixel_neighbors[n][1];
                        to_check = newpixel; /*change the first pixel to be the
                                                new pixel. */
                    }
                    else {              /* segment id's were different */
                        borderPixels++; /* increment for all non null pixels
                                           that are non in no-check or R_iseg
                                           TODO perimeter: move this to include
                                           pixels in no-check ??? */

                        if (!rbtree_find(
                                known_iseg,
                                &current_seg_ID)) { /* we don't have any
                                                       neighbors yet from this
                                                       segment */
                            if (current_seg_ID != 0)
                                /* with seeds, non seed pixels are defaulted to
                                 * zero.  Should we use null instead?? then
                                 * could skip this check?  Or we couldn't insert
                                 * it??? */
                            ...
                            newpixel = (struct pixels *)link_new(files->token);
                            newpixel->next =
                                *neighbors_head; /*point the new pixel to the
                                                    current first pixel */
                            newpixel->row = pixel_neighbors[n][0];
                            newpixel->col = pixel_neighbors[n][1];
                            newpixel->count_shared =
                                1; /*for shared perimeter */
                            *neighbors_head =
                                newpixel; /*change the first pixel to be the new
                                             pixel. */
                        }
                        else { /* todo perimeter we need to keep track of (and
                                * return!) a total count of neighbors pixels for
                                * each neighbor segment, to update the perimeter
                                * value in the similarity calculation. */
                            /* todo perimeter: need to initalize this
                             * somewhere!!! */
                            /* todo perimeter... need to find pixel with same
                             * segment ID....  countShared++; hmmm,  Should we
                             * change the known_iseg tree to sort on segment
                             * ID... need to think of fast way to return this
                             * count?  with pixel?  or with something else? */

                            /* need to increment the count of border pixels for
                             * later know the shared border for calculating the
                             * new perimeter. */
                            if (functions->radio_weight <
                                1) { /* TODO: any reason to calculate this if we
                                        have a weight of 0 for the shape
                                        features? */
                                ...
                                for (pixel_iter = *neighbors_head;
                                    ...
                                    Segment_get(&files->iseg_seg, &temp_ID,
                                                pixel_iter->row,
                                                pixel_iter->col);
                ...
                /* even if no_check tree, if we are using shape measurements we
                 * need to count if there is a shared border. */
                else if (functions->radio_weight < 1) {
                    Segment_get(&files->iseg_seg, &current_seg_ID,
                                pixel_neighbors[n][0], pixel_neighbors[n][1]);
                    if (current_seg_ID != R_iseg) {
                        for (pixel_iter = *neighbors_head; pixel_iter != NULL;
                             pixel_iter = pixel_iter->next) {

                            Segment_get(&files->iseg_seg, &temp_ID,
                                        pixel_iter->row, pixel_iter->col);
...
int find_eight_pixel_neighbors(int p_row, int p_col, int pixel_neighbors[8][2],
                               struct files *files)
{
...
/* similarity / distance functions between two points based on their input
 * raster values */
/* assumes first point values already saved in files->bands_seg */
/* speed enhancement: Segment_get was already done for a[] values in the main
 * function. Could remove a[] from these parameters, reducing number of
 * parameters in function call could provide a speed improvement. */
                                      ...
                                      struct functions *functions)
{
    ...
    for (n = 0; n < files->nbands; n++) {
        val = val + (files->bands_val[n] - files->second_val[n]) *
                        (files->bands_val[n] - files->second_val[n]);
    }
    ...
    if (functions->radio_weight < 1) {
        ...
        /*remaining part of similarity is based on the current shape of the
         * object. */
                                      ...
                                      struct functions *functions)
{
    ...
    /* Manhattan distance, sum the absolute difference between values for each
     * dimension */
    for (n = 0; n < files->nbands; n++) {
        val += fabs(
            files->bands_val[n] -
            files->second_val[n]); /* speed enhancement: is fabs() is the "fast"
                                      way for absolute value calculations? */
    }
                 ...
                 int Rk_count, struct files *files)
{
    ...
    for (n = 0; n < files->nbands; n++) {
        files->bands_val[n] =
            (files->bands_val[n] * Ri_count + files->second_val[n] * Rk_count) /
            (Ri_count + Rk_count);
    }
    ...
    files->bands_val[n + 1] = files->bands_val[n + 1] +
                              files->second_val[n + 1] -
                              2 * Rk_head->count_shared; /* Perimeter Length */
    files->bands_val[n + 2] =
        max(files->bands_val[n + 2], files->second_val[n + 2]); /*max col */
    files->bands_val[n + 3] =
        min(files->bands_val[n + 3], files->second_val[n + 3]); /*min col */
    files->bands_val[n + 4] =
        max(files->bands_val[n + 4], files->second_val[n + 4]); /*max row */
    files->bands_val[n + 5] =
        min(files->bands_val[n + 5], files->second_val[n + 5]); /*min row */
    ...
    /* for each member of Ri and Rk, write new average bands values and segment
     * values */
    for (current = Ri_head; current != NULL; current = current->next) {
        ...
        FLAG_UNSET(files->candidate_flag, current->row,
                   current->col); /*candidate pixel flag, only one merge allowed
                                     per t iteration */
    }
    ...
    /* merged two segments, decrement count if Rk was an actual segment (not a
     * non-seed pixel) */
    if (Rk_iseg > 0)
        files->nsegs--;
...
/* calculates and stores the mean value for all pixels in a list, assuming they
...
int merge_pixels(struct pixels *R_head, int borderPixels, struct files *files)
{
    ...
    /* Note: using files->bands_val for current pixel values, and
     * files->second_val for the accumulated value */
    ...
    if (R_head->next != NULL) {
        ...
        files->bands_val[files->nbands + 1] =
            borderPixels; /* Perimeter Length */
...
int set_candidate_flag(struct pixels *head, int value, struct files *files)
{
    ...
    for (current = head; current != NULL; current = current->next) {
        ...
        if (value == FALSE) {
            ...
            G_fatal_error(_("programming bug, helper function called with "
                            "invalid argument"));
    }
    return TRUE;
}
...
/* Set candidate flag to true/1 or false/0 for all pixels in current processing
 * area checks for NULL flag and if it is in current "polygon" if a bounds map
 * is given */
