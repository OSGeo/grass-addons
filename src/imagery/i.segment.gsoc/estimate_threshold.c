...
int estimate_threshold(char *image_group)
{
    ...
    G_done_msg(
        _("Suggested threshold (if using -w flag and radioweight=1) is: <%g>"),
        est_t);
    return TRUE;
}
...
int check_group(char *image_group)
{
    ...
    if (!I_get_group_ref(image_group, &Ref))
        G_fatal_error(_("Unable to read REF file for group <%s>"), image_group);
...
int read_range(double *min, double *max, char *image_group)
{
    ...
    if (Rast_read_fp_range(Ref.file[0].name, Ref.file[0].mapset, &fp_range) !=
...
/* function to calculate a suggested threshold based on the min and max values
 * of the rasters */
double calc_t(double min, double max)
{
    double t, fraction;

    /* Empirical testing indicated 1 to 5% of the differences was a good
     * starting point. */
    fraction = 0.03;

    /* TODO: allow the community to test this estimate, the formula can be
     * updated based on their advice. */
    t = fraction * (max - min);
