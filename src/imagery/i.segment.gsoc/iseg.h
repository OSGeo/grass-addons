...
/* PROFILE will add some rough time checks for finding neighbors, merging, and
 * pass times. */
/* #define PROFILE */

/* SIGNPOST will add some fprintf statments to indicate what segments are being
 * merged. other diagnostics could be included here for during further
 * development and speed improvements.
 */
/* #define SIGNPOST */
...
struct files {
    /* user parameters */
    char *image_group;
    int weighted; /* 0 if false/not selected, so we should scale input.  1 if
                     the scaling should be skipped */
    ...
    const char *seeds_map, *seeds_mapset, *bounds_map,
        *bounds_mapset; /* optional segment seeds and polygon
                           constraints/boundaries */
    char *out_band;     /* for segment average values */

    /* file processing */
    /* bands_seg is initialized with the input raster valuess, then is updated
     * with current mean values for the segment. */
    /* for now, also storing: Area, Perimeter, maxcol, mincol, maxrow, minrow */
    int nbands; /* number of rasters in the image group */
    SEGMENT bands_seg,
        bounds_seg; /* bands is for input, normal application is landsat bands,
                       but other input can be included in the group. */
    double *bands_val;  /* array, to hold all input values at one pixel */
    double *second_val; /* to hold values at second point for similarity
                           comparison */
    ...
    /* processing flags */
    /* candidate flag for if a cell/segment has already been merged in that
     * pass. */
...
struct functions {
    ...
    int min_segment_size; /* smallest number of pixels/cells allowed in a final
                             segment */
    float radio_weight, smooth_weight; /* radiometric (bands) vs. shape and
                                          smoothness vs. compactness */
    /* Some function pointers to set in parse_args() */
    int (*find_pixel_neighbors)(
        int, int, int[8][2],
        struct files *); /*parameters: row, col, pixel_neighbors */
    double (*calculate_similarity)(
        struct pixels *, struct pixels *, struct files *,
        struct functions
            *); /*parameters: two pixels (each with row,col) to compare */

    /* max number of iterations/passes */
    int end_t;

    int path;    /* flag if we are using Rk as next Ri for non-mutually best
                    neighbor. */
    int limited; /* flag if we are limiting merges to one per pass */
    int estimate_threshold; /* flag if we just want to estimate a suggested
                               threshold value and exit. */
    int final_merge_only; /* flag if we want to just run the final merge portion
                             of the algorithm. */
...
#ifdef VCLOSE
    double
        very_close; /* segments with very_close similarity
                     * will be merged without changing or checking the candidate
                     * flag. The algorithm will continue looking for the "most
                     * similar" neighbor that isn't "very close". */
#endif
};
