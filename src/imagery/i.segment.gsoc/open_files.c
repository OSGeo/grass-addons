...
int open_files(struct files *files, struct functions *functions)
{
    ...
    DCELL **inbuf; /* buffer array, to store lines from each of the imagery
                      group rasters */
    CELL *boundsbuf, *seedsbuf;
    void *ptr; /* for iterating seedsbuf */
    size_t ptrsize;
    struct FPRange
        *fp_range; /* for getting min/max values on each input raster */
    DCELL *min, *max;
    ...
    /* confirm output maps can be opened (don't want to do all this work for
     * nothing!) */
    out_fd = Rast_open_new(files->out_name, CELL_TYPE);
    if (out_fd < 0)
        G_fatal_error(
            _("Could not open output raster for writing segment ID's"));
    else
        Rast_unopen(out_fd);
    ...
    if (files->out_band != NULL) {
        ...
        if (mean_fd < 0)
            G_fatal_error(_("Could not open output raster for writing mean "
                            "segment values"));
        else
            Rast_unopen(mean_fd);
    }
    ...
    if (files->seeds_map != NULL) {
        ...
        if (Rast_read_range(files->seeds_map, files->seeds_mapset, &range) !=
        ...
        if (seeds_min < 0)
            G_fatal_error(
                _("Seeds raster should have postive integers for starting "
                  "seeds, and zero or NULL for all other pixels."));
    }
    ...
    if (files->weighted == FALSE) { /*default, we will scale */
        for (n = 0; n < Ref.nfiles; n++) {
            if (Rast_read_fp_range(Ref.file[n].name, Ref.file[n].mapset,
                                   &fp_range[n]) !=
                1) /* returns -1 on error, 2 on empty range, quiting either way.
                    */
    ...
    /* when fine tuning, should be a power of 2 and not larger than 256 for
     * speed reasons */
    srows = 64;
    scols = 64;

    /* RAM enhancement: have user input limit and make calculations for this,
     * reference i.cost and i.watershed One segment tile is tile_mb = (nbands *
     * sizeof(double) + sizeof(CELL) * srows * scols / (1024*1024) (check if
     * sizeof(CELL) was from when iseg would be included, or the extra
     * overhead?) If user inputs total RAM available, need to subtract the size
     * of the flags, and the size of the linked lists. I'm not sure how to
     * estimate the size of the linked lists, it is allowed to grow when needed.
     * Assume one segment could be 50% of the map?  Or more?  So ll_mb =
     * sizeof(pixels) * nrows * ncols * 0.5 / (1024*1024) then split the
     * remaining RAM between bands_seg and iseg_seg. */
    ...
    if (Segment_open(&files->bands_seg, G_tempfile(), files->nrows,
                     files->ncols, srows, scols, inlen, nseg) != TRUE)
    ...
    if (Segment_open(&files->iseg_seg, G_tempfile(), files->nrows, files->ncols,
                     srows, scols, sizeof(int), nseg) != TRUE)
        G_fatal_error(_("Unable to allocate memory for initial segment ID's"));

    /* bounds/constraints (start with processing constraints to get any possible
     * NULL values) */
    if (files->bounds_map != NULL) {
        if (Segment_open(&files->bounds_seg, G_tempfile(), files->nrows,
                         files->ncols, srows, scols, sizeof(int), nseg) != TRUE)
    ...
    for (row = 0; row < files->nrows; row++) {
        ...
        /* read in rows of data (each input band from the imagery group and the
         * optional seeds map) */
        for (n = 0; n < Ref.nfiles; n++) {
        ...
        for (col = 0; col < files->ncols; col++) {
            ...
            for (n = 0; n < Ref.nfiles; n++) {
                ...
                if (files->weighted == TRUE)
                    ...
                    files->bands_val[n] = (inbuf[n][col] - min[n]) /
                                          (max[n] - min[n]); /* scaled */
            }

            /* besides the user input rasters, also save the shape parameters */

            files->bands_val[Ref.nfiles] =
                1; /* area (just using the number of pixels) */
            files->bands_val[Ref.nfiles + 1] = 4;
            /* Perimeter Length */ /* todo perimeter, not exact for
                                      edges...close enough for now? */
            ...
            if (null_check != -1) {                     /*good pixel */
                ...
                if (files->seeds_map != NULL) {
                    if (Rast_is_c_null_value(ptr) == TRUE) {
                        /* when using iseg_seg the segmentation file is already
                         * initialized to zero.  Just initialize seeds_flag: */
                        FLAG_UNSET(files->seeds_flag, row, col);
                    }
                    else {
                        FLAG_SET(files->seeds_flag, row,
                                 col); /* RAM enhancement, but it might cost
                                          speed.  Could look for seg ID > 0
                                          instead of using seed_flag. */
                    ...
                    s++; /* sequentially number all pixels with their own
                            segment ID */
                    if (s < INT_MAX) { /* Check that the starting seeds aren't
                                          too large. */
                        Segment_put(&files->iseg_seg, &s, row,
                                    col); /*starting segment number */
                        FLAG_SET(files->seeds_flag, row,
                                 col); /*all pixels are seeds */
                    }
                    else
                        G_fatal_error(_("Exceeded integer storage limit, too "
                                        "many initial pixels."));
                }
            }
    ...
    /* if we have seeds that are segments (not pixels) we need to update the
     * bands_seg */
    ...
    if (files->seeds_map != NULL) {
        ...
        for (row = 0; row < files->nrows; row++) {
            G_percent(row, files->nrows,
                      1); /* I think this is the longest part of open_files()
            ...
            for (col = 0; col < files->ncols; col++) {
                ...
                borderPixels = find_segment_neighbors(
                    &R_head, &Rn_head, &R_count, files, functions);

                /* update the segment ID */

                s++;
                if (s == INT_MAX) /* Check that the starting seeds aren't too
                                     large. */
                    G_fatal_error(_("Exceeded integer storage limit, too many "
                                    "initial pixels."));
