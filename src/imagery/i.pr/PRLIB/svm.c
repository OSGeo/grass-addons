                 ...
                 double *svm_W)
{
    ...
    if (svm_kernel == SVM_KERNEL_DIRECT) {
        ...
        svm->models =
            (SVM_direct_kernel *)G_calloc(svm->d, sizeof(SVM_direct_kernel));
            ...
            if (y[i] == -1)
                for (j = 0; j < n; j++)
                    if (y[j] == 1) {
                        ...
                        svm->models[index].w_coeff =
                            (y[j] - y[i]) /
                            (y[j] * svm->dot_prod[j][j] -
                             y[i] * svm->dot_prod[i][i] -
                             (y[j] - y[i]) * svm->dot_prod[i][j]);
                        svm->models[index].b =
                            y[i] - svm->models[index].w_coeff *
                                       (y[i] * svm->dot_prod[i][i] +
                                        y[j] * svm->dot_prod[i][j]);
                        index++;
                    }
        ...
        for (i = 0; i < n; i++)
            for (j = 0; j < svm->d; j++) {
                ...
                svm->dense_points[i][j] =
                    svm->models[j].w_coeff * (y[i1] * svm->dot_prod[i1][i] +
...
static void svm_smo(SupportVectorMachine *SVM)
{
    ...
    if (SVM->kernel_type == SVM_KERNEL_GAUSSIAN) {
        /*
           SVM->precomputed_self_dot_product=(double *)G_calloc
           (SVM->N,sizeof(double));
         */
        for (i = 0; i < SVM->N; i++)
...
static int takeStep(int i1, int i2, SupportVectorMachine *SVM)
{
...

    {
        ...
        if (a1 > 0 && a1 < SVM->Cw[i1])
            bnew =
                SVM->b + E1 + y1 * (a1 - alph1) * k11 + y2 * (a2 - alph2) * k12;
        else {
            if (a2 > 0 && a2 < SVM->Cw[i2])
                bnew = SVM->b + E2 + y1 * (a1 - alph1) * k12 +
                       y2 * (a2 - alph2) * k22;
            else {
                b1 = SVM->b + E1 + y1 * (a1 - alph1) * k11 +
                     y2 * (a2 - alph2) * k12;
                b2 = SVM->b + E2 + y1 * (a1 - alph1) * k12 +
                     y2 * (a2 - alph2) * k22;
                bnew = (b1 + b2) / 2;
            }
        }
                                 ...
                                 double threshold)
{
    ...
    double **invM = NULL;
    double **HM = NULL, **HMH = NULL, *tnH = NULL, **HMK = NULL, **KM = NULL,
           **KMK = NULL, *tnK = NULL, **tH = NULL, **tK = NULL;
    double mMm;
    double gap;
    ...
    for (i = 0; i < features->nexamples; i++) {
        for (j = 0; j < features->nclasses; j++) {
            if (features->class[i] == features->p_classes[j]) {
                ...
                if ((pred = predict_svm(svm, features->value[i])) *
                        features->class[i] <=
                    0.0) {
                    error[j] += 1.0;
                    accuracy += 1.0;
    ...
    if (svm->kernel_type == SVM_KERNEL_GAUSSIAN) {
        for (i = 0; i < svm->N; i++) {
            if (svm->alph[i] > 0) {
                K = 0.0;
                for (j = 0; j < svm->d; j++)
                    K += (svm->dense_points[i][j] - x[j]) *
                         (svm->dense_points[i][j] - x[j]);
                y += svm->alph[i] * svm->target[i] *
                     exp(-K / svm->two_sigma_squared);
            }
        }
        y -= svm->b;
    }
...
void compute_svm_bagging(BSupportVectorMachine *bsvm, int bagging, int nsamples,
                         int nvar, double **data, int *data_class,
                         int svm_kernel, double kp, double C, double tol,
                         double eps, int maxloops, int svm_verbose,
                         double *svm_W)
{
    int i, b;
    int *bsamples;
    ...
    bsvm->svm =
        (SupportVectorMachine *)G_calloc(bagging, sizeof(SupportVectorMachine));
    bsvm->nsvm = bagging;
    ...
    for (b = 0; b < bsvm->nsvm; b++) {
        ...
        compute_svm(&(bsvm->svm[b]), nk, nvar, xdata_training,
                    xclasses_training, svm_kernel, kp, C, tol, eps, maxloops,
                    svm_verbose, svm_W);
    }
 ...
 */
{
    ...
    for (b = 0; b < bsvm->nsvm; b++) {
        ...
        fprintf(fp, "%e\t%e\t%d\n", bsvm->svm[b].tolerance, bsvm->svm[b].eps,
                bsvm->svm[b].maxloops);
                          ...
                          double *svm_W, int weights_boosting)
{
    ...
    for (b = 0; b < bsvm->nsvm; b++) {
        ...
        compute_svm(&(bsvm->svm[b]), nk, nvar, xdata_training,
                    xclasses_training, svm_kernel, kp, C, tol, svm_eps,
                    maxloops, svm_verbose, svm_W);
 ...
 */
{
    ...
    for (i = 0; i < features->nexamples; i++) {
        for (j = 0; j < features->nclasses; j++) {
            if (features->class[i] == features->p_classes[j]) {
                data_in_each_class[j] += 1;
                if ((pred = predict_bsvm(bsvm, features->value[i])) *
                        features->class[i] <=
                    0.0) {
                    error[j] += 1.0;
                    accuracy += 1.0;
        ...
        if ((b < 100 && b % 2 == 1) || (b >= 100) || (b == bsvm->nsvm)) {
            ...
            for (i = 0; i < features->nexamples; i++) {
                for (j = 0; j < features->nclasses; j++) {
                    if (features->class[i] == features->p_classes[j]) {
                        data_in_each_class[j] += 1;
                        if ((pred = predict_bsvm_progressive(
                                 bsvm, features->value[i], b)) *
                                features->class[i] <=
                            0.0) {
                            error[j] += 1.0;
                            accuracy += 1.0;
