...
class TestLutShape(unittest.TestCase):
    ...
    def test_R_atm_shape(self):
        self.assertEqual(self.lut.R_atm.shape, (len(_AOD), len(_H2O), len(_WL)))

    def test_T_down_shape(self):
        self.assertEqual(self.lut.T_down.shape, (len(_AOD), len(_H2O), len(_WL)))

    def test_T_up_shape(self):
        self.assertEqual(self.lut.T_up.shape, (len(_AOD), len(_H2O), len(_WL)))

    def test_s_alb_shape(self):
        self.assertEqual(self.lut.s_alb.shape, (len(_AOD), len(_H2O), len(_WL)))
...
class TestLutPhysicalBounds(unittest.TestCase):
    ...
    def test_all_finite(self):
        for name in ("R_atm", "T_down", "T_up", "s_alb"):
            ...
            self.assertTrue(
                np.all(np.isfinite(arr)), msg=f"Non-finite values in {name}"
            )
...
class TestLutAerosolModels(unittest.TestCase):
    def test_rayleigh_only_R_atm_at_blue(self):
        ...
        self.assertGreater(
            R_blue, 0.05, msg=f"R_atm(Rayleigh, 450nm) too low: {R_blue:.4f}"
        )
        self.assertLess(
            R_blue, 0.20, msg=f"R_atm(Rayleigh, 450nm) too high: {R_blue:.4f}"
        )
    ...
    def test_no_aerosol_lower_R_atm_at_blue(self):
        ...
        self.assertLessEqual(
            float(lut_n.R_atm[-1, 0, 0]), float(lut_c.R_atm[-1, 0, 0]) + 1e-4
        )
...
class TestPolarization(unittest.TestCase):
    ...
    def test_polar_Q_positive_at_blue(self):
        ...
        self.assertGreater(Q_blue, 0.0, msg=f"R_atmQ = {Q_blue:.4f} ≤ 0 at 450 nm")
