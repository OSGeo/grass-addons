 ...
 */
static float log_interp(float y_inf, float y_sup, float wl_inf, float coef,
                        float wl)
{
 ...
 * downward/upward transmittances, spherical albedo, and optional optical
 * depths.
 *
 ...
 *
 * \param[in]  ctx          6SV context with \c disc and \c aer populated by
 * sixs_discom().
                              ...
                              float *T_down_dir_out)
{
    ...
    *s_alb = 0.0f;
    ...
    for (int ll = 0; ll < NWL_DISC - 1; ll++) {
        if (wl > d->wldis[ll] && wl <= d->wldis[ll + 1]) {
            linf = ll;
            break;
        }
    }
    if (wl > d->wldis[NWL_DISC - 1])
        linf = NWL_DISC - 2;
    int lsup = linf + 1;
    ...
    *s_alb = log_interp(d->sphal[1][linf], d->sphal[1][lsup], wl_inf, coef, wl);
     ...
     * d->dtdir[1] = combined (Rayleigh + aerosol) direct transmittance at 20
     * wl. Log-log interpolated then scaled by the same ratio as T_down so that
