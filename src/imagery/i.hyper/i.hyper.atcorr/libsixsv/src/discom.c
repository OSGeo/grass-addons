/* DISCOM: scattering quantities at 20 reference wavelengths — ported from
 * 6SV2.1 DISCOM.f Fills ctx->disc with roatm, dtdir, dtdif, utdir, utdif, sphal
 * at 20 wavelengths. Call once per AOD value before interpolating to specific
 * wavelengths. */
...
static const float wldisc[NWL_DISC] = {0.350f, 0.400f, 0.412f, 0.443f, 0.470f,
                                       0.488f, 0.515f, 0.550f, 0.590f, 0.633f,
                                       0.670f, 0.694f, 0.760f, 0.860f, 1.240f,
                                       1.536f, 1.650f, 1.950f, 2.250f, 3.750f};

/* Component scattering and phase function pointers (mirrors aerosol.c) */
/** \cond INTERNAL */
static const float *const comp_sca_d[4] = {aerosol_dust_sca, aerosol_wate_sca,
 ...
 * \param[in]     iaer    Aerosol model index (0=none, 1=continental,
 * 2=maritime, 3=urban).
    ...
    for (int l = 0; l < NWL_DISC; l++) {
        ...
        if (iaer != 0) {
            ...
            pizmoy = (fabsf(1.0f - piza * coeff) > 1e-10f)
                         ? piza * (1.0f - coeff) / (1.0f - piza * coeff)
                         : piza;
        }
        else {
            /* No aerosol: reset betal to Rayleigh values */
            for (int k = 0; k <= ctx->quad.nquad - 3; k++)
                ctx->polar.betal[k] = 0.0f;
            ctx->polar.betal[0] = 1.0f;
            ctx->polar.betal[2] =
                0.1f; /* Rayleigh: simple 2-term approximation */
        }
...
#define RT_CALL(ta, tr, tap, trp)                                              \
    do {                                                                       \
        ...
        if (ipol) {                                                            \
            ...
            sixs_ospol(ctx, 0, (ta), (tr), pizmoy, (tap), (trp), palt, phirad, \
                       nt, mu, np, nfi, rm_full, gb_full, rp, xl, xlq, xlu,    \
                       xlphim);                                                \
        }                                                                      \
        else {                                                                 \
            sixs_os(ctx, 0, (ta), (tr), pizmoy, (tap), (trp), palt, phirad,    \
                    nt, mu, np, nfi, rm_full, gb_full, rp, xl, xlphim, NULL);  \
        }                                                                      \
    } while (0)
...
#undef RT_CALL
        ...
        sixs_scatra(ctx, tamoy, tamoyp, tray, trayp, pizmoy, palt, nt, mu,
                    rm_full, gb_full, xmus, xmuv, &ddirtt, &ddiftt, &udirtt,
                    &udiftt, &sphalbt, &ddirtr, &ddiftr, &udirtr, &udiftr,
                    &sphalbr, &ddirta, &ddifta, &udirta, &udifta, &sphalba);

        /* Store: [0]=Rayleigh, [1]=total, [2]=aerosol */
        ctx->disc.dtdir[0][l] = ddirtr;
        ctx->disc.dtdif[0][l] = ddiftr;
        ctx->disc.utdir[0][l] = udirtr;
        ctx->disc.utdif[0][l] = udiftr;
        ctx->disc.sphal[0][l] = sphalbr;

        ctx->disc.dtdir[1][l] = ddirtt;
        ctx->disc.dtdif[1][l] = ddiftt;
        ctx->disc.utdir[1][l] = udirtt;
        ctx->disc.utdif[1][l] = udiftt;
        ctx->disc.sphal[1][l] = sphalbt;

        ctx->disc.dtdir[2][l] = ddirta;
        ctx->disc.dtdif[2][l] = ddifta;
        ctx->disc.utdir[2][l] = udirta;
        ctx->disc.utdif[2][l] = udifta;
