...
static void compute_iso(SixsCtx *ctx, float taer, float tray, float pizmoy,
                        float taerp, float trayp, float palt, int nt, int mu,
                        const float *rm, const float *gb, float *xtrans_m1,
                        float *xtrans_0, float *xtrans_1);
 ...
 */
void sixs_scatra(SixsCtx *ctx, float taer, float taerp, float tray, float trayp,
                 float piza, float palt, int nt, int mu, const float *rm,
                 const float *gb, float xmus, float xmuv, float *ddirtt,
                 float *ddiftt, float *udirtt, float *udiftt, float *sphalbt,
                 float *ddirtr, float *ddiftr, float *udirtr, float *udiftr,
    ...
    for (int it = 1; it <= 3; it++) {
        ...
        if (it == 1) {
            if (palt > 900.0f) {
                ...
                *udiftt = (2.f / 3.f + xmuv) +
                          (2.f / 3.f - xmuv) * expf(-tray / xmuv);
                *udiftt = *udiftt / (4.f / 3.f + tray) - expf(-tray / xmuv);
                *ddiftt = (2.f / 3.f + xmus) +
                          (2.f / 3.f - xmus) * expf(-tray / xmus);
 ...
 * - \c *xtrans_1  — downward diffuse transmittance through the total
 * atmosphere.
 *
...
static void compute_iso(SixsCtx *ctx, float tamoy, float trmoy, float pizmoy,
                        float tamoyp, float trmoyp, float palt, int nt, int mu,
                        const float *rm, const float *gb, float *xtrans_m1,
                        float *xtrans_0, float *xtrans_1)
{
    *xtrans_m1 = 0.f;
    ...
    *xtrans_1 = 0.f;
    ...
    if (!h || !xdel || !ydel || !altc || !i1 || !i2 || !i3 || !in_ || !inm1 ||
        !inm2 || !xpl || !psl || !bp)
        goto cleanup;
...
#define BP(j, k) bp[(j) * dim + (k) + mu]
    ...
    double tavion = I1(iplane, mu), tavion0 = tavion, tavion1 = 0.0,
           tavion2 = tavion;
    ...
    for (;;) {
        ...
        if (ig > 2) {
            ...
            if (tavion2 >= accu && tavion1 >= accu && tavion >= accu) {
                double y = fabs(
                    ((tavion0 / tavion1 - tavion1 / tavion2) /
            ...
            for (int l = -mu; l <= mu; l++) {
                ...
                if (INM2(l) <= accu || INM1(l) <= accu || I3(l) <= accu)
                    continue;
                double y =
                    fabs(((IN(l) / INM1(l) - INM1(l) / INM2(l)) /
