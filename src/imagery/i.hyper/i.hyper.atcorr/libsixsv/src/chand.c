...
/**
 * \brief Compute molecular (Rayleigh) reflectance via Chandrasekhar's
 ...
 *
 * \param[in] xphi  Relative azimuth angle between Sun and observer (degrees; 0
 * = backscattering).
...
{
    static const float as0[10] = {0.33243832f,   -6.777104e-02f, 0.16285370f,
                                  1.577425e-03f, -0.30924818f,   -1.240906e-02f,
                                  -0.10324388f,  3.241678e-02f,  0.11493334f,
                                  -3.503695e-02f};
    ...
    float xph1 = 1.0f + (3.0f * xmus * xmus - 1.0f) *
                            (3.0f * xmuv * xmuv - 1.0f) * xfd / 8.0f;
    float xph2 = -xmus * xmuv * sqrtf(1.0f - xmus * xmus) *
                 sqrtf(1.0f - xmuv * xmuv) * xfd * 0.5f * 1.5f;
    float xph3 =
        (1.0f - xmus * xmus) * (1.0f - xmuv * xmuv) * xfd * 0.5f * 0.375f;
    ...
    float xrray =
        xitot1 * xcosf1 + xitot2 * xcosf2 * 2.0f + xitot3 * xcosf3 * 2.0f;
    xrray /= xmus;
