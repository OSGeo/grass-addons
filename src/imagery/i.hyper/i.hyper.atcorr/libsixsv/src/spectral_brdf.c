 ...
 * \param[out] y_out     Interpolated output values (caller-allocated, length \c
 * n_out).
 */
static void spectral_interp_linear(const float *x_anchor, const float *y_anchor,
                                   int n_anchor, const float *x_out, int n_out,
                                   float *y_out)
{
...
/**
 * \brief Smooth a spectral array in-place using second-difference Tikhonov
 * regularisation.
 *
...
void spectral_smooth_tikhonov(float *f, int n, float alpha)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (A0[i] <= 0.0f)
            A0[i] = 1e-12f; /* guard (shouldn't happen for SPD) */
...
/**
 * \brief Disaggregate MCD43 7-band Ross-Li kernel weights to an arbitrary
 * wavelength grid.
 *
 ...
 * \param[out] fiso_wl   Disaggregated f_iso at \c wl_target (caller-allocated,
 * length \c n_wl).
