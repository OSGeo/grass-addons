 ...
 */
void adjacency_r_env(const float *r_boa, float *r_env, int nrows, int ncols,
                     int filter_half)
{
...
/**
 * \brief Compute the Beer-Lambert direct (beam) transmittance for adjacency
 * correction.
 *
 * Calculates the combined direct transmittance through Rayleigh + aerosol
 * along the two-way path (solar down + view up):
 * \f[T_\text{dir} = \exp(-\tau / \cos\theta_s) \cdot \exp(-\tau /
 * \cos\theta_v)\f]
 *
 ...
 */
float adjacency_T_dir(float wl_um, float aod550, float pressure, float sza_deg,
                      float vza_deg)
{
    ...
    float wl4 = wl2 * wl2;
    float tau_r =
        0.008569f / wl4 * (1.0f + 0.0113f / wl2) * (pressure / 1013.25f);
 ...
 * \c pixel_size_m.
 *
 * \param[in,out] r_boa          Surface reflectance array [nrows × ncols];
 * corrected in place.
 * \param[in]     nrows          Number of rows.
 * \param[in]     ncols          Number of columns.
 * \param[in]     psf_radius_km  Environmental PSF radius (km; half-width of the
 * averaging window).
 * \param[in]     pixel_size_m   Pixel size in metres (used to convert km →
 * pixels).
 * \param[in]     T_scat         Combined scattering transmittance
 * \f$T_\downarrow T_\uparrow\f$.
 * \param[in]     s_alb          Spherical albedo of the atmosphere.
 * \param[in]     wl_um          Wavelength in µm.
 * \param[in]     aod550         AOD at 550 nm (for direct transmittance
 * calculation).
 * \param[in]     pressure       Surface pressure in hPa.
 * \param[in]     sza_deg        Solar zenith angle (degrees).
 * \param[in]     vza_deg        View zenith angle (degrees).
...
void adjacency_correct_band(float *r_boa, int nrows, int ncols,
                            ...
                            float T_scat, float s_alb, float wl_um,
                            float aod550, float pressure, float sza_deg,
                            float vza_deg)
{
