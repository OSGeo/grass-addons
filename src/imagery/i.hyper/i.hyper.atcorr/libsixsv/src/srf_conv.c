/**
 * \file srf_conv.c
 * \brief Spectral Response Function (SRF) convolution for gas transmittance
 * correction.
 *
...
/* srf_conv.c — Spectral Response Function (SRF) convolution for gas
 * transmittance.
 *
 * For hyperspectral sensors with FWHM < 5 nm, the 6SV Curtis-Godson
...
{
    static const char *cands[] = {"/usr/local/bin/uvspec", "/usr/bin/uvspec",
                                  NULL};
...
{
    static const char *cands[] = {"/usr/local/share/libRadtran/data",
                                  "/usr/share/libRadtran/data", NULL};
 ...
 *
 * \param[in] sza_deg     Zenith angle of the beam path (degrees; solar or
 * view).
 ...
 * \param[in] resolution Resolution string: "fine" (~0.05 nm) or "coarse" (~0.16
 * nm).
 ...
 * \param[in] thread_id  OpenMP thread ID (used to create unique temp
 * directories).
...
static GasSpectrum *run_uvspec_gas(float sza_deg, float h2o_gcm2, float o3_du,
                                   ...
                                   const char *data_path, int thread_id)
{
    ...
    fprintf(f,
            ...
            "quiet\n",
            data_path, data_path, data_path, (double)wl_lo, (double)wl_hi,
            resolution, (double)h2o_mm, (double)(o3_du > 0.0f ? o3_du : 300.0f),
            (double)sza_deg);
    fclose(f);
    ...
    while (fgets(line, sizeof(line), pipe)) {
        if (line[0] == '#' || line[0] == '\n')
            continue;
        float wl_v, edir_v;
        if (sscanf(line, "%f %f", &wl_v, &edir_v) != 2)
            continue;
                                  ...
                                  const LutConfig *lut_cfg)
{
    ...
    if (find_data_path(data_path, sizeof(data_path)) != 0) {
        fprintf(stderr,
                "[srf_conv] libRadtran data not found — set LIBRADTRAN_DATA\n");
        return NULL;
    }
    ...
    if (n_to_correct == 0) {
        fprintf(stderr,
                "[srf_conv] no bands below %.1f nm threshold — "
                "skipping SRF correction\n",
                thresh * 1000.0f);
        return NULL;
    }
    fprintf(stderr,
            "[srf_conv] SRF correction: %d/%d bands need reptran fine "
            "(threshold %.1f nm)\n",
...
#pragma omp parallel for schedule(dynamic)
    for (int run = 0; run < total_runs; run++) {
        ...
        if (!spectra[run])
            fprintf(stderr,
                    "[srf_conv] run %d failed "
                    "(%s, %s, H2O=%.2f)\n",
                    run, res, i_dir == 0 ? "down" : "up", h2o_v);
    }
    ...
    for (int ih = 0; ih < n_h2o; ih++) {
        ...
        for (int iw = 0; iw < n_wl; iw++) {
            ...
            if (srf_cfg && srf_cfg->fwhm_um && srf_cfg->fwhm_um[iw] >= thresh)
                continue;
            ...
            if (fwhm_nm > 0.05f) {
                ...
                float Tcd_ref =
                    interp1d_nm(cd->wl, cd->T_gas, cd->n_pts, wl_nm);
                float Tcu_ref =
                    interp1d_nm(cu->wl, cu->T_gas, cu->n_pts, wl_nm);

                srf->corr_down[idx] =
                    (Tcd_ref > 1e-6f) ? Tfd_srf / Tcd_ref : 1.0f;
                srf->corr_up[idx] =
                    (Tcu_ref > 1e-6f) ? Tfu_srf / Tcu_ref : 1.0f;
            }
            else {
