/**
 * \file retrieve.c
 * \brief Image-based atmospheric state retrievals (H₂O, AOD, O₃, pressure,
 * quality).
 *
...
/**
 * \brief Retrieve per-pixel column water vapour from the 940 nm absorption
 * band.
 *
 ...
 * at 470 and 660 nm is estimated from the 2130 nm band via empirical ratios,
 * and the difference from TOA reflectance yields the AOD.  Non-DDV pixels
 * receive the scene-mean AOD.
 *
                       ...
                       int doy, float sza_deg, float *out_aod)
{
    ...
    for (int i = 0; i < npix; i++) {
        ...
        if (rho_path_470 < 1e-5f)
            rho_path_470 = 1e-5f;
        if (rho_path_660 < 1e-5f)
            rho_path_660 = 1e-5f;
 ...
 * \param[in] sza_deg   Solar zenith angle (degrees).
 * \param[in] vza_deg   View zenith angle (degrees).
 * \return Scene-mean ozone column in Dobson units; fallback to 300 DU on
 * failure.
 */
                           ...
                           float vza_deg)
{
    ...
    for (int i = 0; i < npix; i++) {
        ...
        if (!(l5 > 0.0f) || !(l6 > 0.0f) || !(l7 > 0.0f))
            continue;

        float L_cont = l5 * (1.0f - FRAC) + l7 * FRAC;
        if (L_cont <= 0.0f || l6 >= L_cont)
            continue;
 ...
 * Beer-Lambert: \f$P = P_0 \times D / (K_{O2}(1/\cos\theta_s +
 * 1/\cos\theta_v))\f$.
 *
                           ...
                           float vza_deg, float *out_pressure)
{
    ...
    for (int i = 0; i < npix; i++) {
        ...
        if (D < 0.0f)
            D = 0.0f;
        if (D > 0.95f)
            D = 0.95f; /* cap: near-zero pressure is unphysical */
 ...
 * \param[in]  L_swir   Radiance at ~1600 nm [npix], or NULL to skip snow/ice
 * detection.
 * \param[in]  npix     Number of pixels.
                           ...
                           int doy, float sza_deg, uint8_t *out_mask)
{
     ...
     *
     * Bitmask:  MASK_CLOUD=0x01  MASK_SHADOW=0x02  MASK_WATER=0x04
     * MASK_SNOW=0x08
     *
 ...
 *
 * \param[in,out] aod_data  Per-pixel AOD map [nrows × ncols] (modified in
 * place).
...
void retrieve_aod_maiac(float *aod_data, int nrows, int ncols, int patch_sz)
{
    ...
    for (int pr = 0; pr < np_rows; pr++) {
        for (int pc = 0; pc < np_cols; pc++) {
            ...
            int r0 = pr * patch_sz, r1 = r0 + patch_sz;
            if (r1 > nrows)
                r1 = nrows;
            int c0 = pc * patch_sz, c1 = c0 + patch_sz;
            if (c1 > ncols)
                c1 = ncols;
...
/**
 * \brief Retrieve per-pixel column water vapour from a generic absorption
 * triplet.
 *
 * Generic CIBR retrieval using three bands (lo, feature, hi) bracketing a
 * water vapour absorption feature.  Supports the 720, 940, and 1135 nm
 * features. The FWHM-dependent absorption coefficient follows the same power
 * law as retrieve_h2o_940() with exponent α = 0.90.
 *
 ...
 * \param[in]  K_ref         Reference absorption coefficient (cm² g⁻¹) at \c
 * fwhm_ref_um.
 * \param[in]  fwhm_ref_um   Reference FWHM for \c K_ref calibration (µm).
 * \param[in]  fwhm_um       Sensor FWHM at the feature band (µm; 0 = use \c
 * K_ref unchanged).
 ...
 * \param[in]  sza_deg       Solar zenith angle (degrees).
 * \param[in]  vza_deg       View zenith angle (degrees).
 * \param[out] out_wvc       Per-pixel WVC in g cm⁻² [npix]; fallback to 2.0 for
 * invalid.
 * \param[out] out_valid     Per-pixel validity flag (1=valid, 0=invalid)
 * [npix]; may be NULL.
 */
 ...
 *
 * \param[in]  n_methods    Number of retrieval methods (typically 3: 720, 940,
 * 1135 nm).
 * \param[in]  wvc_arrays   Array of \c n_methods per-pixel WVC arrays [npix
 * each].
 * \param[in]  valid_arrays Array of \c n_methods per-pixel validity flags [npix
 * each].
 * \param[in]  npix         Number of pixels.
 ...
 */
static const float LEAF_ALB_WL_NM[] = {705.0f, 710.0f, 715.0f, 720.0f, 725.0f,
                                       730.0f, 735.0f, 740.0f, 745.0f, 750.0f,
                                       755.0f, 760.0f, 765.0f, 770.0f, 775.0f,
                                       780.0f, 785.0f, 790.0f, 795.0f};
static const float LEAF_ALB_VAL[] = {0.740f, 0.770f, 0.808f, 0.840f, 0.865f,
                                     0.881f, 0.892f, 0.900f, 0.906f, 0.910f,
                                     0.912f, 0.912f, 0.911f, 0.911f, 0.910f,
                                     0.909f, 0.908f, 0.907f, 0.906f};
 ...
 * \return Leaf single-scattering albedo in [0, 1]; −1 if outside the 705–795 nm
 * range.
 */
 ...
 *
 * \param[in]  refl      Surface reflectance cube [n_dasf × npix], band-major
 * order.
 * \param[in]  wl_dasf   Wavelengths of the DASF bands in µm [n_dasf].
 * \param[in]  n_dasf    Number of bands in the NIR plateau.
 * \param[in]  npix      Number of pixels.
 * \param[out] out_dasf  Per-pixel DASF [npix]; NaN for pixels with no valid
 * bands.
 */
