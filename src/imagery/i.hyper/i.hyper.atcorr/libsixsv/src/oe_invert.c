 ...
 *
 * Fits \f$y = a + b \cdot \lambda\f$ to \c n data points by ordinary least
 * squares. Used by oe_invert_aod_h2o() to assess spectral smoothness of the VIS
 * residuals.
 *
 ...
 */
static void _linfit(const float *wl, const float *y, int n, float *a_out,
                    float *b_out)
{
...
/**
 * \brief Compute the 940 nm water vapour band depth via continuum
 * interpolation.
 *
...
static float _h2o_depth(float l865, float l940, float l1040)
{
    ...
    if (!(l865 > 0.0f) || !(l940 > 0.0f) || !(l1040 > 0.0f))
        return -1.0f;
 ...
 * \f[
 *   J = \frac{\sum_b (\rho^\text{boa} -
 * \hat\rho^\text{boa}_b)^2}{\sigma_\text{spec}^2}
 *     + \frac{D_{940}^2}{\sigma_\text{h2o}^2}
 *     + \frac{(\ln\text{AOD} -
 * \ln\text{AOD}_\text{prior})^2}{\sigma_\text{aod}^2}
 * \f]
 ...
 * \param[in]  rho_toa_vis  TOA reflectance in VIS bands [n_vis × npix],
 * band-major.
 ...
 * \param[in]  sigma_spec   Expected spectral residual (dimensionless
 * reflectance).
 ...
 */
void oe_invert_aod_h2o(const LutConfig *cfg, const LutArrays *lut,
                       const float *rho_toa_vis, int npix, int n_vis,
                       const float *vis_wl, const float *L_865,
                       const float *L_940, const float *L_1040, float sza_deg,
                       float vza_deg, float aod_prior, float h2o_prior,
                       float sigma_aod, float sigma_h2o, float sigma_spec,
                       float fwhm_940_um, float *out_aod, float *out_h2o)
{
    /* K₉₄₀ scaled by sensor FWHM (same power law as retrieve_h2o_triplet,
     * α=0.90). Tanager 6.8 nm → K≈0.217; MODIS 50 nm → K=0.036. */
            ...
            for (int k = -1; k <= 1; k++) {
                ...
                if (do_spec) {
                    ...
                    if (ok) {
                        ...
                        for (int b = 0; b < n_vis; b++) {
                            float dr =
                                rho_boa_buf[b] - (a_fit + b_fit * vis_wl[b]);
                            J_s += dr * dr;
                        }
                        J_s /= sigma_spec2;
                    }
