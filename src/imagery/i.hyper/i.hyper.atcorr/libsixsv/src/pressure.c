 ...
 *
 * \param[in]  ctx   6SV context with the (possibly adjusted) atmospheric
 * profile.
...
static void compute_columns(const SixsCtx *ctx, float *uw, float *uo3)
{
    ...
    for (int k = 0; k < NATM - 1; k++) {
        float roair =
            air * 273.16f * ctx->atm.p[k] / (1013.25f * ctx->atm.t[k]);
...
/**
 * \brief Adjust the atmospheric profile to a given surface pressure or
 * altitude.
 *
...
void sixs_pressure(SixsCtx *ctx, float sp)
{
    ...
    if (sp < 0.0f) {
        ...
        int i = 0;
        while (i < NATM - 1 && ctx->atm.z[i] <= xps2)
            i++;
        isup = i;
        iinf = i - 1;
