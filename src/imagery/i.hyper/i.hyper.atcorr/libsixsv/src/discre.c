 ...
 *
 * \param[in,out] ctx   6SV context; \c ctx->err.ier set on aerosol scale-height
 * error.
 ...
 * \param[in]     dd    Aerosol fraction at the previous level (used for
 * convergence check).
 ...
 */
void sixs_discre(SixsCtx *ctx, double ta, double ha, double tr, double hr,
                 int it, int nt, double yy, double dd, double ppp2, double ppp1,
                 double *zx)
{
    if (ha >= 7.0) {
        fprintf(stderr,
                "atcorr: check aerosol measurements or plane altitude\n");
        ctx->err.ier = true;
        return;
    }
    double dt;
