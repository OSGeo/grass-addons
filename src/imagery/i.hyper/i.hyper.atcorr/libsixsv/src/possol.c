 ...
 * \param[out] phi0  Solar azimuth angle in degrees (measured from north,
 * clockwise).
 */
static void pos_fft(int j, float tu, float xlon, float xlat, float *asol,
                    float *phi0)
{
    ...
    float et = 0.000075f + 0.001868f * cosf(tet) - 0.032077f * sinf(tet) -
    ...
    float delta = 0.006918f - 0.399912f * cosf(tet) + 0.070257f * sinf(tet) -
                  0.006758f * cosf(2.0f * tet) + 0.000907f * sinf(2.0f * tet) -
                  0.002697f * cosf(3.0f * tet) + 0.001480f * sinf(3.0f * tet);

    /* Elevation and azimuth */
    float amuzero =
        sinf(xla) * sinf(delta) + cosf(xla) * cosf(delta) * cosf(ah);
    float elev = asinf(amuzero);
    float az = cosf(delta) * sinf(ah) / cosf(elev);
    if (fabsf(az) > 1.0f)
        az = (az > 0.0f) ? 1.0f : -1.0f;
    float caz =
        (-cosf(xla) * sinf(delta) + sinf(xla) * cosf(delta) * cosf(ah)) /
        cosf(elev);
    float azim = asinf(az);
    if (caz <= 0.0f)
        azim = pi - azim;
