 ...
 *
 * \param[in]  ctx   6SV context with initialised atmosphere (\c
...
void sixs_odrayl(const SixsCtx *ctx, float wl, float *tray)
{
    ...
    double a = (24.0 * M_PI * M_PI * M_PI) *
               ((an * an - 1.0) * (an * an - 1.0)) * (6.0 + 3.0 * delta) /
               (6.0 - 7.0 * delta) / ((an * an + 2.0) * (an * an + 2.0));
    ...
    for (int k = 0; k < NATM - 1; k++) {
        double dppt = (288.15 / 1013.25) *
                      ((double)(p[k] / t[k]) + (double)(p[k + 1] / t[k + 1])) /
                      2.0;
