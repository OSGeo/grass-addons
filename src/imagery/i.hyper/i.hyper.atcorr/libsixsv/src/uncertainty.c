/**
 * \file uncertainty.c
 * \brief Per-band reflectance uncertainty propagation (noise + AOD
 * perturbation).
 *
...
/**
 * \brief Estimate the Noise-Equivalent Radiance Difference (NEDL) from image
 * data.
 *
 ...
 *    \f$\sigma_\rho^\text{noise} = \pi \cdot \text{NEDL} \cdot d^2 / (E_0
 * \cos\theta_s)\f$, then propagated through the 6SV inversion Jacobian.
 * 2. **AOD uncertainty** — re-runs the inversion at \f$\text{AOD} \pm
 * \sigma_\text{AOD}\f$ and takes half the difference as a one-sigma AOD
 * perturbation error.
 *
 * If \c nedl ≤ 0 it is estimated from \c rad_band via
 * uncertainty_estimate_nedl().
 *
 * \param[in]  rad_band   Radiance array for this band [npix] (W m⁻² sr⁻¹ µm⁻¹);
 * may be NULL.
 * \param[in]  refl_band  Corrected surface reflectance array [npix].
 * \param[in]  npix       Number of pixels.
 * \param[in]  E0         Exo-atmospheric solar irradiance at band centre (W m⁻²
 * µm⁻¹).
 ...
 * \param[in]  nedl       Instrument NEDL (W m⁻² sr⁻¹ µm⁻¹); 0 → estimate from
 * \c rad_band.
 * \param[in]  aod_sigma  One-sigma AOD uncertainty for the perturbation test.
 * \param[in]  cfg        LUT configuration (used for AOD perturbation LUT
 * query).
 ...
 * \param[out] sigma_out  Per-pixel combined uncertainty (1σ reflectance)
 * [npix].
 */
void uncertainty_compute_band(const float *rad_band, const float *refl_band,
                              int npix, float E0, float d2, float cos_sza,
                              float T_down, float T_up, float s_alb,
                              float R_atm, float nedl, float aod_sigma,
                              const LutConfig *cfg, const LutArrays *lut,
                              float wl_um, float aod_val, float h2o_val,
                              float *sigma_out)
{
    ...
    float aod_minus =
        (aod_val > aod_sigma + 0.001f) ? aod_val - aod_sigma : 0.001f;

    float R1, Td1, Tu1, s1;
    float R0, Td0, Tu0, s0;
    atcorr_lut_interp_pixel(cfg, lut, aod_plus, h2o_val, wl_um, &R1, &Td1, &Tu1,
                            &s1);
    atcorr_lut_interp_pixel(cfg, lut, aod_minus, h2o_val, wl_um, &R0, &Td0,
                            &Tu0, &s0);

    /* ── Per-pixel uncertainty: offloaded to GPU via OpenMP target ── */
#ifdef _OPENMP
#pragma omp target teams distribute parallel for map(to : refl_band[0 : npix]) \
