...
static float brdf_rahman(const BrdfParams *p, float mu1, float mu2, float fi)
{
    float rho0 = p->rahman.rho0;
    float af = p->rahman.af;
    float k = p->rahman.k;

    float cospha = mu1 * mu2 +
                   sqrtf(1.0f - mu1 * mu1) * sqrtf(1.0f - mu2 * mu2) * cosf(fi);
    ...
    float coef1 =
        powf(mu1, k - 1.0f) * powf(mu2, k - 1.0f) / powf(mu1 + mu2, 1.0f - k);
    /* Phase function: HG with argument -cospha (backscatter convention) */
    float phafun =
        (1.0f - af * af) / powf(1.0f + af * af - 2.0f * af * (-cospha), 1.5f);
...
static float brdf_roujean(const BrdfParams *p, float xmus, float xmuv, float fi)
{
    ...
    float cpsi = xmus * xmuv + sqrtf(1.0f - xmus * xmus) *
                                   sqrtf(1.0f - xmuv * xmuv) * cosf(fi);
    ...
    float f2 = 4.0f / (3.0f * pi * (xmus + xmuv)) *
                   ((pi / 2.0f - psi) * cpsi + sinf(psi)) -
               1.0f / 3.0f;

    float f1 = 0.5f * ((pi - fr) * cosf(fr) + sinf(fr)) * tts * ttv - tts -
               ttv - sqrtf(tts * tts + ttv * ttv - 2.0f * tts * ttv * cosf(fr));
    f1 /= pi;
...
static float brdf_hapke(const BrdfParams *p, float mu1, float mu2, float fi)
{
    float om = p->hapke.om;
    float af = p->hapke.af;
    float s0 = p->hapke.s0;
...
static float brdf_ocean(const BrdfParams *p, float mu1, float mu2, float fi)
{
    ...
    float norm_n = sqrtf(sin1 * sin1 + sin2 * sin2 +
                         2.0f * sin1 * sin2 * cosf(fi + azw) + cos_n * cos_n);
    ...
    float tan_tilt2 =
        (cos_tilt2 > 1e-6f) ? (1.0f - cos_tilt2) / cos_tilt2 : 0.0f;
    float p_cox = (sigma2 > 0.0f)
                      ? expf(-tan_tilt2 / (2.0f * sigma2)) /
                            ((float)M_PI * sigma2 * cos_tilt2 * cos_tilt2)
                      : 0.0f;
 ...
 */
/* WALTBRDF.f */
static float brdf_walthall(const BrdfParams *p, float xmus, float xmuv,
                           float fi)
{
    ...
    return p->walthall.a * (ts * ts * tv * tv) +
           ...
           p->walthall.b * ts * tv * cosf(phi) + p->walthall.c;
}

/* ── Minnaert ───────────────────────────────────────────────────────────────
 */
/* MINNBRDF.f: par1=k (exponent), par2=b (albedo normalisation) */
static float brdf_minnaert(const BrdfParams *p, float xmus, float xmuv)
{
...
static float brdf_rosslimaignan(const BrdfParams *p, float mu_s, float mu_v,
                                float fi)
{
    const float pi = (float)M_PI;
    ...
    float rosselt = ct0 * (((pi - 2.0f * rpha) * cpha + 2.0f * sinf(rpha)) /
                           (cts + ctv + 1e-10f));
    float rossthick =
        rosselt * (1.0f + 1.0f / (1.0f + rpha / (1.5f * pi / 180.0f))) -
        1.0f / 3.0f;
    ...
    float angdist =
        sqrtf(clampf(tanti * tanti + tantv * tantv - 2.0f * tanti * tantv * cfi,
                     0.0f, 1e10f));
    float angtemp = 1.0f / (cts + 1e-10f) + 1.0f / (ctv + 1e-10f);

    float cost =
        2.0f *
        sqrtf(angdist * angdist + tanti * tanti * tantv * tantv * sfi * sfi) /
        (angtemp + 1e-10f);
    ...
    float lispars =
        angover - angtemp + 0.5f * (1.0f + cpha) / (cts * ctv + 1e-10f);

    float result = p->rosslimaignan.f_iso + p->rosslimaignan.f_vol * rossthick +
 ...
 * ══════════════════════════════════════════════════════════════════════════════
 */
float sixs_brdf_eval(BrdfType type, const BrdfParams *params, float cos_sza,
...
static float brdf_lisparse_std(float sza_deg, float vza_deg, float raa_deg)
{
    ...
    /* cos(t): h/b = 2 factor */
    float cost = 2.0f *
                 sqrtf(clampf(D2 + tants * tants * tantv * tantv * sfi * sfi,
                              0.0f, 1e10f)) /
                 (sec_sum + 1e-10f);
    cost = clampf(cost, -1.0f, 1.0f);
    float t = acosf(cost);
...
float atcorr_brdf_normalize(float rho_boa, float f_iso, float f_vol,
                            float f_geo, float sza_obs, float vza_obs,
                            float raa_obs, float sza_nbar)
{
