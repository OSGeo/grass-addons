/**
 * \file polglit.c
 * \brief Polarized surface reflectance — C port of 6SV2.1 POLGLIT.f and
 * POLNAD.f.
 *
                  ...
                  float *ropq, float *ropu)
{
    ...
    *ropu = 0.0f;
    ...
    double r2 =
        0.5 * (rl * rl - rr * rr); /* polarizing term; r3=rl*rr set to 0 */
    ...
    double coef = 1.0 - C21 / 2.0 * (xe2 - 1.0) * xn -
                  C03 / 6.0 * (xn2 - 3.0) * xn +
    ...
    double proba =
        coef / (2.0 * pi * sqrt(sigmaU * sigmaC)) * exp(-(xe2 + xn2) * 0.5);

    double ct = cos(tilt);
    double ct4 = ct * ct * ct * ct;
    if (ct4 < 1e-12)
        return; /* surface near-horizontal; avoid ÷0 */

    double factor = pi * proba / (4.0 * cs * cv * ct4);

    /* Rotation angle cksi (cosine of rotation from scattering to meridional
     * plane) */
    double cksi;
    if (xtv > 0.5f) {
        ...
        if (denom < 1e-12)
            denom = 1e-12;
        cksi = (sphi < 0.0) ? (cxtv * csca + cxts) / denom
...
/**
 * \brief Compute polarised Stokes Q and U for vegetation/soil (Nadal-Breon
 * model).
 *
 ...
 */
void sixs_polnad(float xts, float xtv, float phi, float pveg, float *ropq,
                 float *ropu)
{
    *ropq = 0.0f;
    *ropu = 0.0f;
