...
/**
 * \brief Build the Legendre-polynomial RT kernel matrices for scalar radiative
 * transfer.
 *
 ...
 * \param[out] xpl_off  Legendre values at quadrature nodes, size 2mu+1 (offset
 * by mu).
 ...
 */
void sixs_kernel(
    const SixsCtx *ctx, int is, int mu,
...
#define RM(j) rm_off[(j) + mu]
    ...
    if (k < ip1) {
        ...
        for (int l = k; l <= ip1 - 1; l++) {
            int lp = l + 1, lm = l - 1;
            double aa =
                (2.0 * l + 1.0) / sqrt((double)((l + is + 1) * (l - is + 1)));
