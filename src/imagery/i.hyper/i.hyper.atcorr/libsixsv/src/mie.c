...
static const float wldisc_mie[NWL_DISC] = {
    0.350f, 0.400f, 0.412f, 0.443f, 0.470f, 0.488f, 0.515f,
    0.550f, 0.590f, 0.633f, 0.670f, 0.694f, 0.760f, 0.860f,
    1.240f, 1.536f, 1.650f, 1.950f, 2.250f, 3.750f};

/**
 * \brief Compute Mie extinction/scattering efficiencies and phase function for
 * a sphere.
 *
 ...
 * \param[in]  ni        Imaginary part (absorption; positive value means
 * absorbing).
 * \param[out] Qext_out  Extinction efficiency \f$Q_\text{ext}\f$.
 * \param[out] Qsca_out  Scattering efficiency \f$Q_\text{sca}\f$.
 * \param[out] p11       Scattering phase function at \c nbmu Gauss quadrature
 * angles.
 ...
 */
static void exscphase(double X, double nr, double ni, double *Qext_out,
                      double *Qsca_out, double *p11, int nbmu,
                      const float *cgaus_S, const float *pdgs_S)
{
    ...
    int mu1 = (int)(N + 30.0 * (0.10 + 0.35 * Up * (2 - Up * Up) / 2.0 /
                                           (1 - Up + 1e-30)));
    int Np = (int)(Y - 0.5 + sqrt(30.0 * 0.35 * Y));
    int mu2 = mu1;
    if (Np > N) {
        Up = 2.0 * Y / (2.0 * Np + 1.0);
        mu2 = (int)(Np + 30.0 * (0.10 + 0.35 * Up * (2 - Up * Up) / 2.0 /
                                            (1 - Up + 1e-30)));
    }
    ...
    if (mu < 2)
        mu = 2;
    if (mu > 50000)
        mu = 50000; /* hard safety cap */

    /* Dynamic allocation for Mie recursion arrays (0..mu) */
    double *xj = (double *)calloc(mu + 2, sizeof(double));
    double *xy = (double *)calloc(
        mu + 2, sizeof(double)); /* shifted: index 0=xy[-1], 1=xy[0], ... */
    ...
    if (!xj || !xy || !Rn || !RDnX || !RDnY || !IDnY || !RGnX || !IGnX ||
...
seed_found:;
    ...
    for (int k = 1; k <= mu; k++) {
        ...
        /* Gn */
        double den_G =
            (RGnX[k - 1] - (double)k / X) * (RGnX[k - 1] - (double)k / X) +
            IGnX[k - 1] * IGnX[k - 1];
        if (den_G > 0.0) {
        ...
        /* Convergence test */
        double test = (RAn[k] * RAn[k] + IAn[k] * IAn[k] + RBn[k] * RBn[k] +
                       IBn[k] * IBn[k]) /
                      (double)k;
        if (test < 1.0e-14) {
            mu_final = k;
        ...
        Qsca += xpond * (RAn[k] * RAn[k] + IAn[k] * IAn[k] + RBn[k] * RBn[k] +
                         IBn[k] * IBn[k]);
    ...
    for (int j = 0; j < nbmu; j++) {
        ...
        for (int k = 1; k <= mu_final; k++) {
            ...
            PIn[k + 1] =
                ((2.0 * k + 1) * xmud * PIn[k] - (k + 1.0) * PIn[k - 1]) /
                (double)k;
            TAUn[k + 1] = (k + 1.0) * xmud * PIn[k + 1] - (k + 2.0) * PIn[k];
        }
        p11[j] =
            (RS1 * RS1 + IS1 * IS1 + RS2 * RS2 + IS2 * IS2) / (X * X * 2.0);
    }
...
/**
 * \brief Initialise a custom log-normal aerosol by computing Mie scattering
 * properties.
 *
 ...
 *       all 20 wavelengths.  For a spectrally varying index, call
 * aerosol_init() with a pre-built mixture table instead.
 *
 * \param[in,out] ctx      6SV context; writes \c ctx->aer on return.
 * \param[in]     r_mode   Mode radius of the log-normal distribution (µm).
 * \param[in]     sigma_g  Geometric standard deviation (dimensionless; must be
 * > 1).
 * \param[in]     m_r_550  Real part of the refractive index at 550 nm.
 * \param[in]     m_i_550  Imaginary part of the refractive index at 550 nm (> 0
 * = absorbing).
 */
                   ...
                   double m_i_550)
{
    ...
    while (r < rmax) {
        ...
        for (int l = 0; l < NWL_DISC; l++) {
            ...
            double Qext, Qsca;
            exscphase(X, m_r_550, m_i_550, &Qext, &Qsca, p11, nbmu, cgaus_S,
                      pdgs_S);
    ...
    if (sca_acc[7] > 0.0) {
        ...
        for (int j = 0; j < nbmu; j++) {
            ...
            for (int k = 0; k <= nbmu - 3; k++) {
                float plnew =
                    ((2.0f * k + 1.0f) * rm_ * pl0 - (float)k * plm1) /
                    (float)(k + 1);
                betal[k] += x * pl0;
                plm1 = pl0;
                pl0 = plnew;
