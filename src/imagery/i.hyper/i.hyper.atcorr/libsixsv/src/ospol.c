...
/**
 * \brief Compute atmospheric path reflectance and polarisation via vector
 * successive orders.
 *
 ...
 *
 * On return:
 * - \c xl[0] contains \f$I \cdot \cos\theta_s\f$ (total path reflectance × cos
 * SZA),
 * - \c xlq[0] and \c xlu[0] contain the corresponding \f$Q\f$ and \f$U\f$
 * components.
 *
 * Ported from 6SV2.1 OSPOL.f.
 *
 * \param[in,out] ctx       6SV context (reads polar.betal, del.delta; writes
 * err).
 * \param[in]     iaer_prof Aerosol profile type (0 = exponential; only value
 * supported).
 ...
 * \param[out]    xl        Stokes-I radiance [(2mu+1)*np]; \c xl[0] = I × cos
 * SZA.
...
#define XL(m, l)  xl[((m) + mu) * np + (l)]
    ...
    for (int is = 0; is <= iborm; is++) {
        ...
        (void)srl;
        (void)stl; /* Only spl enters the primary term */
        ...
        for (int j = -mu; j <= mu; j++) {
            ...
            for (int k = 0; k <= nt; k++) {
                ...
                U2(k, j) =
                    -c * (sc2 * b + sc1 * a); /* minus sign per OSPOL.f */
            }
        }
                ...
                for (int k = 1; k <= mu; k++) {
                    ...
                    for (int i = 0; i <= nt; i++) {
                        ...
                        for (int j = 1; j <= mu; j++) {
                             ...
                             */
                            double arrjk =
                                ARR(j, k) * x + y * alpha2 * xrj * xrk;
                            double arrjmk =
                                ARR(j, -k) * x + y * alpha2 * xrj * yrk;
                            double artjk =
                                ART(j, k) * x + y * alpha2 * xtj * xrk;
                            double artjmk =
                                ART(j, -k) * x + y * alpha2 * xtj * yrk;
                            double artkj =
                                ART(k, j) * x + y * alpha2 * xtk * xrj;
                            double artkmj =
                                ART(k, -j) * x + y * alpha2 * xtk * yrj;
                            double attjk =
                                ATT(j, k) * x + y * alpha2 * xtj * xtk;
                            double attjmk =
                                ATT(j, -k) * x + y * alpha2 * xtj * ytk;
...
#define CK3(n2, n1, n0, s3)                                                  \
    if (fabs(n2(l)) >= accu && fabs(n1(l)) >= accu && fabs(s3(l)) >= accu) { \
        double _r = (n0(l)) / (n1(l));                                       \
        double _y =                                                          \
            fabs(((_r - (n1(l)) / (n2(l))) / ((1.0 - _r) * (1.0 - _r))) *    \
                 ((n0(l)) / (s3(l))));                                       \
        z = fmax(_y, z);                                                     \
    }
                    CK3(INM2, INM1, IN, I3)
    ...
    done_sos:;
        ...
        for (int l_idx = 0; l_idx < np; l_idx++) {
            ...
            for (int m = -mum1; m <= mum1; m++) {
                double cosfac =
                    (m > 0) ? cos(is * (phi_l + pi)) : cos(is * phi_l);
                double sinfac =
                    (m > 0) ? sin(is * (phi_l + pi)) : sin(is * phi_l);
                XL(m, l_idx) += (float)(delta0s * I3(m) * cosfac);
                if (xlq)
                    xlq[((m) + mu) * np + l_idx] +=
                        (float)(delta0s * Q3(m) * cosfac);
                if (xlu)
                    xlu[((m) + mu) * np + l_idx] +=
                        (float)(delta0s * U3(m) * sinfac);
            }
        }
        ...
        if (xlq)
            xlq[(mu + mu) * np] +=
                (float)(delta0s * Q3(mu) * cos(is * (phirad + pi)));
        if (xlu)
            xlu[(mu + mu) * np] +=
                (float)(delta0s * U3(mu) * sin(is * (phirad + pi)));

        /* xl(-mu, 1): atmospheric path reflectance (×xmus) */
        XL(-mu, 0) += (float)(delta0s * roI * cos(is * (phirad + pi)));
        if (xlq)
            xlq[(0) * np] += (float)(delta0s * roQ * cos(is * (phirad + pi)));
        if (xlu)
            xlu[(0) * np] += (float)(delta0s * roU * sin(is * (phirad + pi)));
