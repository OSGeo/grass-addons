...
/**
 * \brief Compute atmospheric path reflectance via scalar successive orders of
 * scattering.
 *
 ...
 *
 * \param[in,out] ctx       6SV context (reads polar.betal, del.delta,
 * quad.nquad, multi.igmax; writes ctx->err on error).
 ...
 * \param[out]    xl        Radiance array [(2mu+1)*np]; \c xl[0] = R_atm ×
 * xmus.
 * \param[out]    xlphim    Path reflectance as a function of azimuth [nfi].
 * \param[out]    rolut     Angular LUT reflectance [mu × 61]; may be NULL to
 * skip.
 */
void sixs_os(SixsCtx *ctx, int iaer_prof, float tamoy, float trmoy,
             float pizmoy, float tamoyp, float trmoyp, float palt, float phirad,
             int nt, int mu, int np, int nfi, const float *rm_off,
             const float *gb_off, const float *rp, float *xl, float *xlphim,
             float *rolut)
{
...
#define GB(j) gb_off[(j) + mu]
    ...
    float *filut =
        rolut ? (float *)calloc((size_t)mu * 61, sizeof(float)) : NULL;
        ...
        for (;;) {
                ...
                for (int k = 1; k <= mu; k++) {
                    ...
                    for (int i = 0; i <= nt; i++) {
                        ...
                        for (int j = 1; j <= mu; j++) {
                            ...
                            double bpjk =
                                BP(j, k) * x + y * (beta0 + beta2 * xpj * xpk);
                            double bpjmk =
                                BP(j, -k) * x + y * (beta0 + beta2 * xpj * ypk);
            ...
            if (ig > 2) {
                ...
                if (roavion2 >= accu && roavion1 >= accu && roavion >= accu) {
                    ...
                    double y = fabs(
                        ((r - roavion1 / roavion2) / ((1.0 - r) * (1.0 - r))) *
                        (roavion0 / roavion));
                    z = fmax(y, z);
                }
                for (int l = -mu; l <= mu; l++) {
                    if (l == 0)
                        continue;
                    if (INM2(l) <= accu || INM1(l) <= accu || I3(l) <= accu)
                        continue;
                    double r = IN(l) / INM1(l);
                    double y = fabs(
                        ((r - INM1(l) / INM2(l)) / ((1.0 - r) * (1.0 - r))) *
                        (IN(l) / I3(l)));
                    z = fmax(y, z);
                }
                if (z < 0.0001) {
        ...
        if (rolut && filut && nfilut) {
            for (int m2 = 0; m2 < mu; m2++) {
                for (int l2 = 0; l2 < nfilut[m2]; l2++) {
                    ...
                    ROLUT(m2, l2) +=
                        (float)(delta0s * I3(m2 + 1) * cos(is * (phimul + pi)));
                }
            }
        }
