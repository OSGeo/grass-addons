...
class TestSpatialFilter(unittest.TestCase):
    ...
    def test_box_uniform_preserved(self):
        ...
        np.testing.assert_allclose(
            out, val, atol=1e-5, err_msg="Box filter changed a uniform field"
        )
    ...
    def test_box_center_pixel_3x3(self):
        ...
        data = np.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=np.float32)
        ...
        self.assertAlmostEqual(
            ...
            expected_center,
            places=4,
            msg=f"Center pixel {float(out[1, 1]):.4f} ≠ mean {expected_center:.4f}",
        )
    ...
    def test_box_nan_excluded_from_mean(self):
        ...
        self.assertTrue(
            np.isfinite(float(out[3, 3])), msg="NaN pixel produced non-finite output"
        )
        self.assertAlmostEqual(
            float(out[3, 3]),
            1.0,
    ...
    def test_box_nan_does_not_propagate(self):
        ...
        out = spatial_box_filter(data, filter_half=1)
        self.assertEqual(
            np.isnan(out).sum(), 0, msg="NaN propagated to neighbouring pixels"
        )
    ...
    def test_box_output_finite_for_clean_input(self):
        ...
        self.assertTrue(
            np.all(np.isfinite(out)), msg="Non-finite values in box filter output"
        )
    ...
    def test_box_reproducible(self):
        ...
        np.testing.assert_array_equal(
            out1, out2, err_msg="Box filter is not deterministic"
        )
    ...
    def test_gaussian_uniform_preserved(self):
        ...
        np.testing.assert_allclose(
            out, val, atol=1e-4, err_msg="Gaussian filter changed a uniform field"
        )
    ...
    def test_gaussian_nan_excluded(self):
        ...
        self.assertTrue(
            np.isfinite(float(out[5, 5])), msg="NaN was not filled by Gaussian filter"
        )
        self.assertEqual(
            np.isnan(out).sum(), 0, msg="NaN propagated in Gaussian filter output"
        )
    ...
    def test_gaussian_reproducible(self):
        ...
        np.testing.assert_array_equal(
            out1, out2, err_msg="Gaussian filter is not deterministic"
        )
