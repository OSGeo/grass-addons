...
def lut_slice(lut, aod_val, h2o_val):
    ...
    _lib.atcorr_lut_slice(
        ctypes.byref(lut["cfg_c"]),
        ctypes.byref(lut["arr_c"]),
...
wl = np.array(
    [0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.84, 0.87], dtype=np.float32
)
n_bands = len(wl)
...
_lib.retrieve_quality_mask.argtypes = [
    ...
    _FP,
    _FP,
    _FP,  # L_blue, L_red, L_nir, L_swir
    ctypes.c_int,
    ctypes.c_int,
    ctypes.c_float,  # npix, doy, sza_deg
...
E0_all = np.array(
    [float(_lib.sixs_E0(ctypes.c_float(float(w)))) for w in wl], dtype=np.float32
)

rho_boa = np.full((n_bands, npix), np.nan, dtype=np.float32)
for b in range(n_bands):
    rho_toa = (bands[b] * math.pi / (float(E0_all[b]) * cos_sza * d2)).astype(
        np.float32
    )
    rho_raw = invert(
        rho_toa, sl["R_atm"][b], sl["T_down"][b], sl["T_up"][b], sl["s_alb"][b]
    )
...
for b in range(n_bands):
    ...
    _lib.adjacency_correct_band(
        ...
        ctypes.c_float(1.0),
        ctypes.c_float(30.0),
        ctypes.c_float(T_scat),
        ctypes.c_float(float(sl["s_alb"][b])),
        ctypes.c_float(float(wl[b])),
        ctypes.c_float(aod_scene),
...
for b in range(n_bands):
    ...
    _lib.uncertainty_compute_band(
        ...
        _fp(boa_b),
        ctypes.c_int(npix),
        ctypes.c_float(float(E0_all[b])),
        ctypes.c_float(float(d2)),
        ctypes.c_float(cos_sza),
        ctypes.c_float(float(sl["T_down"][b])),
        ctypes.c_float(float(sl["T_up"][b])),
        ctypes.c_float(float(sl["s_alb"][b])),
        ctypes.c_float(float(sl["R_atm"][b])),
        ctypes.c_float(0.0),
        ctypes.c_float(0.04),
        ctypes.byref(lut["cfg_c"]),
        ctypes.byref(lut["arr_c"]),
...
dasf_refl = np.ascontiguousarray(
    np.tile(np.array([0.40, 0.43, 0.45, 0.44], dtype=np.float32)[:, None], (1, npix))
)  # [n_dasf, npix]
