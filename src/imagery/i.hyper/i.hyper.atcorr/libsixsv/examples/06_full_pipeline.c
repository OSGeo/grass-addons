 ...
 *       -L../testsuite -lsixsv -lm -fopenmp -Wl,-rpath,../testsuite -o
 * 06_full_pipeline
 */
...
int main(void)
{
    ...
    float aod_scene =
        retrieve_aod_ddv(L_470, L_660, L_865, L_2130, npix, doy, sza, aod_px);
    ...
    for (int b = 0; b < n_bands; b++) {
        ...
        atcorr_lut_slice(&cfg, &lut, aod_scene, wvc_mean, Rs, Tds, Tus, ss,
                         NULL);
...
#pragma omp parallel for schedule(static)
    ...
    atcorr_lut_slice(&cfg, &lut, aod_scene, wvc_mean, Rs_avg, Tds_avg, Tus_avg,
                     ss_avg, NULL);

    for (int b = 0; b < n_bands; b++) {
        float T_scat = 1.0f - (Tds_avg[b] - 0.7f * Tds_avg[b]); /* approx */
        adjacency_correct_band(rho_boa + b * npix, nrows, ncols, 1.0f, 30.0f,
                               T_scat, ss_avg[b], wl[b], aod_scene, pressure,
                               sza, vza);
    }
    ...
    for (int b = 0; b < n_bands; b++) {
        ...
        atcorr_lut_slice(&cfg, &lut, aod_scene, wvc_mean, Rs2, Tds2, Tus2, ss2,
                         Tdds);
        float T_eff =
            atcorr_terrain_T_down(Tds2[b], Tdds[b], cos_sza, cos_i, V_d);
        float T_up_eff =
            atcorr_terrain_T_up(Tus2[b], cos_vza, vza + slope * 0.3f);
    ...
    for (int b = 0; b < n_bands; b++) {
        ...
        uncertainty_compute_band(
            bands[b], rho_boa + b * npix, npix, sixs_E0(wl[b]), (float)d2,
            cos_sza, Tds2[b], Tus2[b], ss2[b], Rs2[b], 0.0f, 0.04f, &cfg, &lut,
            wl[b], aod_scene, wvc_mean, sigma + b * npix);
    }
