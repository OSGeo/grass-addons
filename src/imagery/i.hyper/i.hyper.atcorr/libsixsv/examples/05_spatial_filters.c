...
int main(void)
{
    ...
    for (int i = 0; i < npix; i++) {
        if (aod_map[i] != aod_map[i])
            n_nan_before++;
        if (aod_gauss[i] != aod_gauss[i])
            n_nan_after++;
    }
    ...
    float T_down_eff =
        atcorr_terrain_T_down(T_down, T_down_dir, cos_sza, cos_i, V_d);
    ...
    float T_up_eff =
        atcorr_terrain_T_up(T_up, cos_vza, /*vza_pixel_deg=*/15.0f);
    ...
    uncertainty_compute_band(
        /*rad_band=*/NULL, r_boa, npix, E0, d2, cosf(35.0f * M_PI / 180.0f),
        /*T_down=*/0.85f, /*T_up=*/0.90f, /*s_alb=*/0.08f, /*R_atm=*/0.05f,
        /*nedl=*/0.0f, /* estimate from rad_band — NULL → use 0.01 fallback */
        /*aod_sigma=*/0.04f, &cfg_u, &lut_u, wl_um, aod_val, h2o_val,
        sigma_out);

    float sig_mean = 0.0f;
    for (int i = 0; i < npix; i++)
        sig_mean += sigma_out[i];
