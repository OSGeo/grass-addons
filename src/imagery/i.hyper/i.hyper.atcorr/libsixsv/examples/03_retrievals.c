...
int main(void)
{
    ...
    sixs_possol(/*month=*/7, /*day=*/4, /*UTC=*/10.5f,
                /*lon=*/-122.0f, /*lat=*/37.5f, &sol_zen, &sol_az,
                /*year=*/2024);
    ...
    float aod_scene =
        retrieve_aod_ddv(L_470, L_660, L_860, L_2130, npix, doy, sza, aod_px);
    ...
    for (int i = 0; i < npix; i++) {
        if (mask[i] & RETRIEVE_MASK_CLOUD)
            n_cloud++;
        if (mask[i] & RETRIEVE_MASK_WATER)
            n_water++;
    }
    ...
    oe_invert_aod_h2o(&cfg_oe, &lut_oe, rho_vis, npix, n_vis, wl_vis, L_865,
                      L_940, L_1040, sza, vza,
                      /*aod_prior=*/aod_scene,
                      /*h2o_prior=*/wvc_mean / npix,
                      /*sigma_aod=*/0.5f, /*sigma_h2o=*/1.0f,
                      /*sigma_spec=*/0.01f, /*fwhm_940_um=*/0.0f, out_aod,
                      out_h2o);
