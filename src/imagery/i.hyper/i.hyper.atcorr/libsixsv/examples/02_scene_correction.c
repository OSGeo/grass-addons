...
int main(void)
{
    ...
    for (int b = 0; b < n_bands; b++)
        for (int i = 0; i < npix; i++)
            rho_toa[b * npix + i] =
                0.05f + 0.25f * b / n_bands + 0.02f * sinf(i * 0.1f);
...
#pragma omp parallel for schedule(static)
        ...
        for (int i = 0; i < npix; i++) {
            float Ra, Td, Tu, sa;
            atcorr_lut_interp_pixel(&cfg, &lut, aod_map[i], h2o_map[i], wl[b],
                                    &Ra, &Td, &Tu, &sa);
            rho_boa[b * npix + i] =
                atcorr_invert(rho_toa[b * npix + i], Ra, Td, Tu, sa);
        }
        float mean_boa = 0.0f;
        for (int i = 0; i < npix; i++)
            mean_boa += rho_boa[b * npix + i];
