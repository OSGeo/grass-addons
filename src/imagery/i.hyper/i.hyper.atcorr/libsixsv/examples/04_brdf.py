...
rho_rpv = _lib.sixs_brdf_eval(
    ctypes.c_int(1),
    rpv_params,
    ctypes.c_float(cos_sza),
    ctypes.c_float(cos_vza),
    ctypes.c_float(raa_deg),
)
albe_rpv = _lib.sixs_brdf_albe(
    ctypes.c_int(1),
    rpv_params,
    ctypes.c_float(cos_sza),
    ctypes.c_int(48),
...
rl_params = (ctypes.c_float * 5)(0.0774, 0.0372, 0.0079, 0.0, 0.0)

rho_rl = _lib.sixs_brdf_eval(
    ctypes.c_int(9),
    rl_params,
    ctypes.c_float(cos_sza),
    ctypes.c_float(cos_vza),
...
hapke_params = (ctypes.c_float * 5)(0.85, 0.10, 0.06, 0.20, 0.0)
rho_hapke = _lib.sixs_brdf_eval(
    ctypes.c_int(3),
    hapke_params,
    ctypes.c_float(cos_sza),
    ctypes.c_float(cos_vza),
...
_lib.mcd43_disaggregate.argtypes = [
    _FP,
    _FP,
    _FP,  # fiso_7, fvol_7, fgeo_7
    _FP,  # wl_target [n_wl]
    ctypes.c_int,  # n_wl
    ctypes.c_float,  # alpha (Tikhonov regularisation)
    _FP,
    _FP,
    _FP,  # fiso_out, fvol_out, fgeo_out [n_wl]
]
_lib.mcd43_disaggregate.restype = None

fiso_7 = np.array(
    [0.0774, 0.1306, 0.0688, 0.2972, 0.1218, 0.1030, 0.0747], dtype=np.float32
)
fvol_7 = np.array(
    [0.0372, 0.0580, 0.0261, 0.1177, 0.0412, 0.0324, 0.0203], dtype=np.float32
)
fgeo_7 = np.array(
    [0.0079, 0.0178, 0.0066, 0.0598, 0.0094, 0.0068, 0.0040], dtype=np.float32
)
...
spectrum = np.array([0.05, 0.07, 0.09, 0.11, 0.12, 0.20, 0.22, 0.23], dtype=np.float32)
