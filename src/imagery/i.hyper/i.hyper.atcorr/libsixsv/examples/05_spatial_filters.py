...
_lib.spatial_box_filter.argtypes = [_FP, _FP, ctypes.c_int, ctypes.c_int, ctypes.c_int]
...
cos_i = _lib.cos_incidence(
    ctypes.c_float(sza),
    ctypes.c_float(saa),
    ctypes.c_float(slope),
    ctypes.c_float(aspect),
)
...
T_down_eff = _lib.atcorr_terrain_T_down(
    ctypes.c_float(T_down),
    ctypes.c_float(T_down_dir),
    ctypes.c_float(cos_sza),
    ctypes.c_float(cos_i),
    ctypes.c_float(V_d),
)
...
T_up_eff = _lib.atcorr_terrain_T_up(
    ctypes.c_float(T_up),
    ctypes.c_float(math.cos(math.radians(8.0))),
    ctypes.c_float(15.0),
)
...
_lib.uncertainty_compute_band.argtypes = [
    _FP,
    _FP,  # rad_band (or NULL), refl_band
    ctypes.c_int,  # npix
    ctypes.c_float,
    ctypes.c_float,  # E0, d2
    ctypes.c_float,  # cos_sza
    ctypes.c_float,
    ctypes.c_float,  # T_down, T_up
    ctypes.c_float,
    ctypes.c_float,  # s_alb, R_atm
    ctypes.c_float,
    ctypes.c_float,  # nedl, aod_sigma
    ctypes.POINTER(_Cfg),
    ctypes.POINTER(_Arr),
    ctypes.c_float,
    ctypes.c_float,
    ctypes.c_float,  # wl_um, aod_val, h2o_val
    _FP,  # sigma_out
]
