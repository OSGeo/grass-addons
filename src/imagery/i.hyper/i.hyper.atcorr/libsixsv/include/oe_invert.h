 ...
 *
 * \param[in]  cfg          LUT grid specification (wavelengths, AOD/H₂O grid,
 * geometry).
 ...
 *                          Pixel \c i at band \c b: \c rho_toa_vis[i*n_vis +
 * b].
 * \param[in]  npix         Number of pixels.
 * \param[in]  n_vis        Number of VIS diagnostic bands (e.g. 4 for
 * 470/550/660/870 nm).
 * \param[in]  vis_wl       Central wavelengths of the VIS bands [µm], length \p
 * n_vis.
 * \param[in]  L_865        TOA radiance near 865 nm [npix]; NULL → disable
 * J_H₂O.
 * \param[in]  L_940        TOA radiance near 940 nm [npix]; NULL → disable
 * J_H₂O.
 * \param[in]  L_1040       TOA radiance near 1040 nm [npix]; NULL → disable
 * J_H₂O.
 ...
 * \param[in]  h2o_prior    Scene-mean H₂O prior [g/cm²] (e.g. from
 * retrieve_h2o_940()).
