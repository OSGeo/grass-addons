 ...
 *
 * Propagates two uncertainty sources through the algebraic Lambertian
 * inversion:
 *
...
/**
 * \brief Estimate the noise-equivalent delta radiance (NEDL) from a radiance
 * array.
 *
 ...
 * \param[in]  d2        Squared Earth–Sun distance [AU²] from
 * sixs_earth_sun_dist2().
 * \param[in]  cos_sza   cos(solar zenith angle).
 * \param[in]  T_down    Scene-average downward transmittance at this
 * wavelength.
 ...
 * \param[out] sigma_out Pre-allocated float[npix]; total reflectance σ per
 * pixel.
 */
