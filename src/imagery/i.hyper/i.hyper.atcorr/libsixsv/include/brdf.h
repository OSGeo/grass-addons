/**
 * \file brdf.h
 * \brief BRDF surface reflectance models — C port of 6SV2.1 BRDF Fortran
 * subroutines.
 *
...
typedef enum {
    BRDF_LAMBERTIAN =
        0, /*!< Lambertian (constant reflectance; analytically handled) */
...
typedef union {
    struct {
        float rho0;
    } lambertian; /*!< Constant surface reflectance ρ₀ */
    struct {
        float rho0, af, k;
    } rahman; /*!< RPV: ρ₀ intensity, af asymmetry, k structure */
    struct {
        float k0, k1, k2;
    } roujean; /*!< k₀ isotropic, k₁ geometric, k₂ volumetric */
    struct {
        float om, af, s0, h;
    } hapke; /*!< ω single-scatter albedo, af asymmetry, s₀ hotspot amp, h width
              */
    struct {
        float wspd, azw, sal, pcl, wl;
    } ocean; /*!< Wind speed [m/s], azimuth offset [deg], salinity [ppt],
                chlorophyll [mg/m³], wavelength [µm] */
    struct {
        float a, ap, b, c;
    } walthall; /*!< Polynomial coefficients a, a', b, c */
    struct {
        float k, b;
    } minnaert; /*!< k Minnaert exponent, b albedo normalisation */
    struct {
        float f_iso, f_vol, f_geo;
    } rosslimaignan; /*!< Ross-Li kernel weights */
} BrdfParams;
