...
def run_langsam_segmentation(
    ...
    Returns:
    numpy.ndarray: A 2D array where each segmented object has a unique value.
    """
    ...
    # Masks have shape (N, rows, cols) and come back as a CPU tensor from
    # SAM 1 models and as a numpy array from SAM 2 models.
    return merge_masks(np.asarray(masks))
...
def merge_masks(masks):
    """
    Merges per-object masks into a single array with a unique value per object.

    Parameters:
    masks (numpy.ndarray): Array of shape (N, rows, cols) with one mask per object.

    Returns:
    numpy.ndarray: A 2D int32 array where cells of object i have value i + 1.
    """
    if len(masks) == 0:
        gs.fatal(_("No masks found."))

    merged_raster = np.zeros(masks[0].shape, dtype=np.int32)
    for idx, mask in enumerate(masks):
        merged_raster[mask != 0] = idx + 1
    return merged_raster


def write_raster(objects, output_raster_masks):
    """
    Writes a 2D array of segmented objects as a GRASS raster map.

    Parameters:
    objects (numpy.ndarray): A 2D array where each object has a unique value.
    output_raster_masks (str): The name of the output raster map to be created in GRASS.
    """
    gs.message(_("Importing the segmented raster into GRASS..."))

    mask_raster = garray.array(dtype=np.int32)
    mask_raster[...] = objects
...
def main():
    ...
    checkpoint_dir = options["checkpoint_dir"] or None
