                  ...
                  long unsigned int *ent_counter, int level)
{
    ...
    *ent_counter += 1;
    ...
    case DWG_TYPE_LINE:
        ...
        Vect_append_point(Points, line->start.x, line->start.y, line->start.z);
        Vect_append_point(Points, line->end.x, line->end.y, line->end.z);
    ...
    case DWG_TYPE_CIRCLE:
        ...
        if (circle_flag->answer) {
            Vect_append_point(Points, circle->center.x, circle->center.y,
                              circle->center.z);
            write_line(obj, GV_POINT, level);
        }
        else {
    ...
    case DWG_TYPE_INSERT: /* insert */
        ...
        G_debug(3, " x,y,z: %f, %f, %f", insert->ins_pt.x, insert->ins_pt.y,
                insert->ins_pt.z);
