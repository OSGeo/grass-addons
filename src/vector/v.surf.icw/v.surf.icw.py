...
def main():
    ...
    # do the maps exist?
    if not gs.find_file(pts_input, element="vector")["file"]:
        gs.fatal(_("Vector map <%s> not found") % pts_input)
    if post_mask:
        if gs.find_file("MASK")["file"]:
            gs.fatal(
                _("A MASK already exists; remove it before using the post_mask option.")
            )
        if not gs.find_file(post_mask)["file"]:
            gs.fatal(_("Raster map <%s> not found") % post_mask)

    gs.verbose(_("v.surf.icw -- Inverse Cost Weighted Interpolation"))
    gs.verbose(
    ...
    if flags["r"]:
        gs.verbose(_("Using (d^n)*log(d) radial basis function."))
    ...
    try:
        coltype = gs.vector_columns(pts_input, layer)[column]
    except KeyError:
        gs.fatal(
    ...
    if coltype["type"] not in ("INTEGER", "DOUBLE PRECISION"):
        gs.fatal(_("Data column must be numberic"))
    ...
    gs.message(_("Generating cost maps ..."))
    ...
    for i in range(n):
        ...
        try:
            ...
            gs.fatal("Data value [%s] is non-numeric" % data_value)

        cost_site_name = tmp_base + "cost_site." + "%05d" % num
        proc[num - 1] = gs.start_command(
            "r.cost",
            flags="k",
            input=area_mask,
    ...
    for i in range(n):
        if proc[i].wait() != 0:
            gs.fatal(_("Problem running %s") % "r.mapcalc")

    gs.message(_("Applying radial decay ..."))
    ...
    for i in range(n):
        ...
        gs.debug("r.mapcalc expression is: [%s]" % expr)
    ...
    if post_mask:
        gs.message(_("Setting post_mask <%s>") % post_mask)
        gs.mapcalc("MASK = $maskmap", maskmap=post_mask, overwrite=True)
    ...
    if where:
        gs.run_command("r.support", map=output, history="  SQL query= WHERE " + where)
    ...
    #######################################################
    # Step 6) done!
    gs.message(_("Done! Results written to <%s>.") % output)
