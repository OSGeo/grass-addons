...
int main(int argc, char *argv[])
{
    ...
    module->description =
	_("Create greedy colors for vector areas.");
	...
	G_fatal_error(_("Vector map <%s> is not in current mapset"), input_opt->answer);

     /* topology required */
    if (Vect_open_old2(&Map, input_opt->answer, mapset, field_opt->answer) < 2) {
    ...
    if (mincat == -1) {
	G_fatal_error(_("Areas in vector map <%s> don't have categories in layer %d"),
	              input_opt->answer, fieldnr);
    }

    if (mincat == maxcat) {
	G_fatal_error(_("All areas in vector map <%s> have the same category in layer %d"),
	              input_opt->answer, fieldnr);
    }
	...
	for (j = 0; j < list->n_values; j++) {
		...
		if (prb_insert(lexbfs, anreinsert))
		    G_fatal_error("Failed to re-insert neighbor %d into queue", neighbor);
	    }
	}
	/* assign greedy color */
	...
	/* create column */
	sprintf(buf, "ALTER TABLE '%s' ADD COLUMN '%s' integer", Fi->table, outcol);
	...
	sprintf(buf, "update %s set %s = %d where %s = %d",
	        Fi->table, outcol, areaclr[cat - mincat], Fi->key, cat);
	db_set_string(&stmt, buf);
