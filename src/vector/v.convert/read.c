...
/* read old 3.0 or 4.0 dig file into array
   ...
   or -1 on error */
int read_dig(FILE *Digin, struct Map_info *Mapout, struct Line **plines,
             int endian, int att)
{
    char buf[100];
    ...
    if (buf[0] != '%' || buf[1] != '%') { /* Version3.0 */
        ...
        if (buf[6] == 1 &&
            (~buf[6] & 0xff) == (buf[7] & 0xff)) { /* portable ? */
    ...
    while (1) {
        ...
        if ((type & GV_BOUNDARY) || !att) {
            ...
            if (line >= lalloc) {
                lalloc += 10000;
                lines = (struct Line *)G_realloc(lines,
...
int read_att(FILE *Attin, struct Categ **pcats)
{
    ...
    while (fgets(buf, 200, Attin) != NULL) {
        ...
        if (cat >= ctalloc) {
            ctalloc += 10000;
            cats =
                (struct Categ *)G_realloc(cats, ctalloc * sizeof(struct Categ));
        }
        cats[cat].type = type;
        cats[cat].x = x;
        cats[cat].y = y;
