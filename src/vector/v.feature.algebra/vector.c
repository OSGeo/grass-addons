...
static VECFUNC vf[] = {{"v_copy", v_copy, "p=rp"},
                       {"v_add", v_add, "p=rpp"},
...
SYMBOL *pntfunc(SYMBOL *func, SYMBOL *arglist)
{
    ...
    if (argc == 0 && (!func->proto || !*func->proto))
        ...
        res = (*(p_func_ppd)func->v.p)(res, arglist->v.p, arglist->next->v.d);
        ...
        res = (*(p_func_ppp)func->v.p)(res, arglist->v.p, arglist->next->v.p);
    else if (argc == 3 && !strcmp(func->proto, "ppp"))
        res = (*(p_func_ppp)func->v.p)(arglist->v.p, arglist->next->v.p,
