        ...
        if model is "":
            ...
            if not isinstance(kappa, float):
                ...
                VariogramModel = robjects.r.autofitVariogram(
                    formula, inputdata, **DottedParams
                )
            else:
                VariogramModel = robjects.r.autofitVariogram(
            ...
            VariogramModel = robjects.r["fit.variogram"](
                DataVariogram,
                model=robjects.r.vgm(
                    psill=psill, model=model, nugget=nugget, range=range, kappa=kappa
                ),
            )
...
def main(argv=None):
    ...
    if argv is None:
        ...
        app = wx.App()
        KrigingFrame = GUI.KrigingModule(
            parent=None, Rinstance=robjects, controller=controller
        )
        KrigingFrame.Centre()
        KrigingFrame.Show()
        app.MainLoop()
