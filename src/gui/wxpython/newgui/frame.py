...
class MyFrame(MySingleMapFrame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        MySingleMapFrame.__init__(
            self, parent=parent, title=title, size=size, name=name, **kwargs
        )

        self.cmd = ["d.rast", "map=aspect@PERMANENT"]
        self.rlayer = MapLayer(
            ltype="raster", cmd=self.cmd, Map=self.GetMap(), name="elevation"
        )
