...
href="https://grass.osgeo.org/grass-stable/manuals/r.reclass.area.html"
target="_blank">r.reclass.area</a> for more details.

<p>
<img src="r_suitability_regions_01.png"><i>
...
the value 2. See <a
href="https://grass.osgeo.org/grass-stable/manuals/r.neighbors.html"
...
species has no problem to briefly stay in non-suitable habitat, e.g.,
to cross it on their way to more suitable habitat. As the example below
shows, it results in larger regions than in the previous option.

<p>
<img src="r_suitability_regions_02.png"><i> <br>Figure 2: Like figure 1, but based on
the median suitability scores of the neighboring cells within a radius
of 300 meter (3x3 moving window).</i></p>
...
even result in a net loss of habitat if one or more of the fragments
are too small to maintain a population (the user can set a minimum area
size to account for this).

<p>
<img src="r_suitability_regions_03.png"><i> <br>Figure 3: Like figure 2, but
considering raster cells with suitability 0 (mostly roads) as absolute
barriers. Diagonally connected raster cells are not considered to form
a contiguous region.</i></p>

<p>
Note that for line elements like roads, results may differ if the
option to 'include the diagonal neighbors when defining clumps' (flag
d) is selected. For example, in figure 4, diagonally connected cells
are considered as neighbors. As a consequence, the suitable areas on
both sides of the road are considered to be part of the same region.
I.e., the road does not act as a barrier here.

<p>
<img src="r_suitability_regions_04.png"><i> <br>Figure 4: Like figure 3, but this time,
...
desirable for visualisation purposes, or it may in fact be acceptable
to include such areas in the final selection of a region.

<p>
<img src="r_suitability_regions_05.png"><i> <br>Figure 5: Like figure 3 (left), but
here gaps (areas within a suitable region) of 500 hectares or less were
...
'filled patches'. This makes it easier to e.g., inspect the feasibility
or desirability to actually include these areas in a protected area.

<h3>Compactness of the regions</h3>

To compare the compactness of the resulting regions, the compactness of
an area is calculated using the formula below (see also <a
href="https://grass.osgeo.org/grass-stable/manuals/v.to.db.html">v.to.db</a>.

<p>
<pre><code>compactness = perimeter / (2 * sqrt(PI * area))</code></pre>
...
<p>
<pre><code>fd = 2 * (log(perimeter) / log(area))</code></pre>
...
See <a
href="https://ecodiv.earth/TutorialsNotes/SuitabilityRegions/index.html">this
tutorial</a> for examples.

<h2>See also</h2>

<ul>
<li><a href="https://grass.osgeo.org/grass-stable/manuals/r.reclass.area.html">r.reclass.area</a></li>
<li><a href="https://grass.osgeo.org/grass-stable/manuals/r.clump.html">r.clump</a></li>
<li><a href="https://grass.osgeo.org/grass-stable/manuals/v.to.db.html">v.to.db</a></li>
<li><a href="r.area.html">r.area</a></li>
</ul>

<h2>REQUIREMENTS</h2>

If you use the option to compute the area per clump (using the
<b>-a</b> flag), you need to install the <em>r.area</em> module first.

<h2>AUTHOR</h2>

Paulo van Breugel, paulo at ecodiv.earth<br> <a
href="https://has.nl">HAS green academy</a><br> <a
