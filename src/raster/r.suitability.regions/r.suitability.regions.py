...
def tmpmask(input, absmin):
    ...
    gs.write_command(
        "r.recode", input=input, output=tmprecode, rule="-", stdin=rules, quiet=True
    )
    return tmprecode
...
def main(options, flags):
    ...
    if (radius > 1) & (len(absminsuit) == 0):
        ...
        gs.run_command(
            "r.series", input=[suitrast, tmp02a], method="maximum", output=tmp02
        )
    elif radius > 1:
        gs.message("Computing neighborhood statistic")
        try:
        ...
        gs.run_command(
            "r.series", input=[suitrast, tmp01], method="maximum", output=tmp02a
        )
        gs.run_command(
            "r.mapcalc",
            quiet=True,
            expression=(
                "{0} = if({1} > {2},{3},null())".format(
                    tmp02, suitrast, absminsuit, tmp02a
                )
            ),
        )
    else:
    ...
    gs.run_command(
        "r.mapcalc",
        quiet=True,
        expression=("{} = if({} >= {},1,null())".format(tmp03, tmp02, minsuit)),
    )
    ...
    if maxgap > 0:
        ...
        expr3 = "{} = int(if(isnull({}),1,null()))".format(tmp07a, tmp06)
        ...
        if len(absminsuit) > 0:
            ...
            gs.run_command(
                "r.mapcalc",
                expression=(
                    "{} = if(isnull({}), {},null())".format(tmp07, bumask, tmp07a)
                ),
            )
        else:
        ...
        gs.run_command(
            "r.series", quiet=True, output=xtralayer, input=[tmp04, tmp07], method="sum"
        )
        gs.write_command(
            "r.category",
            map=xtralayer,
        ...
        gs.run_command("r.clump", flags=d, input=tmp07, output=suitreg, quiet=True)
        gs.run_command(
            "r.support",
            map=xtralayer,
        ...
        gs.write_command(
            "r.colors", rules="-", map=xtralayer, stdin=COLORRULES2, quiet=True
        )

    else:
        # Assign unique ids to clumps
        gs.message("Assigning unique id's to clumps")
        gs.run_command("r.clump", flags=d, input=tmp04, output=suitreg, quiet=True)
    gs.run_command(
        "r.support",
        map=suitreg,
    ...
    if flag_a:
        ...
        gs.run_command(
            "r.mapcalc",
            quiet=True,
            expression=("{} = {} * area()/10000".format(areastat, tmp08)),
        )
    ...
    if flag_k:
        ...
        gs.write_command(
            "r.colors", rules="-", map=rname, stdin=COLORRULES1, quiet=True
        )
