        ...
        /*    for (cnt1 = 0; cnt1 < my; cnt1++) {
          ...
          //  G_message(_("if"));
                    if (cnt2 == 0)
                    {
                        ...
                        {
           // G_message(_("ifif"));
                           lw_start[cnt1][cnt2] = (zz[cnt1][cnt2] != UNDEF ?
         mgen : 0);
                        }
                        else
                        {
          //  G_message(_("ifelse"));
                            lw_start[cnt1][cnt2] = lw_start[cnt1 - 1][mx - 1] +
         (zz[cnt1][cnt2] != UNDEF ? mgen : 0);
                        }
                    }
                    else
                    {
         //   G_message(_("ifelse"));
                        lw_start[cnt1][cnt2] = lw_start[cnt1][cnt2 - 1] +
         (zz[cnt1][cnt2] != UNDEF ? mgen : 0);
                    }
...
//---------------------------------------------------------------------------------------------------------------
        ...
        // recalculate: lw
        //       #pragma omp parallel
        //       firstprivate(gen,mgen,wei,x,y,gen2,mgen2,wei2,mgen3,wei3,nmult,weifac,iw,walkwe)
        ...
        for (k = 0; k < my; k++) {
            for (l = 0; l < mx; l++) { /* run thru the whole area */
                if (zz[k][l] != UNDEF) {
                    ...
                    // if (lw != lw_start[k][l])
                    //
                    //   G_message(_("lw %d lws %d k %d l
                    //   %d"),lw,lw_start[k][l],k,l);
                    //}
                    for (iw = 1; iw <= mgen + 1; iw++) { /* assign walkers */

                        if (lw >=
                            MAXW) /* max valid value is MAXW - 1, not MAXW */
                            G_fatal_error(_("nwalk (%d) > maxw (%d)!"), lw,
                                          MAXW);
...
#pragma omp parallel firstprivate(l, lw, k, decr, d1, hhc, velx, vely, eff, \
                                      gaux, gauy) // nwalka
            {
                ...
                /*#pragma omp critical
                        {
                        G_message("tid: %d min_loop: %d max_loop: %d nwalk: %d,
                   steps: %d", tid, min_loop, max_loop, nwalk, steps);
                        }*/
                ...
                for (lw = min_loop; lw < max_loop; lw++) {
                    ...
                    if (w[lw][2] > EPS) { /* check the walker weight */
                        ...
                        if (zz[k][l] != UNDEF) {
                            ...
                            //{
                            gama[k][l] +=
                                (addac * w[lw][2]); /* add walker weigh to water
                                                       depth or conc. */
                            d1 = gama[k][l] * conn;
                            //
                            //}
                            ...
                            if (w[lw][0] <= xmin || w[lw][1] <= ymin ||
                                w[lw][0] >= xmax || w[lw][1] >= ymax) {
                                w[lw][2] = 1e-10; /* eliminate walker if it is
                                                     out of area */
                            }
                            else {
                                if (wdepth != NULL) {
                                    l = (int)((w[lw][0] + stxm) / stepx) - mx -
                                        1;
                                    k = (int)((w[lw][1] + stym) / stepy) - my -
                                        1;
                                    w[lw][2] *= sigma[k][l];
                                }
