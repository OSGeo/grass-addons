        ...
        while (done) {
            ...
            for (i = 1; i < 9; ++i) {
                ...
                if (streams[next_r][next_c] == stat_streams[s].order &&
                    ...
                    cur_length = G_distance(next_easting, next_northing,
                                            cur_easting, cur_northing);
...
int ram_calculate_basins(CELL **dirs)
{
    ...
    for (i = 0; i < outlets_num; ++i) {
        stat_streams[i].basin_area = ram_calculate_basins_area(
            dirs, stat_streams[i].r, stat_streams[i].c);
...
int seg_calculate_basins(SEGMENT *dirs)
{
    ...
    for (i = 0; i < outlets_num; ++i) {
        stat_streams[i].basin_area = seg_calculate_basins_area(
            dirs, stat_streams[i].r, stat_streams[i].c);
        if (stat_streams[i].is_outlet)
            total_basins += stat_streams[i].basin_area;
    }
