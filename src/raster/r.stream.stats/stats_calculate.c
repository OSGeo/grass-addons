...
int stats(int order_max)
{
    ...
    for (i = 0; i < outlets_num; ++i) { /* recalculate and unify */
        ...
        tmp_num = ((stat_streams[i].cell_num - 1) < 1)
                      ? 1
        ...
        ord_stats[stat_streams[i].order].sum_area += stat_streams[i].basin_area;
    ...
    for (i = 1; i <= order_max; ++i) {
        ...
        snum = (ord_stats[i].stream_num > 1)
                   ? ((float)ord_stats[i].stream_num) /
                         (ord_stats[i].stream_num - 1)
                   : 0.;
        ...
        ord_stats[i].std_length =
            sqrt((ord_stats[i].std_length / num -
                  (ord_stats[i].avg_length * ord_stats[i].avg_length)) *
                 snum);

        ord_stats[i].std_slope =
            sqrt((ord_stats[i].std_slope / num -
                  (ord_stats[i].avg_slope * ord_stats[i].avg_slope)) *
                 snum);

        ord_stats[i].std_gradient =
            sqrt((ord_stats[i].std_gradient / num -
                  (ord_stats[i].avg_gradient * ord_stats[i].avg_gradient)) *
                 snum);

        ord_stats[i].std_area =
            sqrt((ord_stats[i].std_area / num -
                  (ord_stats[i].avg_area * ord_stats[i].avg_area)) *
                 snum);

        ord_stats[i].std_elev_diff =
            sqrt((ord_stats[i].std_elev_diff / num -
                  (ord_stats[i].avg_elev_diff * ord_stats[i].avg_elev_diff)) *
                 snum);
    ...
    stats_total.std_area_ratio =
        sqrt((stats_total.std_area_ratio / ord_num -
              (stats_total.area_ratio * stats_total.area_ratio)) *
             ord_snum);
