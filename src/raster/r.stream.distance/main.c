 ...
 *               according to user's input data. The elevation and distance are
 *               calculated along watercourses. It uses any stream map and
 *               r.watershed or r.stream.extract direction map. Stream network
...
int main(int argc, char *argv[])
{
    ...
    struct Option *in_dir_opt, *in_stm_opt, *in_elev_opt, *in_method_opt,
        *opt_swapsize, *out_dist_opt, *out_diff_opt;
    ...
    module->label =
        _("Calculates distance to and elevation above streams and outlet.");
    module->description =
    ...
    in_stm_opt->description =
        _("Name of input raster map with stream network "
    ...
    out_diff_opt->description =
        _("Name for output elevation difference raster map");
    ...
    flag_outs->key = 'o';
    flag_outs->description = _("Calculate parameters for outlets (outlet mode) "
                               "instead of (default) streams");
    ...
    if (method == UPSTREAM && in_elev_opt->answer) {
        seg_size = (sizeof(CELL) * 2.0 + sizeof(DCELL) * 2.0) * SROWS * SCOLS /
                   (1 << 20);
    }
    else {
        seg_size = (sizeof(CELL) * 2.0 + sizeof(DCELL) * 1.0) * SROWS * SCOLS /
                   (1 << 20);
    }
    ...
    if (!segmentation) {
        ...
        if (method == DOWNSTREAM) {
            ...
            for (j = 0; j < outlets_num; ++j) {
                ...
                ram_calculate_downstream(dirs, distance, elevation, outlets[j],
                                         outs);
            }
            G_percent(j, outlets_num, 1);
        }
        ...
        seg_create_map(&map_distance, SROWS, SCOLS, number_of_segs, DCELL_TYPE);
        ...
        if (method == DOWNSTREAM) {
            ...
            for (j = 0; j < outlets_num; ++j) {
                G_percent(j, outlets_num, 1);
                seg_calculate_downstream(dirs, distance, elevation, outlets[j],
                                         outs);
            }
            G_percent(j, outlets_num, 1);
        }
