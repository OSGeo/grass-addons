...
def main():
    ...
    net_hist_str = (
        gs.read_command("v.info", map=network_map, flags="h")
        .split("\n")[0]
        .split(": ")[1]
    )
    ...
    try:
        ...
        gs.fatal("No edges selected to compute corridors for...")
    ...
    patchmaps = (
        gs.read_command("g.list", pattern=pattern, type="raster")
        .rstrip("\n")
        .split("\n")
    )
    ...
    for patch in selected_patches:
        ...
        if patchmap not in patchmaps:
            gs.fatal("Cannot find raster map {}.".format(patchmap))
    ...
    corridormaps[corridor_base] = (
        gs.read_command("g.list", flags="e", pattern=pattern, type="raster")
        .rstrip("\n")
        .split("\n")
    )
    ...
    gs.verbose("Aggregating corridor maps...")
