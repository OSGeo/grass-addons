    ...
    /*  if using the whole raster map as the sampling
       ...
       the output files */
        ...
        if (choice->att[0]) {
            ...
            fprintf(fp0, "                 MEAN        ST. DEV.    MINIMUM     "
                         "MAXIMUM\n");
            fprintf(fp0, "Scale Unit       PIXEL ATT.  PIXEL ATT.  PIXEL ATT.  "
                         "PIXEL ATT.\n");
            fclose(fp0);
        }
        ...
        if (choice->div[0]) {
            ...
            fprintf(fp1, "                                                     "
                         "INVERSE\n");
            fprintf(fp1, "Scale Unit       RICHNESS    SHANNON     DOMINANCE   "
                         "SIMPSON\n");
            fclose(fp1);
        }
        ...
        if (choice->te2[0]) {
            ...
            fprintf(fp2, "Scale Unit       CONTAGION   SEC. MOM.   DIFF. MOM.  "
                         "ENTROPY     CONTRAST\n");
            fclose(fp2);
        }
...
void mv_driver()
{
    ...
    fprintf(stdout, "   warning may be printed or appear in a window; ignore "
                    "this warning.\n");
        ...
        for (i = 0; i < nrows; i++) {
            ...
            for (j = 0; j < ncols; j++) {
                ...
                case (CELL_TYPE):
                    ...
                    break;
                case (FCELL_TYPE):
                    if ((*(ftmp + j) || *(ftmp + j) == 0.0) &&
                        *(nul_buf + j) == 0.0)
                        get_rich_whole((double)*(ftmp + j), richwhole,
                                       &cntwhole);
                    break;
                case (DCELL_TYPE):
                    if ((*(dtmp + j) || *(dtmp + j) == 0.0) &&
                        *(nul_buf + j) == 0.0)
                        get_rich_whole(*(dtmp + j), richwhole, &cntwhole);
                    break;
                }
