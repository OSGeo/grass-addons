...
void mv_texture(int nrows, int ncols, double **buf, double **null_buf,
                double **value, int index, double *rich, int cnt, int cntwhole)
{
    int lc;
    register int i, j;
...
void cal_divers(double **buf, double **null_buf, int i0, int j0, int nr, int nc,
                int lc, int cnt, double *diver)
{
    int tot;
    static int *density;
    register int i;
        ...
        for (i = 0; i < cnt; i++)
            for (j = 0; j < cnt; j++)
                if (p = GLCM[i][j] / (double)(r)) {
                    tex[3] += p * log(p);
                    tex[1] += p * p; /* ASM */
                    tex[2] += p / (1 + (rich[i] - rich[j]) *
                                           (rich[i] - rich[j])); /* IDM */
                    tex[4] += p * (rich[i] - rich[j]) *
                              (rich[i] - rich[j]); /* Contrast */
                }
...
void cal_edge(double **buf, double **null_buf, int i0, int j0, int nr, int nc,
              int lc, double *edge, int cntwhole, double *atts, double **weight,
              double *edgeatts, double **edgemat, int *edge1, int *edge2)
{
