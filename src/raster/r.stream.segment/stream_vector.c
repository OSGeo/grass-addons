...
int create_sector_vector(char *out_vector, int number_of_streams, int radians)
{
    ...
    for (i = 1; i < number_of_streams; ++i) {
        ...
        for (j = 0; j < SA[i].number_of_sectors; ++j) {
            ...
            sprintf(buf, "insert into %s values( %d, %d, %d, %d, ", Fi->table,
                    sector_category, segment, sector, order);
...
int create_segment_vector(char *out_vector, int number_of_streams, int radians)
{
    ...
    for (i = 1; i < number_of_streams; ++i) {
        ...
        sprintf(buf, "insert into %s values( %d, %d, %d, %d, %d, ", Fi->table,
                i, segment, next_segment, order, next_order);
        ...
        if (Rast_is_d_null_value(&sinusoid) || Rast_is_d_null_value(&stright) ||
            stright == 0)
            sprintf(buf2, "null, ");
        else
            sprintf(buf2, "%g, ", sinusoid);
        ...
        if (Rast_is_d_null_value(&gradient) || Rast_is_d_null_value(&length) ||
            length <= 0)
            sprintf(buf2, "null, ");
        else
            sprintf(buf2, "%g, ", gradient);
