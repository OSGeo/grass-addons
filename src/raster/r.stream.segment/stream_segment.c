...
{
    return (cp - cn) == 0  ? (rp - rn) > 0 ? 0 : PI
           : (cp - cn) < 0 ? PI / 2 + atan((rp - rn) / (float)(cp - cn))
                  ...
                  int number_streams)
{
    ...
    if (reached_end) {
        G_warning(_("Network topology error: cannot identify stream join for "
                    "stream %d"),
                  cur_stream->stream);
    ...
    cell_down = i >= (SA[next_stream].number_of_cells - seg_length)
                    ? SA[next_stream].number_of_cells - seg_length - 1
                    : seg_length;
    ...
    cur_stream->tangent = i == 1         ? -1
                          : i < seg_skip ? cur_stream->continuation
                                         : calc_dir(r_up, c_up, r_down, c_down);
