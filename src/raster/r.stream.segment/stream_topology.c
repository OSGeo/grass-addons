...
{ /* calculate number of tributaries */
    ...
    if (trib_num > 5)
        G_fatal_error(_("Error finding inits. Stream and direction maps "
                        "probably do not match"));
    if (trib_num > 3)
        G_warning(_("Stream network may be too dense"));

    if (same_id > 1)
        G_warning(_("Too many stream segments with the same ID %d"),
                  streams_cell);
    else if (same_id == 1)
        trib_num = 1;
                          ...
                          int number_of_streams)
{
    ...
    for (r = 0; r < nrows; ++r) {
        for (c = 0; c < ncols; ++c) {
            if (streams[r][c]) {
                if (ram_trib_nums(r, c, streams, dirs) !=
                    1) { /* adding inits */
                    if (stream_num > number_of_streams)
                        G_fatal_error(
                            _("Error finding inits. Stream and direction maps "
                              "probably do not match"));
    ...
    for (i = 1; i < stream_num; ++i) {
        ...
        do {
            ...
            c = NC(d);
                 ...
                 ram_trib_nums(r, c, streams, dirs) == 1);

        SA[i].number_of_cells +=
            2; /* add two extra points for init+ and outlet+ */
    }

    for (i = 1; i < number_of_streams; ++i) {

        SA[i].points =
            (long int *)G_malloc((SA[i].number_of_cells) * sizeof(long int));
        SA[i].elevation =
            (float *)G_malloc((SA[i].number_of_cells) * sizeof(float));
        SA[i].distance =
            (double *)G_malloc((SA[i].number_of_cells) * sizeof(double));
        ...
        SA[i].elevation[0] =
            (contrib_cell == 0) ? -99999 : elevation[prev_r][prev_c];
        d = (contrib_cell == 0) ? dirs[r][c] : dirs[prev_r][prev_c];
        SA[i].distance[0] = (contrib_cell == 0)
                                ? get_distance(r, c, d)
        ...
        do {
            ...
            if (NOT_IN_REGION(d) || d == 0) {
                ...
                SA[i].elevation[cell_num] = 2 * SA[i].elevation[cell_num - 1] -
...
int seg_build_streamlines(SEGMENT *streams, SEGMENT *dirs, SEGMENT *elevation,
                          int number_of_streams)
{
    int r, c, i;
    int d, next_d;
    ...
    for (r = 0; r < nrows; ++r) {
        for (c = 0; c < ncols; ++c) {
            ...
            if (streams_cell) {
                if (seg_trib_nums(r, c, streams, dirs) !=
                    1) { /* adding inits */
                    if (stream_num > number_of_streams)
                        G_fatal_error(
                            _("Error finding inits. Stream and direction maps "
    ...
    for (i = 1; i < stream_num; ++i) {
        ...
        do {
            ...
            Segment_get(streams, &streams_cell, r, c);
                 ...
                 seg_trib_nums(r, c, streams, dirs) == 1);

        SA[i].number_of_cells +=
            2; /* add two extra points for point before init and after outlet */
    }
    ...
    for (i = 1; i < number_of_streams; ++i) {
        ...
        SA[i].points[0] = (contrib_cell == 0) ? -1 : INDEX(prev_r, prev_c);
        SA[i].elevation[0] = (contrib_cell == 0) ? -99999 : elevation_prev_cell;
