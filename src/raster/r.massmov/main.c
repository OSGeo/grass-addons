...
#define laxfactor \
    0.5 /* Low pass filtering for the lax function (0 = no filtering) */
...
int main(int argc, char *argv[])
{
    ...
    struct Option *input_ELEV, *input_HINI, *input_DIST, *input_RHEOL,
        *input_RHO, *input_YSTRESS, *input_CHEZY, *input_VISCO, *input_BFRICT,
        *input_IFRICT, *input_FLUID, *input_TIMESTEPS, *input_DELTAT, *output_H,
        *output_VEL, *input_STOP_THRES, *input_STEP_THRES, *input_THREADS,
        *output_HMAX, *output_VMAX;
    ...
    module->description = _("Estimates run-out and deposition of landslide "
                            "phenomena over a complex topography.");
    ...
    input_YSTRESS->description = _("Apparent yield stress [Pa]. Used only for "
                                   "viscous rheologies (optional).");
    ...
    input_CHEZY->description = _("Chezy roughness coefficient [m/s2]. Required "
                                 "only for Voellmy rheology");
    ...
    input_FLUID->description = _("Upward velocity of transition from solid to "
                                 "fluid of the landsliding mass [m/s]");
    ...
    if (THREADS != -1) {
        ...
        if (THREADS <= max_th)
            ...
            G_fatal_error(
                _("The maximum number of parallel threads available is %i"),
                max_th);
    }
    ...
    if (((STOP_THRES != -1) && (STEP_THRES == -1)) ||
        ...
        G_fatal_error(
            _("To apply the stopping criterion both the threshold value and "
              "the calculation step of stopping criterion are required"));
    }
    ...
    if (testPar == -2)
        G_fatal_error(
            _("For the selected rheology Chezy parameter is required"));

    if (testPar == -3)
        G_fatal_error(_("For the selected rheology viscosity and density "
    ...
    if (flag_mem->answer) {
        mem =
            (((25.0 * sizeof(double) * nrows * ncols) / pow(1024.0, 2)) + 0.55);
        fprintf(stdout,
                "The memory required to run the model on the selected region "
                "is about %f MB\n",
                mem);
    }
    ...
    k_act =
        (1 - sin((M_PI * IFRICT) / 180.0)) / (1 + sin((M_PI * IFRICT) / 180.0));
...
#pragma omp parallel for private(row, col, gx, gy, slope)
    for (row = 1; row < nrows - 1; row++) {
        for (col = 1; col < ncols - 1; col++) {
            ...
            slope = sqrt(pow(gradx3(m_ELEV, row, col, res_ew, 0), 2) +
                ...
                if (m_HINI[row][col] > 0.0) {
                    if (t * dT * FLUID >=
                        ...
                        m_H[row][col] +=
                            ((t * dT * FLUID) -
                             (m_DIST[row][col] - (res_ew + res_ns) / 4)) /
                            ((res_ew + res_ns) / 2) * m_HINI[row][col];
                        Vol_in +=
                            ((((t * dT * FLUID) -
                               (m_DIST[row][col] - (res_ew + res_ns) / 4)) /
                              ((res_ew + res_ns) / 2) * m_HINI[row][col]) /
    ...
    dt, CFL_u, CFL_v, CFL) shared(dn_loops, CFL_max)
                for (row = 1; row < nrows - 1; row++) {
                    ...
                    for (col = 1; col < ncols - 1; col++) {
                        ...
                        Uloop_a = filter_lax(m_Uloop, row, col, laxfactor,
                                             m_Hloop, 0.01, 0.0);
                        Vloop_a = filter_lax(m_Vloop, row, col, laxfactor,
                                             m_Hloop, 0.01, 0.0);
                        ...
                        /* calculation of T_x_b and T_y_b as function of Uloop_b
                         * and Vloop_b */
                        ...
                        CFL_u = dt * sqrt(2) *
                                fabs((m_ca_gx[row][col]) * Uloop_dt) / res_ew;

                        CFL_v = dt * sqrt(2) *
                                fabs((m_ca_gy[row][col]) * Vloop_dt) / res_ns;
...
#pragma omp parallel for private(row, col, dH_dT, Hloop_a, Hloop_dt)
                    for (row = 1; row < nrows - 1; row++) {
                        for (col = 1; col < ncols - 1; col++) {
                            ...
                            dH_dT = -m_ca_gx[row][col] *
                                        (shift0(m_HUloop, row, col, nrows - 1,
                                                ncols - 1, 1, 1, 0, -1) -
                                         shift0(m_HUloop, row, col, nrows - 1,
                                                ncols - 1, 1, 1, 0, 1)) /
                                        (2 * res_ew / m_ca_gx[row][col]) -
                                    m_ca_gy[row][col] *
                                        (shift0(m_HVloop, row, col, nrows - 1,
                                                ncols - 1, 1, 1, 1, 0) -
                                         shift0(m_HVloop, row, col, nrows - 1,
                                                ncols - 1, 1, 1, -1, 0)) /
                                        (2 * res_ns / m_ca_gy[row][col]);
                    ...
                    for (row = 1; row < nrows - 1; row++) {
                        for (col = 1; col < ncols - 1; col++) {
                            ...
                            if (m_OUTLET[row][col] == 1) {
                                Vol_out_t += m_Hloop_dt[row][col] /
                                             (m_slope[row][col]) * ca;
                                m_Hloop_dt[row][col] = 0.0;
                            }

                            /* Vol_sim */
                            Vol_sim += (m_Hloop_dt[row][col] /
                                        (m_slope[row][col]) * ca);
                    ...
                    mbe =
                        (Vol_sim + Vol_out_t) / (Vol_in_tot - Vol_out_tot_corr);
...
#pragma omp parallel for private(row, col)
                    for (row = 1; row < nrows - 1; row++) {
                        for (col = 1; col < ncols - 1; col++) {
                            if (m_H[row][col] > verysmall) {
                                if ((sqrt(pow(m_U[row][col], 2) +
                                          pow(m_V[row][col], 2))) >
                                    m_Vmax[row][col]) {
                                    m_Vmax[row][col] =
                                        (sqrt(pow(m_U[row][col], 2) +
                                              pow(m_V[row][col], 2)));
                                }
                            }
                        }
                ...
                if ((t == TIMESTEPS) || (STOP_count == 3)) {
                    sprintf(timestr,
                            "Calculated in %d steps; %0.2f of %0.2f mc gone "
                            "outbound",
    ...
    if (Vol_out_tot_corr_loop > 0) {
        G_warning("The flow run out of the current computational domain. You "
                  "may need to change the region extension.\nSee raster "
                  "metadata (r.info) for more information.");
    }
