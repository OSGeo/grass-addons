...
int main(int argc, char *argv[])
{
    ...
    int outfd_H, outfd_VEL, outfd_VEL_DIR, outfd_HMAX, outfd_T_HMAX,
        outfd_I_HMAX;
    int outfd_VMAX, outfd_T_VMAX, outfd_I_VMAX, outfd_DIR_VMAX, outfd_IMAX,
        outfd_T_IMAX, outfd_WAVEFRONT;
    ...
    struct Option *input_ELEV, *input_LAKE, *input_DAMBREAK, *input_MANNING,
        *input_DELTAT, *input_TSTOP, *input_TIMESTEP, *input_U, *input_V;
    struct {
        struct Option *opt_t;
    } parm;
    struct Option *output_VEL, *output_H, *output_HMAX, *output_VMAX,
        *output_IMAX, *output_WAVEFRONT;
    struct Flag *flag_d;
    struct {
        struct Option *met;
    } opt;
    ...
    flag_d->key = 'd';
    flag_d->description =
        _("Flow direction additional output (aspect visualization)");
    ...
    opt.met->required = NO;
    opt.met->description =
        _("Computational method for initial velocity estimation");
    opt.met->options =
        "dambreak-without_hypotesis,uniform drop in of lake,small dam breach";
    ...
    total_dambreak = Hp altezza critica --> velocita' iniziale (h critica) =
    0.93*sqrt(h); small_dam_breach = Hp stramazzo --> velocita' iniziale =
    0.4*sqrt(2*g*h)
    */
    ...
    input_TIMESTEP->description =
        _("Initial computational time step [s] - CFL condition");
    ...
    input_ELEV->description =
        _("Name of elevation raster map (including lake bathymetry and dam)");
    ...
    input_MANNING->description =
        _("Name of Manning's roughness coefficient raster map");
    ...
    input_V->description =
        _("Initial velocity along the north direction [m/s]");
    ...
    parm.opt_t->description =
        _("Additional instants for output map generation [s]");
    ...
    output_HMAX->description =
        _("Name of output maximum water depth raster map; relative intensity "
          "[h*v] and time map are also generated");
    ...
    output_VMAX->description =
        _("Name of output maximum water velocity raster map; relative "
          "intensity [h*v] and time map are also generated");
    ...
    output_IMAX->description =
        _("Name of output maximum intensity [h*v] raster map; relative time "
          "map are also generated");
    ...
    output_WAVEFRONT->description =
        _("Name of output wave front time[s] raster map");
    ...
    if ((output_H->answer == NULL) && (output_VEL->answer == NULL) &&
        (output_HMAX->answer == NULL) && (output_VMAX->answer == NULL) &&
        (output_IMAX->answer == NULL) && (output_WAVEFRONT->answer == NULL)) {
        G_fatal_error(_("Sorry, you must choose an output map."));
    }

    if (flag_d->answer && (output_H->answer == NULL)) {
        G_fatal_error(
            "You choose flow direction map without flow velocity prefix name");
    }
    ...
    if (strcmp(opt.met->answer, "uniform drop in of lake") == 0)
        ...
        G_fatal_error(
            _("Unknown method. Please, select a computational method"));
        ...
        for (row = 0; row < nrows; row++) {
            for (col = 0; col < ncols; col++) {
                ...
                if (m_DAMBREAK[row][col] > 0) {
                    ...
                    G_fatal_error(
                        _("Don't find the dambreak - Please select a correct "
            ...
            for (col = 1; col < ncols - 1; col++) {
                if (m_DAMBREAK[row][col] > 0) {
                    ...
                    if (m_h2[row][col] <= 0) {
                        m_h2[row][col] = 0.0;
                        if (m_h1[row][col] > 0) {
                            // questo warning va modificato perchè vale per ogni
                            // cella ---> bisogna metterne uno generico che
                            // valga quando tutte le celle sono con h=0
                            num_break--;
                            if (num_break == 0) {
                                if (warn1 == 0) {
                                    G_warning("At the time %.0fs no water go "
                                              "out from lake",
                                              t);
                                }
                            }
    ...
    for (t = 0; t <= TSTOP; t += timestep) {
        ...
        if (t < TSTOP) {
            ...
            for (row = 1; row < nrows - 1; row++) {
                for (col = 1; col < ncols - 1; col++) {
                    if ((row == 1 || row == (nrows - 2) || col == 1 ||
                         col == (ncols - 2)) &&
                        (m_v2[1][col] > 0 || m_v2[nrows - 2][col] < 0 ||
                         m_u1[row][1] < 0 || m_u1[row][ncols - 2] > 0)) {
                        if (reg_lim == 0) {
                            G_warning("At the time %.3f the computational "
                                      "region is smaller than inundation",
                                      t);
                            reg_lim = 1;
                    ...
                    if (m_h2[row][col] >= hmin) {
                        timestep_ct_temp = max(
                            (fabs(m_u2[row][col]) + sqrt(g * m_h2[row][col])) /
                                res_ew,
                            (fabs(m_v2[row][col]) + sqrt(g * m_h2[row][col])) /
                                res_ns);
                    ...
                    if (OUT_HMAX) {
                        if (m_hmax[row][col] < m_h2[row][col]) {
                            ...
                            m_t_hmax[row][col] = t;
                            velocity = sqrt(pow(m_u1[row][col], 2.0) +
                                            pow(m_v1[row][col], 2.0));
                    ...
                    if (OUT_VMAX) {
                        ...
                        if (m_vmax[row][col] < velocity) {
                            ...
                            if (flag_d->answer) {
                                ...
                                else if (m_u1[row][col] > 0 &&
                                         m_v1[row][col] > 0) {
                                    m_dir_vmax[row][col] =
                                        180 / PI *
                                        atan(fabs(m_v1[row][col]) /
                                             fabs(m_u1[row][col]));
                                }
                                else if (m_u1[row][col] == 0 &&
                                         m_v1[row][col] > 0) {
        ...
        if ((input_DELTAT->answer != NULL) || (parm.opt_t->answer != NULL)) {
            if ((((m * DELTAT - t) <= timestep && (m * DELTAT) < TSTOP) &&
                ...
                if ((m * DELTAT - t) <= timestep &&
                    m * DELTAT < TSTOP) { /* devo cambiare il nome del raster e
                                             aggiungere ogni volta _timestep*/
                    if (OUT_H) {
                ...
                if (flag_d->answer) {
                    if ((outfd_VEL_DIR = Rast_open_new(name3, DCELL_TYPE)) <
                        0) {
                ...
                for (row = 0; row < nrows; row++) {
                    for (col = 0; col < ncols; col++) {
                        ...
                        if (OUT_VEL) {
                            ((DCELL *)outrast_VEL)[col] =
                                sqrt(pow(m_u1[row][col], 2.0) +
                                     pow(m_v1[row][col], 2.0));
                        }
                        if (flag_d->answer) {
    ...
    for (row = 0; row < nrows; row++) {
        ...
        for (col = 0; col < ncols; col++) {
            ...
            if (OUT_VEL) {
                ((DCELL *)outrast_VEL)[col] =
                    sqrt(pow(m_u1[row][col], 2.0) + pow(m_v1[row][col], 2.0));
            }
            if (flag_d->answer) {
                if (m_u1[row][col] == 0 && m_v1[row][col] == 0) {
                    Rast_set_d_null_value(&outrast_VEL_DIR[col], 1);
                    ...
                    ((DCELL *)outrast_VEL_DIR)[col] =
                        180 / PI *
                        atan(fabs(m_v1[row][col]) / fabs(m_u1[row][col]));
                }
                else if (m_u1[row][col] == 0 && m_v1[row][col] > 0) {
                    ((DCELL *)outrast_VEL_DIR)[col] = 90.0;
                }
                    ...
                    ((DCELL *)outrast_VEL_DIR)[col] =
                        90.0 +
                        180 / PI *
                            atan(fabs(m_u1[row][col]) / fabs(m_v1[row][col]));
                }
                else if (m_u1[row][col] < 0 && m_v1[row][col] == 0) {
                    ((DCELL *)outrast_VEL_DIR)[col] = 180.0;
                }
    ...
    if (OUT_HMAX) {
        G_message(
            "Writing the output final map %s, corresponding time and intensity",
            OUT_HMAX);
    }
    if (OUT_VMAX) {
        G_message(
    ...
    if (OUT_IMAX) {
        G_message("Writing the output final map %s and corresponding time",
                  OUT_IMAX);
    }
    if (OUT_WAVEFRONT) {
