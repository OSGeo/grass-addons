...
Parameter **input elevation raster map** is the DEM used in the terrain correction.
The region resolution should be set to match the DEM. Calculations are done in
meters, so a UTM coordinate system should be used. Parameter **input vector map**
is the vector points file with the measured gravity station locations. Parameter
**output file** is the output file with the calculated terrain corrections in mgal.
This tool also allows multi-thread parallel processing to speed calculations.

## NOTES

This tool calculates the final term in the below equation (g(Terrain correction)).

**g(Bouguer complete)=g(Observed) - g(Latitude) + g(Free Air) - g(Simple Bouguer)
...
measurement location. The tool must be used in a UTM coordinate system as the
...
This (g) is the gravitational effect difference from a uniform slab correction for
a single DEM pixel with respect to the gravity station measurement location. It is
a calculation for a flat-topped rectangular prism. There are other methods that
could be implemented, but this is computationally efficient. Other methods include
using prisms with slanted tops to better "fit" topography or fitting polynomial
...
This is the cross-sectional area of a square DEM pixel in meters. It should match
the GRASS region grid spacing (and the actual DEM).
...
```sh
r.gravity.terrain input=90mSRTM_DEM Maximum_distance=30000m Minimum_distance=90m
...
due to the small number of points with terrain correction values greater than 20 mgal.*
