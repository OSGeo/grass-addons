                               ...
                               Viewpoint * vp, GridHeader hd)
{
	...
	double dist = G_distance(G_col_to_easting(sn->col + 0.5, &(hd.window)),
				 G_row_to_northing(sn->row + 0.5, &(hd.window)),
				 G_col_to_easting(vp->col + 0.5, &(hd.window)),
		    ...
		    Viewpoint * vp, GridHeader hd)
{
    ...
    if (G_projection() == PROJECTION_LL) {
	double dist = G_distance(G_col_to_easting(col + 0.5, &(hd.window)),
