...
get_square_distance_from_viewpoint(const AEvent & a, const Viewpoint & vp)
{
	...
	dist = G_distance(G_col_to_easting(vp.col + 0.5, &window),
			  G_row_to_northing(vp.row + 0.5, &window),
			  G_col_to_easting(eventx + 0.5, &window),
			  G_row_to_northing(eventy + 0.5, &window));
...
int DistanceCompare::compare(const AEvent & a, const AEvent & b)
{
	...
	da = G_distance(G_col_to_easting(globalVP.col + 0.5, &window),
			  G_row_to_northing(globalVP.row + 0.5, &window),
			  G_col_to_easting(eventx + 0.5, &window),
			  G_row_to_northing(eventy + 0.5, &window));
	...
	db = G_distance(G_col_to_easting(globalVP.col + 0.5, &window),
			  G_row_to_northing(globalVP.row + 0.5, &window),
			  G_col_to_easting(eventx + 0.5, &window),
			  G_row_to_northing(eventy + 0.5, &window));
