...
*/
    ...
    */
    ...
    if (palette) {
        ...
        G_debug(1, "max %f min %f mapsize %d", colors.cmax, colors.cmin,
                mapsize);

        mapptr =
            (unsigned short *)G_calloc(mapsize * 3, sizeof(unsigned short));
        redp = mapptr;
        grnp = redp + mapsize;
        blup = redp + mapsize * 2;
...
#define SCALE(x) (((x) * ((1L << 16) - 1)) / 255)
        ...
        for (i = colors.cmin; i <= colors.cmax; i++, redp++, grnp++, blup++) {
            ...
            *blup = (unsigned short)(SCALE(blu));
            ...
            G_debug(1, " %d : %d %d %d   %d %d %d", i, red, grn, blu, *redp,
                    *grnp, *blup);
        }

        TIFFSetField(out, TIFFTAG_COLORMAP, mapptr, mapptr + mapsize,
                     mapptr + mapsize * 2);
        ...
        TIFFSetField(out, TIFFTAG_PHOTOMETRIC,
                     h.ras_depth == 24 ? PHOTOMETRIC_RGB
    ...
    if (tiled) {
        ...
        for (row = 0; row < imagelength; row += tilelength) {
            uint32 nrow = (row + tilelength > imagelength) ? imagelength - row
                                                           : tilelength;
            uint32 colb = 0;
            ...
            for (col = 0; col < imagewidth; col += tilewidth) {
                ...
                for (i = 0; i < nrow; i++) {
                    ...
                    if (palette) {
                        ...
                        for (j = 0; j < width; j++) {
                            Rast_get_c_color(&(cellptr[col + j]), &red, &grn,
                                             &blu, &colors);
