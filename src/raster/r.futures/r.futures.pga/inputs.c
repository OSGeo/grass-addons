        ...
        for (col = 0; col < cols; col++) {
            ...
            if (segments->use_potential_subregions) {
                if (!Rast_is_null_value(&((CELL *)pot_subregions_row)[col],
                    ...
                    if (!KeyValueIntInt_find(potential_region_map, c,
                                             &pot_region_index)) {
                        KeyValueIntInt_set(potential_region_map, c,
                                           pot_count_regions);
                         ...
                         struct KeyValueIntInt *region_map, int num_predictors)
{
    ...
    potentialInfo->intercept =
        (double *)G_malloc(region_map->nitems * sizeof(double));
    ...
    for (i = 0; i < region_map->nitems; i++) {
        if (!check[i]) {
            G_warning("Region %d missing in potential file.",
                      region_map->key[i]);
            problem = true;
        }
    }
...
void read_patch_sizes(struct PatchSizes *patch_sizes,
                      struct KeyValueIntInt *region_map, double discount_factor)
{
    FILE *fp;
    size_t buflen = 4000;
    char buf[buflen];
            ...
            for (i = 0; i < region_map->nitems; i++) {
                ...
                for (j = 0; j < num_regions; j++)
                    if (region_map->key[i] == atoi(header_tokens[j]))
                        found = true;
                if (!found)
                    G_fatal_error(_("Subregion id <%d> not found in header of "
                                    "patch file <%s>"),
            ...
            for (i = 0; i < ntokens; i++) {
                if (strcmp(tokens[i], "") != 0) {
                    ...
                    if (patch > 0) {
                        ...
                        if (use_header) {
                            if (!KeyValueIntInt_find(region_map,
                                                     atoi(header_tokens[i]),
                                                     &region_id))
                                continue;
                        }
                        else
