                      ...
                      int number_of_streams)
{
    ...
    if (0 < stream_add_table(number_of_streams))
        G_warning(_("Unable to add attribute table to vector map <%s>"),
                  out_vector);
...
int stream_add_table(int number_of_streams)
{
    ...
    char *tab_orders = "strahler integer, horton integer, shreve integer, hack "
                       "integer, topo_dim integer";
    ...
    sprintf(buf,
            ...
            "%s, %s, %s, %s, %s,"
            "%s, %s, %s)",
            Fi->table, tab_cat_col_name,                               /* 1 */
            tab_stream, tab_next_stream, tab_prev_streams, tab_orders, /* 5 */
            tab_scheidegger, tab_drwal_old, tab_length, tab_stright,
            tab_sinusoid, /* 10 */
            tab_cumlength, tab_accum, tab_distance, tab_elev_init,
            tab_elev_outlet,                     /* 15 */
    ...
    for (i = 0; i < number_of_streams; ++i) {
        ...
        sprintf(insert_orders, "%d, %d, %d, %d, %d", all_orders[0][i],
                all_orders[1][i], all_orders[2][i], all_orders[3][i],
                all_orders[4][i]);
        ...
        sprintf(buf,
                ...
                "%f, %f, %f)",
                Fi->table, i, /* 1 */
                SA[i].stream, SA[i].next_stream,
                ins_prev_streams,      /* buffer created before */
                insert_orders, /* 5 */ /* buffer created before */
                scheidegger, drwal_old, SA[i].length, SA[i].stright,
                sinusoid, /* 10 */
                SA[i].accum_length, fabs(SA[i].accum), SA[i].distance,
                SA[i].init_elev, SA[i].outlet_elev, /* 15 */
