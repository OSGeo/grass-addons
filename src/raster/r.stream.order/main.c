...
int main(int argc, char *argv[])
{
    ...
    IO output[] = {
        {"strahler", NO,
         _("Name for output Strahler's stream order raster map")},
        {"horton", NO,
         _("Name for output original Hortons's stream order raster map")},
        {"shreve", NO,
         _("Name for output Shereve's stream magnitude raster map")},
        {"hack", NO,
         _("Name for output Hack's streams or Gravelius stream hierarchy "
           "raster map")},
        {"topo", NO,
         _("Name for output topological dimension of streams raster map")}};
    ...
    if (use_vector)
        if (!opt_input[o_elev]->answer || !opt_input[o_accum]->answer)
            G_fatal_error(_("To calculate vector map both accumulation and "
                            "elevation raster maps are required"));
    if (use_accum)
        if (!opt_input[o_accum]->answer)
            G_fatal_error(_("Flag -a (use accumulation) accumulation raster "
                            "map is required"));
    ...
    if (!output_num && !opt_vector->answer)
        G_fatal_error(_("You must select one or more output orders raster maps "
                        "or insert the table name"));
