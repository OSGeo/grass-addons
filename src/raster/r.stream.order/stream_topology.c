...
int ram_stream_topology(CELL **streams, CELL **dirs, int number_of_streams)
{
    ...
    outlet_streams =
        (unsigned int *)G_malloc((number_of_streams) * sizeof(unsigned int));
    init_streams =
        (unsigned int *)G_malloc((number_of_streams) * sizeof(unsigned int));
    init_cells = (unsigned long int *)G_malloc((number_of_streams) *
    ...
    for (r = 0; r < nrows; ++r)
        for (c = 0; c < ncols; ++c)
            if (streams[r][c] > 0) {
                ...
                if (cur_stream !=
                    next_stream) { /* junction: building topology */

                    if (outlet_num > (number_of_streams - 1))
                        G_fatal_error(_("Error finding nodes. "
                                        "Stream and direction maps probably do "
                                        "not match."));
                ...
                if (trib_num == 0) { /* is init */
                    ...
                    init_streams[init_num++] =
                        cur_stream; /* collecting inits */
                }
    ...
    for (s = 0; s < init_num; ++s) { /* main loop on springs */
        ...
        while (done) {
            ...
            if (d < 1 || NOT_IN_REGION(d) || !streams[next_r][next_c]) {
                SA[cur_stream].stright = G_distance(
                    cur_easting, cur_northing, init_easting, init_northing);
                SA[cur_stream].outlet =
                    (r * ncols + c); /* add outlet to sorting */
                break;
            }

            next_northing = window.north - (next_r + .5) * window.ns_res;
            next_easting = window.west + (next_c + .5) * window.ew_res;
            cur_length = G_distance(next_easting, next_northing, cur_easting,
                                    cur_northing);
            ...
            if (streams[next_r][next_c] != cur_stream) {
                SA[cur_stream].stright = G_distance(
                    next_easting, next_northing, init_easting, init_northing);
