...
int stream_sample_map(char *input_map_name, int number_of_streams, int what)
{
    ...
    for (i = 0; i < (number_of_streams); ++i) {
        ...
        case 0: /* inits for elev */
            pointers_to_stream[i]->init_elev = Rast_get_d_value(
                input_ptr + c * input_data_size, input_data_type);
            break;
        case 1: /* outlets for elev */
            pointers_to_stream[i]->outlet_elev = Rast_get_d_value(
                input_ptr + c * input_data_size, input_data_type);
            break;
        case 2: /* outlets for accum */
            pointers_to_stream[i]->accum = Rast_get_d_value(
                input_ptr + c * input_data_size, input_data_type);
            break;
        }
    }
