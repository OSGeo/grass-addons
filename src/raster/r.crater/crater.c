                 ...
                 int target_type)
{
    ...
    if (target_type == 3) { /*Solid rock*/
        ...
        Dat = 0.015 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
              pow(W, 0.37) * pow(sin(theta), 2 / 3.0);
        if (Dat > 10) {
            /*for craters up to 100 m + loose target rock or regolith*/
            Dat = 0.25 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
                  pow(W, 0.29) * pow(sin(theta), 1 / 3.0);
            if (Dat > 100) {
                /*for craters > 100 m up to 1000 m + any kind of target
                 * material*/
                Dat = 0.27 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
                      pow(W, 0.28) * pow(sin(theta), 1 / 3.0);
            }
        }
    }
    else {
...
{
    return (0.0133 * pow(W, 1 / 3.4) +
            1.51 * pow(r_proj, 1 / 2.0) * pow(r_targ, -1 / 2.0) * L);
}
...
{
    return (1.8 * pow(r_proj, 0.11) * pow(r_targ, -1 / 3.0) * pow(g, -0.22) *
            pow(L, 0.13) * pow(W, 0.22));
}
               ...
               double theta, int target_type)
{
    ...
    if (Dat >= 100) {
        /*for craters > 100 m up to 1000 m + any kind of target material*/
        L = pow(Dat / (0.27 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
                       pow(1 / 3.0 * PI * Vi * Vi, 0.28) * pow(1 / 2.0, 3.28) *
                       pow(sin(theta), 1 / 3.0)),
                1 / 3.28);
    }
    ...
    if (target_type == 3) { /*Solid rock*/
        if (Dat < 10) {
            /*for craters up to 10 m + solid target rock*/
            L = pow(Dat /
                        (0.015 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
                         pow(1 / 3.0 * PI * Vi * Vi, 0.37) *
                         pow(1 / 2.0, 3.37) * pow(sin(theta), 2 / 3.0)),
                    1 / 3.37);
        }
        else {
            /*for craters up to 100 m + loose target rock or regolith*/
            L = pow(Dat / (0.25 * pow(r_proj, 1 / 6.0) * pow(r_targ, -1 / 2.0) *
                           pow(1 / 3.0 * PI * Vi * Vi, 0.29) *
                           pow(1 / 2.0, 3.29) * pow(sin(theta), 1 / 3.0)),
                    1 / 3.29);
        }
    }
    else {
...
{
    return pow(Dat / (0.0133 * pow(1 / 3.0 * PI * Vi * Vi, 1 / 3.4) *
                          pow(1 / 2.0, 3 / 3.4) +
                      1.51 * pow(r_proj, 1 / 2.0) * pow(r_targ, -1 / 2.0)),
               3.4 / 6.4);
}
...
{
    return pow(Dat / (1.8 * pow(r_proj, 0.11) * pow(r_targ, -1 / 3.0) *
                      pow(g, -0.22) * pow(1 / 3.0 * PI * Vi * Vi, 0.22) *
                      pow(1 / 2.0, 0.66)),
               1 / 0.79);
}
