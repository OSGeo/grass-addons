...
def test_random_reproducible_with_seed(setup_nlcd):
    """Test that random method produces reproducible results with same seed."""
    session = setup_nlcd
    ...
    val1 = get_cell_value("mannings_n_1", 0, 1, session.env)
    val2 = get_cell_value("mannings_n_2", 0, 1, session.env)
    assert val1 == pytest.approx(val2)


def test_random_different_with_different_seed(setup_nlcd):
    """Test that random method produces different results with different seeds."""
    session = setup_nlcd
