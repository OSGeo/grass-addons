...
def main():
    ...
    # array per la somma dei massi (Array for the sum of boulders)
    tot = garray.array(r_elevation)
    tot[...] = (tot * 0.0).astype(float)

    # array per le velocita (Array for velocity)
    velMax = garray.array()
    velMean = garray.array()

    # array per energia (Array for energy)
    enMax = garray.array()
    enMean = garray.array()
    ...
    for i in xrange(len(quota) - 1):
        ...
        # (Cost calculation (replace the starting points in start_raster at the point of the point))
        grass.run_command(
            "r.cost",
            flags="k",
            input="uno",
        ...
        # (Calculate A=tangent of shadow angle (INPUT) * cost in meters)
        ...
        # valore di deltaH (F) (delatH value (F))
        grass.mapcalc("F=D*E", overwrite=True)

        # calcolo velocita (velocity calculation)
