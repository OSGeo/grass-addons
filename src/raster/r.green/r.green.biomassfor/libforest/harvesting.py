...
def slope_computation(elevation):
    ...
    run_command(
        "r.slope.aspect", overwrite=True, elevation=elevation, slope=tmp_slope_deg
    )


def yield_pix_process(
    opts,
    vector_forest,
    yield_,
    yield_surface,
    ...
    r.mapcalc(
        expr.format(
            pix_cross=("tmprgreen_%i_pix_cross" % pid), tmp_slope_deg=tmp_slope_deg
        ),
        overwrite=True,
    )
    ...
    ccextr = (
        "{cable_crane_extraction} = if({yield_} >0 && {tmp_slope}"
    ...
    fwextr = (
        "{forwarder_extraction} = if({yield_}>0 && {tmp_slope}<="
    ...
    expression = (
        "{technical_surface} = {cable_crane_extraction} +"
    ...
    ehf = (
        "{tech_bioHF} = {technical_surface}*{yield_pix}*"
        "({m1t1}*{ton_tops_hf}+"
