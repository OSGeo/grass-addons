...
def revenues(
    opts, yield_surface, m1t1, m1t2, m1, m2, forest, yield_, technical_bioenergy
):
    # Calculate revenues
    pid = os.getpid()
...
def productivity(
    ...
    expr = (
        "{fell_productHFtr2} = {mt}*{cable_crane_extraction}*"
        "(42-2.6*{tree_diam})/(-20)*1.65*(1-(1000-90*{slope}/(-80))/100)"
    )
    r.mapcalc(
        expr.format(
            fell_productHFtr2=fell_productHFtr2,
            mt=m1t2,
            cable_crane_extraction="cable_crane_extraction",
            tree_diam="tmprgreen_%i_tree_diam" % pid,
    ...
    expr = (
        "{chipp_prod} = {m1t1}*{yield_pix}/{num11}"
        "+{m1t2}*{yield_pix}/{num12}"
        "+{m2}*{yield_pix}/{num2}"
    )
    r.mapcalc(
        expr.format(
            chipp_prod=chipp_prod,
    ...
    extr_product = {}
    extr_product[extr_product_cableHF] = [
        m1,
        "cable_crane_extraction",
        149.33,
        extr_dist,
        -1.3438,
        0.75,
    ]
    extr_product[extr_product_cableC] = [
        m2,
        "cable_crane_extraction",
        149.33,
        extr_dist,
    ...
    extr_product[extr_product_forw] = [
        1,
        "forwarder_extraction",
        36.293,
        extr_dist,
        -1.1791,
        0.6,
    ]
    extr_product[extr_product_other] = [
        1,
        "other_extraction",
        36.293,
        extr_dist,
    ...
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression=("%s = %s ||| %s" % (tot_roads, forest_roads, main_roads)),
    )
    run_command("r.null", map=tot_roads, null=0)

    expr = (
        "{frict_surf_tr}={frict_surf_extr}*not({tot_roads})"
        "*{tot_roads}*((ewres()+nsres())/2)"
    )
    r.mapcalc(
        expr.format(
            frict_surf_tr="tmprgreen_%i_frict_surf_tr" % pid,
    ...
    try:
        ...
        run_command(
            "r.mapcalc",
            overwrite=True,
            expression=("%s = %s - %s" % (transp_dist, tot_dist, extr_dist)),
        )
    except:
        run_command(
            "r.mapcalc",
    ...
    expr = (
        "{transport_prod} = {transp_dist}/30000"
        "*({not2}*({yield_pix}/32)*2 +{m1t2}*({yield_pix}*2.7/32)*2)"
    )

    r.mapcalc(
        expr.format(
            transport_prod=transport_prod,
            yield_pix="yield_pix1",
            not2=not2,
            m1t2=m1t2,
            transp_dist="tmprgreen_%i_transp_dist" % pid,
...
def costs(opts, dic1, dic2, total_revenues, yield_pix):
    ...
    for key, val in dic1.items():
        r.mapcalc(
            expr.format(
                out="tmprgreen_%i_cost_%s" % (pid, key),
                yield_pix="yield_pix1",
                cost=val,
                productivity=key,
            ),
        ...
        run_command("r.null", map=("tmprgreen_%i_cost_%s" % (pid, key)), null=0)
    ...
    for key, val in dic2.items():
        r.mapcalc(
            expr.format(
                out="tmprgreen_%i_cost_%s" % (pid, key), cost=val, productivity=key
            ),
            overwrite=True,
        )
        run_command("r.null", map=("tmprgreen_%i_cost_%s" % (pid, key)), null=0)
    ...
    run_command("r.mapcalc", overwrite=True, expression=command[:-1])
    ...
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression="%s =  %s *0.05" % (direction_cost, prod_costs),
    )
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression=("%s =  %s*0.07" % (administrative_cost, total_revenues)),
    )
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression=(
            "%s= (%s + %s)*0.03/4" % (interests, prod_costs, administrative_cost)
        ),
    )
    ...
    run_command(
        "r.mapcalc", overwrite=True, expression="%s = %s" % (total_cost, listcost)
    )
    return total_cost


def net_revenues(
    opts, technical_bioenergy, tech_bioC, tech_bioHF, total_revenues, total_costs
):
    pid = os.getpid()
    ...
    econ_bioenergyC = (
        opts["econ_bioenergyc"]
        if opts["econ_bioenergyc"]
        else "tmprgreen_%i_econ_bioenergyc" % pid
    )
    econ_bioenergyHF = (
        opts["econ_bioenergyhf"]
        if opts["econ_bioenergyhf"]
    ...
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression="%s = %s - %s" % (net_revenues, total_revenues, total_costs),
    )
    positive_net_revenues = "tmprgreen_%i_positive_net_revenues" % pid
    run_command(
        "r.mapcalc",
        overwrite=True,
        expression=("%s = if(%s<=0,0,1)" % (positive_net_revenues, net_revenues)),
    )
    ...
    r.mapcalc(
        expr.format(
            econ_bioenergy=econ_bioenergyHF,
    ...
    r.mapcalc(
        expr.format(
            econ_bioenergy=econ_bioenergyC,
            economic_surface=economic_surface,
            tech_bio=tech_bioC,
        ),
        overwrite=True,
