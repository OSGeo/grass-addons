              ...
              double *mx)
{
    ...
    int xp = 0, yp = 0, x1, y1, btn = 0, s, l0, t0, xpl, ypt, u_w = 0, u_l = 0,
        u_w0, u_l0, w_w = 0, w_l = 0, initl = 0, initt = 0, d, fmask, j,
        circle = 0;
    register int i;
...
keyboard:
    ...
    if (d == 1 || d == 2) {
        ...
        if (d == 1) {
            ...
            back:
                fprintf(stderr, "\n    Enter number of COLUMNS & ROWS for the "
                                "dimensions of");
                ...
                if (!u_w0 || !u_l0) {
                    fprintf(stderr, "\n    You entered a dimension as 0; enter "
                                    "dimensions again\n");

                    goto back;
                }
                else if (u_w == 1 && u_l == 1) {
                    fprintf(stderr, "\n    You entered dimensions as 1 1; This "
                                    "will not produce");
                    fprintf(stderr, "\n       meaningful results; enter larger "
                                    "dimensions\n");

                    goto back;
                }
                else if (u_w >= r || u_l >= b) {
                    fprintf(stderr, "\n    Window size you chose allows < 2 "
                                    "windows across each row;");
                    fprintf(stderr,
        ...
        /* setup the moving window using the mouse */
            ...
            fprintf(stderr, "\n\n    Do you want to use a rectangular "
                            "(including squares) (y)");

            if (!G_yes("\n       or circular (n) moving window?   ", 1)) {
                circle = 1;
                fprintf(stderr, "\n    Draw a rectangular area to contain a "
                                "circular moving window.");
                fprintf(stderr, "\n    First select upper left corner, then "
                                "lower right:\n");
                fprintf(stderr, "       Left button:     Check unit size\n");
                fprintf(
                    stderr,
                    "       Middle button:   Upper left corner of area here\n");
                fprintf(stderr, "       Right button:    Lower right corner of "
                                "area here\n");
            }
            else {
                circle = 0;
                ...
                fprintf(stderr, "       Middle button:   Upper left corner of "
                                "window here\n");
                fprintf(stderr, "       Right: button:   Lower right corner of "
                                "window here\n");
            }
                ...
                if (btn == 1) { /** show the size and ratio **/
                    ...
                    ypt =
                        (int)((double)((int)((double)(yp - t) * mx[1] + 0.5)) /
                              mx[1]);
                    if (xpl < 0 || ypt < 0) {
                        fprintf(stderr, "\n    You did not put lower right "
                                        "corner below and to the");
                        fprintf(stderr, "\n       of upper left corner. Please "
                                        "select lower right");
                        fprintf(stderr, "\n       corner again");
                        ...
                        if (circle) {
                            ...
                            draw_circle(initl, initt, initl + xpl, initt + ypt,
                                        3);
                        }
                        else
                        ...
                        if (circle) {
                            ...
                            fprintf(stderr,
                                    "\n\n    Circular moving window has radius "
                                    "= %5.2f pixels\n",
                                    radius);
                        }
                        else {
                            fprintf(stderr,
                                    "\n    Rectangular moving window has %d "
                                    "columns and %d rows",
                                    u_w, u_l);
                            fprintf(stderr,
                                    "\n    with width/length ratio of %5.2f "
                                    "and area of %d pixels\n",
                                    (float)u_w / u_l, u_w * u_l);
                        }
                    }
                }
