...
static void draw_grid(int l, int t, int w_w, int w_l, int h_d, int v_d,
                      int starty, int startx, double colratio, double rowratio);
static void man_unit(int t, int b, int l, int r, char *n1, char *n2, char *n3,
                     double *mx, int fmask);
static void get_rd(int exp1, int exp2, int dx, int dy, int u_w, int u_l, int *l,
                   int *t);
...
static int calc_num(int w_w, int w_l, double ratio, int u_w, int u_l,
                    ...
                    int count);
static void graph_unit(int t, int b, int l, int r, char *n1, char *n2, char *n3,
                       double *mx, int fmask);
...
static void man_unit(int t, int b, int l, int r, char *n1, char *n2, char *n3,
                     double *mx, int fmask)
{
    int i, j, dx, dy, w_w, w_l, u_w, u_l, method, l0, t0,
        randflag = 0, unit_num, num = 0, scales, h_d = 1, v_d = 1, itmp, thick,
        sites, *row_buf, fr, k, count = 0, maxsize = 0, nx = 0, ny = 0,
        numx = 0, numy = 0, al = 0, ar = 0, at = 0, ab = 0, au_w = 0, au_l = 0;
    double *ux, *uy;
    FILE *fp;
    ...
    for (i = 0; i < scales; i++) {
        for (;;) {
            ...
            strata:
                ...
                start[0] = start[0] - 1.0;
                start[1] = start[1] - 1.0;
                if (start[0] > w_l || start[0] < 0 || start[1] > w_w ||
                    start[1] < 0) {
                    fprintf(stderr, "\n    The starting row and col you "
                                    "entered are outside");
            ...
            if (!G_yes("\n       (including squares) (y) or circles (n)?   ",
                       1)) {
            getradius:
                fprintf(
                    stderr,
            ...
            getratio:
                ...
                if (ratio < 0)
                    ...
                    if (!G_yes("\n    Are you sure you want such a large "
                               "ratio?   ",
                               1))
                        goto getratio;
            ...
            tryagain:
                if (method == 1) {
                    ...
                    if (fmask > 0) {
                        ...
                        maxsize =
                            ((cnt * dtmp / 2) * (cnt * dtmp / 2) > 1.0 / dtmp)
                                ...
                                : 1.0 / dtmp;
                        fprintf(stderr,
                                "\n    Recommended maximum SIZE is %d in %d "
                                "cell total",
                                maxsize, count);
                        fprintf(stderr, " area\n");
                    }
                        ...
                        fprintf(stderr, " %d in %d pixel total area\n",
                                (int)((w_l - (int)(start[0])) *
                                      (w_w - (int)(start[1])) * dtmp / 2),
                                (w_l - (int)(start[0])) *
                                    (w_w - (int)(start[1])));

                        count =
                            (w_l - (int)(start[0])) * (w_w - (int)(start[1]));
                        maxsize = (int)((w_l - (int)(start[0])) *
                                        (w_w - (int)(start[1])) * dtmp / 2);
                    }
                }
                    ...
                    fprintf(stderr,
                            "\n    Recommended maximum SIZE is %d in %d pixel "
                            "total",
                            (int)((w_l - (int)(start[0])) *
                                  (w_w - (int)(start[1])) * dtmp / 2),
                            (w_l - (int)(start[0])) * (w_w - (int)(start[1])));
                ...
                fprintf(stderr,
                        "    What size (in pixels) for each sampling unit of "
                        "scale %d?  ",
                        i + 1);
                ...
                u_l = sqrt(size / ratio);
                fprintf(stderr,
                        "\n    The nearest size is %d cells wide X %d cells "
                        "high = %d",
                        u_w, u_l, u_w * u_l);
                fprintf(stderr, " cells\n");

                if (!u_w || !u_l) {
                    fprintf(stderr, "\n    0 cells wide or high is not "
            ...
            if (method == 3) {
                fprintf(stderr, "\n    The interval, in pixels, between the "
                                "units of scale");
            ...
            if (u_w + intv > w_w / h_d || u_l + intv > w_l / v_d) {
                fprintf(stderr, "\n    Unit size too large for sampling frame; "
                                "try again\n");
            ...
            /* for the other distributions, calculate the
               ...
               number of units */
                ...
                if (method == 1) {
                    ...
                    if (((w_w - (int)(start[1])) %
                         (numx * (u_w + (int)(intv)))) >= u_w)
                        numx++;
                    if (((w_l - (int)(start[0])) %
                         (numy * (u_l + (int)(intv)))) >= u_l)
                        numy++;
                    unit_num = numx * numy;
                    fprintf(
                        stderr,
                ...
                do {
                    fprintf(stderr, "\n    What NUMBER of sampling units do "
                                    "you want to try");
                    ...
                    if ((num = dtmp) > unit_num || num < 1) {
                        ...
                        fprintf(stderr, "\n    How many sampling units do you "
                                        "want per row?  ");

                        numtrap(1, &dtmp);
                        if ((nx = dtmp) > num) {
                            fprintf(stderr, "\n    Number in each row > number "
                                            "requested; try");
            ...
            if (!calc_unit_loc(radius, t, b, l, r, ratio, u_w, u_l, method,
                               intv, num, h_d, v_d, ux, uy, &sites,
                               (int)(start[1]), (int)(start[0]), fmask, nx,
                               mx[0], mx[1]))
                goto last;
                      ...
                      int starty, int startx, double colratio, double rowratio)
{
    ...
    if (startx > 0) {
        dx = (int)((double)((int)(colratio * ((int)((double)(w_w - startx) /
                                                    (double)(h_d)))) /
                                colratio +
                            0.5));
        l0 = l + startx;
        ...
        dx = (int)((
            double)((int)(colratio * ((int)((double)(w_w) / (double)(h_d)))) /
                        colratio +
                    0.5));
        l0 = l;
    }
    if (starty > 0) {
        dy = (int)((double)((int)(rowratio * ((int)((double)(w_l - starty) /
                                                    (double)(v_d)))) /
                                rowratio +
                            0.5));
        ...
        dy = (int)((
            double)((int)(rowratio * ((int)((double)(w_l) / (double)(v_d)))) /
                        rowratio +
                    0.5));
        t0 = t;
    }
                         ...
                         int fmask, double nx, double x, double y)
{
    ...
    if (method == 1 || method == 4) {
        ...
        for (i = 0; i < num; i++) {
            ...
            back:
                ...
                lap = 0;
                for (j = 0; j < cnt; j++) {
                    if (overlap(l + left1, t + top1, (int)ux[j], (int)uy[j],
                                u_w, u_l))
                        lap = 1;
                }
                if (lap)
    ...
    /* for centered over sites */
        ...
        sites_mapset = G_ask_vector_old("    Enter name of vector points map",
                                        sites_file_name);
        ...
        *sites = 0;
        ...
        while (1) {
            ...
            if (ltype == -2)
                break; /* EOF */
            /* point features only. (GV_POINTS is pts AND centroids, GV_POINT is
             * just pts) */
            if (!(ltype & GV_POINT))
                continue;
            ...
            if (ulcol <= left || ulrow <= top || ulcol + u_w - 1 > right ||
                ulrow + u_l - 1 > bot) {
                fprintf(stderr,
                        "    No sampling unit over site %d at east=%8.1f "
                        "north=%8.1f\n",
                       ...
                       double *mx, int fmask)
{
    int x0 = 0, y0 = 0, xp, yp, ux[250], uy[250], u_w, u_l, btn = 0, k = 0,
        w_w = 0, w_l = 0, *row_buf, at, ab, al, ar, circle = 0, tmpw, tmpl,
        au_w, au_l, lap = 0, l0 = 0, r0 = 0, t0 = 0, b0 = 0;
    FILE *fp;
        ...
        back1:
            ...
            if (btn == 1) {
                if (ar > r || ab > b || ar < l || ab < t) {
                    fprintf(stderr, "\n    This point is not in the sampling "
                                    "frame; try again\n");

                    goto back1;
                }
                if (x0 < l || y0 < t) {
                    fprintf(stderr, "\n    Use the middle button to first put "
                                    "the upper left");
            ...
            /* mid button, move the start point of the
               rubber box */
                ...
                if (ar > r || ab > b || ar < l || ab < t) {
                    fprintf(stderr, "\n    Point is not in the sampling frame; "
                                    "try again\n");
            ...
            /* right button, outline the unit */
                ...
                if (ar > r || ab > b || al < l || at < t) {
                    fprintf(stderr, "\n    The unit extends outside the "
                                    "sampling frame or map;");
                    fprintf(stderr, "\n       try again\n");
                ...
                if (xp - x0 > 0 && yp - y0 > 0) {
                    ...
                    fprintf(stderr, "\n       and to the right of the upper "
                                    "left corner. Please try again");
