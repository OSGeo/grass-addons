...
static void set_frame(double *msc, int *t, int *b, int *l, int *r)
{
    ...
    if (G_yes("\n    Will the sampling frame (total area within which "
...
static void ppoint(double *m, int x, int y, int num)
{
    ...
    if (num < 0) {
        ...
        if (num > 0) {
            fprintf(stderr, "    Point %d is at Row %5d and Col %5d", num,
                    (int)(y * m[1]), (int)(x * m[0]));
        }
        else if (num == 0) {
...
static void pbutton(int opt)
{
    ...
    fprintf(stderr, "       Left button:     What are row & column coordinates "
                    "at this point?\n");
...
static void print_hd(FILE *mapfile, struct Cell_head *universe)
{
    ...
    fprintf(mapfile, "BOUND     %10.2f %10.2f %10.2f %10.2f\n",
            universe->ns_res, universe->ew_res, universe->south,
            universe->west);
