...
int get_group_drv(char **sel)
{
    ...
    if (sel[2][0] == 'x') {
        ...
        fprintf(stderr, "\n    The shape (P/A) classes are saved in "
                        "\"r.le.para/shape_PA\".");
        G_sleep(2);
    }
    if (sel[3][0] == 'x') {
        ask_limits("shape_CPA", "    SHAPE CLASSES: CORRECTED PERIMETER/AREA");
        fprintf(stderr, "\n    The shape (CPA) classes are saved in "
                        "\"r.le.para/shape_CPA\".");
        G_sleep(2);
    }
    if (sel[4][0] == 'x') {
        ...
        fprintf(stderr, "\n    The shape (RCC) classes are saved in "
                        "\"r.le.para/shape_RCC\".");
        G_sleep(2);
    }
    if (sel[5][0] == 'x') {
        ask_limits("dist_cc", "    DISTANCE CLASSES: CENTER-CENTER");
        fprintf(stderr, "\n    The distance (CC) classes are saved in "
                        "\"r.le.para/dist_cc\".");
        G_sleep(2);
    }
    if (sel[6][0] == 'x') {
        ask_limits("dist_ce", "    DISTANCE CLASSES: CENTER-EDGE");
        fprintf(stderr, "\n    The distance (CE) classes are saved in "
                        "\"r.le.para/dist_ce\".");
        G_sleep(2);
    }
    if (sel[7][0] == 'x') {
        ask_limits("dist_ee", "    DISTANCE CLASSES: EDGE-EDGE");
        fprintf(stderr, "\n    The distance (EE) classes are saved in "
                        "\"r.le.para/dist_ee\".");
        G_sleep(2);
    }
    if (sel[8][0] == 'x') {
...
static void ask_fromto(void)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        fputs(buf, fp);
        if (i == 0)
            fprintf(stderr, "\n\n  Please enter \"TO\" attribute group "
                            "followed by \"0 end\"\n");
    }
    fclose(fp);
    G_free(buf);
