/*******************************************************************************
r.sun.mp: This program was writen by Jaro Hofierka in Summer 1993 and
...
int main(int argc, char *argv[])
{
    ...
    struct {
        struct Option *elevin, *aspin, *aspect, *slopein, *slope, *linkein,
            ...
            *step, *declin, *ltime, *dist, *horizon, *horizonstep,
...
int INPUT_part(int offset, double *zmax)
{
    ...
    if (longin != NULL) {
        ...
        if (latitudeArray == NULL) {
            ...
            // for (l = 0; l < numRows; l++)
            //        longitudeArray[l] = (float *)G_malloc(sizeof(float) *
            //        (n));
        }
    ...
    if (useHorizonData()) {
        if (horizonarray == NULL) {
            ...
            //             horizonarray =
            //                 (unsigned char *)G_malloc(sizeof(char) *
            //                 arrayNumInt *
            //                                           numRows * n);
            //
            //             horizonbuf = (FCELL **) G_malloc(sizeof(FCELL *) *
            // arrayNumInt);             fd_shad = (int *)G_malloc(sizeof(int) *
            // arrayNumInt);
        }
        /*
         * if(ttime != NULL)
...
#pragma omp for schedule(dynamic)
            for (i = 0; i < n; i++) {
                ...
                if (i >= n || arrayOffset >= m) {
                    G_message("i: %d, n: %d, arrayOffset: %d, m: %d", i, n,
                              arrayOffset, m);
                }
                ...
                if (sunVarGeom.z_orig != UNDEFZ) {
                    ...
                    if ((G_projection() != PROJECTION_LL)) {
                        ...
                        if (latin == NULL || longin == NULL) {
                            ...
                            if (pj_do_proj(&longitude, &latitude, &iproj,
                                           &oproj) < 0) {
    ...
    Rast_append_format_history(
        &hist, " Solar constant (W/m^2):                   1367");
