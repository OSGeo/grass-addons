...
class TestForestFrag(TestCase):
    ...
    def setUp(self):
        ...
        gr.mapcalc("{f} = if({c} == 5, 1, 0)".format(c=self.landclass, f=self.forest))
    ...
    def test_3x3(self):
        self.assertModule(
            "r.forestfrag", input=self.forest, output=self.forest_frag, window=3
        )
        ...
        self.assertRasterFitsInfo(
            raster=self.forest_frag, reference=ref, precision=0.0006
        )
        ref = dict(
            n=249323,
            null_cells=1,
