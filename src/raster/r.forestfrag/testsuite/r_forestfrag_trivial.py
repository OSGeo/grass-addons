...
class TestForestFragTrivial(TestCase):
    ...
    def forest_frag_general(self, forest, window, reference):
        ...
        self.assertRasterMinMax(self.forest, refmin=0, refmax=1)
        self.runModule(
        ...
        self.runModule("r.in.ascii", input="-", stdin_=PF_RIITTERS, output=pf_ref)
        self.to_remove.append(pf_ref)
        pff_ref = self.forest_frag_ref + "_pff"
        self.runModule("r.in.ascii", input="-", stdin_=PFF_RIITTERS, output=pff_ref)
        ...
        self.assertRastersNoDifference(actual=pf, reference=pf_ref, precision=0.01)

        self.assertRastersNoDifference(actual=pff, reference=pff_ref, precision=0.01)
