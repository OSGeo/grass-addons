...
void show_patch(char *fn, double *msc, char *cmd)
{
    ...
    if (G_yes("\n\nShow data for some patches in sequence (y)\
             \nor show data for all patches (n)? ",
              1)) {
        tmp = patch_list;
        while (tmp) {
...
skip:
    if (!fp)
        return;
    if (p->num == 1) {
        fprintf(fp, "Patch         Patch   Patch   Patch     ----Shape "
                    "Index----  Twist   Omega\n");
        fprintf(fp, "Number        Attr.   Area    Perim.    P/A    CP/A     "
                    "RCC  Number  Index\n");
    }
    fprintf(fp, "%6d %12.3f %7.0f %7.0f %7.3f %7.3f %7.3f  %6d %6.3f\n", p->num,
            p->att, p->area, p->perim, shp1, shp2, shp3, p->twist, p->omega);
}
                   ...
                   int index)
{
    ...
    /*
       ...
       throughout the program null_buf =      pointer to array containing 0.0 if
       pixel in input raster map is not null and 1.0 if pixel in input raster
       map is null infd =          pointer to file with map of patches i, j =
       counts the row and column when going through arrays p = counts the number
       of measures in fd =            pointer to file to hold patch number map
               ...
               int ncols, int index, int *centernull)
{
    ...
    /*
       ...
       clipped, if there's a region map centernull = 1 if the center pixel of
       the clipped area is a null value, 0 otherwise INTERNAL: tmp = pointer to
       a temporary buffer to store a row of a CELL_TYPE (integer) raster ftmp =
       pointer to a temporary buffer to store a row of an FCELL_TYPE (floating
       point) raster dtmp =       pointer to a temporary buffer to store a row
       of a DCELL_TYPE (double) raster tmp1 =       pointer to a temporary
       buffer to store a row of the region map tmpname =    one of the above:
       tmp, ftmp, dtmp fr =         return value from attempting to open the
       region map i, j =       indices to rows and cols of the arrays center_row
       = row of the center of the circle, if circles used center_col = column of
       the center of the circle, if circles used dist =       used to measure
       distance from a row/column to the center of the circle, to see if a
       row/column is within the circle data_type =  the type of raster map:
       integer, floating point, or double rastptr =    void pointer used to
       advance through null values in the tmp, ftmp, or dtmp buffers
              ...
              DCELL **buf, DCELL **null_buf, PATCH *p_list, CELL **pat)
{
    int i = row0, j = col0, pts = 0, di = 0, dj = -1, not_done, k, m, tmp,
        lng = 0, roww = 0, rowe = 0, coln = 0, cols = 0, row1, col1, di2, dj2,
        p, q, area, per, nozero;
    ...
    /*
       ...
       map of patch numbers; this map can only be integer INTERNAL i, j = counts
       the row and column as tracing occurs pts =         counts the number of
       points as tracing proceeds di, dj =      indicates moves of 1 pixel in
       the row or column direction, used to examine adjoining 4- or
       8-neighboring pixels when tracing not_done =    flag to indicate when the
       patch has not been fully traced yet k = m = tmp =         integer to hold
       intermediate results in calculation of long axis of patch lng = integer
       to hold the calculated long axis of the patch roww =        integer to
       hold the westernmost point in each row of the patch rowe =        integer
       to hold the easternmost point in each row of the patch coln = integer to
       ...
       perimeter nozero = twist2 =      2-dimensional array to hold preliminary
       countes needed to calculate the twist number and omega index trows =
       number of rows in the bounding box for the patch tcols =       number of
       columns in the bounding box for the patch n, e =        northing (in
       rows) and easting (in columns) for the patch a, b =        indexes
       particular pixels used in calculating twist number in certain cases
