...
%8.0f %8.0f %8.0f %6.3f %6.3f %6.3f %8d %4.3f\n",
                    n_scale, n_unit, tmp->num, tmp->att, tmp->c_row, tmp->c_col,
                    tmp->area, tmp->core, tmp->edge, tmp->perim,
                    tmp->perim / tmp->area,
                    0.282 * tmp->perim / sqrt(tmp->area),
...
{
    static double sumx = 0.0, sumx2 = 0.0, w_att = 0.0, w_att2 = 0.0,
                  total = 0.0;
...
{
    static int **density1c = NULL, **density1e = NULL, *densityc = NULL,
               *densitye = NULL, first = 1;
               ...
               int *density, int *type_dens, int **density1)
{
    ...
    if (choice->size2) {
        /*     if(!(fp = fopen("r.le.out/s6.out", "a")))
           G_fatal_error("Can't write file s6.out; do you have write
           permission?\n"); fprintf(fp, "%5d %5d\n", n_scale, n_unit);     */
        for (i = 0; i < ntype; i++) {
...
void df_shape(PATCH *tmp, int type_coh, int *type_dens)
{
    ...
    if (!tmp->next) { /** if all the patches are done **/
        save_shape(sq1, sq2, sq3, sqr11, sqr21, sqr31, mshape, mshape_p,
                   mshape_r, mshape1, mshape1_p, mshape1_r, type_dens, den1,
...
void m_size(PATCH *tmp, double **value, int index)
{
    ...
    if (choice->size[1] || choice->size[2] || choice->size[7] ||
        choice->size[8]) {
        sum1 += tmp->area;
...
void m_core(PATCH *tmp, double **value, int index)
{
    ...
    if (choice->core[5] || choice->core[6] || choice->core[7] ||
        choice->core[8])
        if (in_group(tmp->att, recl_tb[0], 0)) {
