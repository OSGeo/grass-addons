...
void open_files()
{
    ...
    if (choice->att[1] || choice->att[2] || choice->att[3] || choice->att[4]) {
        ...
        fprintf(fp, "Scale  Unit  MN. PIXEL ATT.   S.D. PIXEL ATT.  MN. PATCH "
                    "ATT.   S.D. PATCH ATT.\n");
        fclose(fp);
    }
    ...
    if (choice->size[6]) {
        ...
        fprintf(fp, "               NUMBER OF PATCHES BY SIZE CLASS BY GROUP "
                    "-- in pixels\n");
        fprintf(fp, "Scale  Unit ");
        for (i = 0; i < size_cl_no; i++)
            fprintf(fp, "   Class[%2d]", i + 1);
    ...
    if (choice->core[1] || choice->core[2] || choice->core[3] ||
        choice->core[4]) {
        fp = fopen0("r.le.out/c1-4.out", "w");
        fprintf(fp, "Scale  Unit  MEAN CORE SIZE   S.D. CORE SIZE   MEAN EDGE "
                    "SIZE   S.D. EDGE SIZE\n");
        fclose(fp);
    }
    ...
    if (choice->core[9]) {
        ...
        fprintf(fp, "               NUMBER OF PATCH CORES BY SIZE CLASS -- in "
                    "pixels\n");
        fprintf(fp, "Scale  Unit ");
        for (i = 0; i < size_cl_no; i++)
            fprintf(fp, "   Class[%2d]", i + 1);
        ...
        fprintf(fp, "               NUMBER OF PATCH EDGES BY SIZE CLASS -- in "
                    "pixels\n");
        fprintf(fp, "Scale  Unit ");
        for (i = 0; i < size_cl_no; i++)
            fprintf(fp, "   Class[%2d]", i + 1);
    ...
    if (choice->core[10]) {
        ...
        fprintf(fp, "               NUMBER OF PATCH CORES BY SIZE CLASS BY "
                    "GROUP -- in pixels\n");
        fprintf(fp, "Scale  Unit ");
        for (i = 0; i < size_cl_no; i++)
            fprintf(fp, "   Class[%2d]", i + 1);
        ...
        fprintf(fp, "               NUMBER OF PATCH EDGES BY SIZE CLASS BY "
                    "GROUP -- in pixels\n");
        fprintf(fp, "Scale  Unit ");
        for (i = 0; i < size_cl_no; i++)
            fprintf(fp, "   Class[%2d]", i + 1);
    ...
    if (choice->boundary[1] || choice->boundary[2] || choice->boundary[3] ||
        choice->boundary[4]) {
        fp = fopen0("r.le.out/n1-4.out", "w");
        fprintf(fp, "Scale  Unit  MEAN TWIST NUM.  SD. TWIST NUM.   MN. OMEGA "
                    "INDEX  SD. OMEGA INDEX\n");
        fclose(fp);
    }

    if (choice->perim[1] || choice->perim[2] || choice->perim[3]) {
        fp = fopen0("r.le.out/p1-3.out", "w");
        fprintf(fp, "Scale  Unit  SUM PERIMETER    MEAN PERIMETER   S.D. "
                    "PERIMETER -- in pixels\n");
        fclose(fp);
    }
...
void mv_driver()
{
    ...
    int a1, a2, a3, a4, a5, a6, a7, a8, s1, s2, s3, s4, s5, s6, s7, s8, c1, c2,
        c3, c4, c5, c6, c7, c8, c9, c10, h1, h2, h3, h4, h5, h6, p1, p2, p3, p4,
        p5, p6, n1, n2, n3, n4, b;
    long finished_time;
    float radius;
    ...
    fprintf(stderr, "   warning may be printed or appear in a window; ignore "
                    "this warning.\n");
    ...
    for (i = 0; i < nr; i++) {
        ...
        if (fmask > 0) {
            ...
            Rast_get_row_nomask(fmask, row_buf, y0 + i + u_l / 2, CELL_TYPE);
...
void meter(int n, int i, int div)
{
    ...
    if ((n - i) % d == 0) {
        ...
        fprintf(stdout, "WINDOWS LEFT  = %8d   EST. COMPLETION = %s\r", (n - i),
                done);
        fflush(stdout);
        k++;
    }
...
void get_para()
{
    ...
    /*
       ...
       para3 = set to 1 if perimeter-area shape index classes are chosen
       para4 = set to 1 if corrected perimeter-area shape index classes are
       chosen para5 = set to 1 if related circumscribing circle shape index
       classes are chosen
     */
    ...
    if (choice->att[5] || choice->att[6] || choice->size[3] ||
        choice->size[4] || choice->size[6] || choice->core[5] ||
        choice->core[6] || choice->core[7] || choice->core[8] ||
        choice->core[10] || choice->shape[3] || choice->shape[4] ||
        choice->shape[6] || choice->perim[4] || choice->perim[5] ||
        choice->perim[6]) {
        ...
        if (!ntype) {
            ...
            fprintf(
                ...
                "   "
                "********************************************************\n");
            fprintf(stderr, "    The attribute group file (r.le.para/recl_tb) "
                            "seems to  \n");
            fprintf(stderr, "    be incorrect as no attribute groups were "
                            "found.  Check \n");
        ...
        if (radius) {
            ...
            for (i = top + 1; i < top + 1 + u_l; i++) {
                for (j = left + 1; j < left + 1 + u_w; j++) {
                    dist = sqrt(
                        ((double)i - center_row) * ((double)i - center_row) +
