...
int main(int argc, char **argv)
{
    ...
    struct Option *map_dem, *map_slope, *map_aspect, *par_window, *par_method,
        *par_differnce, *map_output;
    ...
    module->description = _("Calculate convergence index.");
    ...
    {
        FCOLORS fcolr[7] = {{-100, 56, 0, 0}, {-70, 128, 0, 0},
                            {-50, 255, 0, 0}, {0, 255, 255, 255},
                            {50, 0, 0, 255},  {70, 0, 0, 128},
                            {100, 0, 0, 56}};

        Rast_init_colors(&colors);
        for (i = 0; i < 6; ++i)
            Rast_add_d_color_rule(&fcolr[i].cat, fcolr[i].r, fcolr[i].g,
                                  fcolr[i].b, &fcolr[i + 1].cat, fcolr[i + 1].r,
                                  fcolr[i + 1].g, fcolr[i + 1].b, &colors);
