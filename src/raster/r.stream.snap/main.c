...
int main(int argc, char *argv[])
{
    ...
    struct Option *in_points_opt, *out_points_opt, *in_stream_opt,
        *in_accum_opt, *opt_accum_treshold, *opt_distance_treshold,
        *opt_swapsize;
    ...
    module->description = _("Input can be stream network, point vector map "
    ...
    if (!in_stream_opt->answer && !in_accum_opt->answer)
        G_fatal_error(_(
            "At least one of accumulation or streams raster maps is required"));
