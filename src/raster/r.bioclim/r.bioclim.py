...
def main():
    ...
    if len(tmin.split(",")) != 12:
        gs.fatal(_("12 maps with minimum temperatures are required"))
    if len(tmax.split(",")) != 12:
        gs.fatal(_("12 maps with maximum temperatures are required"))
    if tavg and len(tavg.split(",")) != 12:
        gs.fatal(_("12 maps with average temperatures are required"))
    if prec:
        if len(prec.split(",")) != 12:
            gs.fatal(_("12 maps with precipitation are required"))

    tinscale = int(tinscale)
    if tinscale <= 0:
        gs.fatal(_("Input temperature scale must be positive"))
    toutscale = int(toutscale)
    if toutscale <= 0:
        gs.fatal(_("Output temperature scale must be positive"))
    ...
    if not tavg:
        ...
        gs.message(_("Calculating monthly averages from min and max"))
        e = "$ta = ($tmax + $tmin) / 2.0"
        if workers > 1:
            ...
            for i in range(12):
                ...
                gs.mapcalc(e, ta=ta, tmax=tmaxl[i], tmin=tminl[i])

        tavg = gs.read_command(
            "g.list",
            quiet=True,
            type="raster",
    ...
    gs.message(_("BIO1 = Annual Mean Temperature ..."))
    output = outpre + "bio01." + str(pid)
    gs.run_command("r.series", input=tavg, output=output, method="average")
    gs.mapcalc(
    ...
    gs.run_command("r.support", map=outpre + "bio01", history=os.environ["CMDLINE"])
    gs.run_command("g.remove", flags="f", type="raster", name=output, quiet=True)

    # BIO2 = Mean Diurnal Range (Mean of monthly (max temp - min temp))
    gs.message(_("BIO2 = Mean Diurnal Range ..."))
    e = "$tr = $tmax - $tmin"
    if workers > 1:
        for i in range(0, 12, workers):
            jend = 12 - i
            ...
            for j in range(jend):
                ...
                ps[j] = gs.mapcalc_start(e, tr=tr, tmax=tmaxl[i + j], tmin=tminl[i + j])
        ...
        for i in range(12):
            ...
            gs.mapcalc(e, tr=tr, tmax=tmaxl[i], tmin=tminl[i])
    ...
    gs.run_command("r.series", input=tr, output=output, method="average")
    gs.mapcalc(
        "$bio = round(double($oscale) * $input / $iscale)",
        bio=outpre + "bio02",
        oscale=toutscale,
    ...
    gs.run_command("r.support", map=outpre + "bio02", history=os.environ["CMDLINE"])
    gs.run_command("g.remove", flags="f", type="raster", name=output, quiet=True)
    gs.run_command(
        "g.remove",
        flags="f",
    ...
    gs.message(_("BIO4 = Temperature Seasonality ..."))
    output = outpre + "bio04." + str(pid)
    gs.run_command("r.series", input=tavg, output=output, method="stddev")
    gs.mapcalc(
    ...
    gs.run_command("r.support", map=outpre + "bio04", history=os.environ["CMDLINE"])
    gs.run_command("g.remove", flags="f", type="raster", name=output, quiet=True)

    # BIO5 = Max Temperature of Warmest Month
    gs.message(_("BIO5 = Max Temperature of Warmest Month ..."))
    output = outpre + "bio05." + str(pid)
    gs.run_command("r.series", input=tmax, output=output, method="maximum")
    gs.mapcalc(
    ...
    gs.run_command("r.support", map=outpre + "bio05", history=os.environ["CMDLINE"])
    gs.run_command("g.remove", flags="f", type="raster", name=output, quiet=True)

    # BIO6 = Min Temperature of Coldest Month
    gs.message(_("BIO6 = Min Temperature of Coldest Month ..."))
    output = outpre + "bio06." + str(pid)
    gs.run_command("r.series", input=tmin, output=output, method="minimum")
    gs.mapcalc(
        "$bio = round(double($oscale) * $biotmp / $iscale)",
        bio=outpre + "bio06",
        oscale=toutscale,
    ...
    gs.run_command("r.support", map=outpre + "bio06", history=os.environ["CMDLINE"])
    gs.run_command("g.remove", flags="f", type="raster", name=output, quiet=True)

    # BIO7 = Temperature Annual Range (BIO5-BIO6)
    gs.message(_("BIO7 = Temperature Annual Range ..."))
    gs.mapcalc(
        "$bio = $bio5 - $bio6",
        bio=outpre + "bio07",
        bio5=outpre + "bio05",
    ...
    gs.run_command("r.support", map=outpre + "bio07", history=os.environ["CMDLINE"])

    # BIO3 = Isothermality (BIO2/BIO7) (* 100)
    gs.message(_("BIO3 = Isothermality (BIO2/BIO7) ..."))
    gs.mapcalc(
    ...
    gs.run_command("r.support", map=outpre + "bio03", history=os.environ["CMDLINE"])

    # mean of mean for each quarter year
    gs.message(_("Mean temperature for each quarter year ..."))
    ...
    gs.message(_("BIO10 = Mean Temperature of Warmest Quarter,"))
    gs.message(_("BIO11 = Mean Temperature of Coldest Quarter ..."))

    tavgq = gs.read_command(
        "g.list",
        quiet=True,
        type="raster",
    ...
    gs.run_command("r.support", map=outpre + "bio10", history=os.environ["CMDLINE"])
    gs.mapcalc(
        "$bio = round(double($oscale) * $biotmp / $iscale)",
        bio=outpre + "bio11",
        oscale=toutscale,
    ...
    gs.run_command("r.support", map=outpre + "bio11", history=os.environ["CMDLINE"])
    ...
    gs.message(_("Precipitation for each quarter year ..."))
    for i in range(quartals):
    ...
    gs.message(_("BIO8 = Mean Temperature of Wettest Quarter ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio08", history=os.environ["CMDLINE"])

    # BIO9 = Mean Temperature of Driest Quarter
    gs.message(_("BIO9 = Mean Temperature of Driest Quarter ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio09", history=os.environ["CMDLINE"])

    # BIO12 = Annual Precipitation
    gs.message(_("BIO12 = Annual Precipitation ..."))
    output = outpre + "bio12"
    gs.run_command("r.series", input=prec, output=output, method="sum")
    gs.run_command(
        "r.support",
        map=outpre + "bio12",
        description="BIOCLIM12: Generated by r.bioclim",
    )
    gs.run_command("r.support", map=outpre + "bio12", history=os.environ["CMDLINE"])

    # BIO13 = Precipitation of Wettest Month
    # BIO14 = Precipitation of Driest Month
    gs.message(_("BIO13 = Precipitation of Wettest Month,"))
    gs.message(_("BIO14 = Precipitation of Driest Month ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio13", history=os.environ["CMDLINE"])
    gs.run_command(
        "r.support",
        map=outpre + "bio14",
        description="BIOCLIM14: Generated by r.bioclim",
    )
    gs.run_command("r.support", map=outpre + "bio14", history=os.environ["CMDLINE"])

    # BIO15 = Precipitation Seasonality (Coefficient of Variation)
    gs.message(_("BIO15 = Precipitation Seasonality ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio15", history=os.environ["CMDLINE"])

    # BIO16 = Precipitation of Wettest Quarter
    # BIO17 = Precipitation of Driest Quarter
    gs.message(_("BIO16 = Precipitation of Wettest Quarter,"))
    gs.message(_("BIO17 = Precipitation of Driest Quarter ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio16", history=os.environ["CMDLINE"])
    gs.run_command(
        "r.support",
        map=outpre + "bio17",
        description="BIOCLIM17: Generated by r.bioclim",
    )
    gs.run_command("r.support", map=outpre + "bio17", history=os.environ["CMDLINE"])

    precql = precq.split(",")

    # BIO18 = Precipitation of Warmest Quarter
    gs.message(_("BIO18 = Precipitation of Warmest Quarter ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio18", history=os.environ["CMDLINE"])

    # BIO19 = Precipitation of Coldest Quarter
    gs.message(_("BIO19 = Precipitation of Coldest Quarter ..."))
    ...
    gs.run_command("r.support", map=outpre + "bio19", history=os.environ["CMDLINE"])
