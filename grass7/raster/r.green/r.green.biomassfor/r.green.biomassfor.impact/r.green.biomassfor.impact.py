...
def fertility_maintenance(opts, flgs):
    ...
    expr_prodT = (
        "site_prod_bioenergyT = if("
        + treatment
        + "==2 && ("
        + opts["soilp_map"]
        + "==1 || "
        + opts["soilp_map"]
        + "==2), analysis_surface*yield_pix*%f*0+yield_pix*%f, if("
        + treatment
        + "==2 && "
        + opts["soilp_map"]
        + ">2, analysis_surface*yield_pix*%f*0.7+yield_pix*%f))"
    )
...
def soil_water_protection(opts, flgs):
    ...
    expr_sw6 = (
        "S_W_prot_bioenergy6 = if("
        + treatment
        + "==2 && "
        + opts["soild_map"]
        + "==1,analysis_surface*(yield_pix*%f*0+yield_pix*%f),if("
        + treatment
        + "==2 && "
        + opts["soild_map"]
        + "!=1, analysis_surface*(yield_pix*%f+yield_pix*%f)))"
    )
    expr_sw7 = (
        "S_W_prot_bioenergy7 = if("
        + treatment
        + "==2 && "
        + opts["soiltx_map"]
        + "==3,analysis_surface*(yield_pix*%f*0.35+yield_pix*%f),if("
        + treatment
        + "==2 && "
        + opts["soiltx_map"]
        + "!=3, analysis_surface*(yield_pix*%f+yield_pix*%f)))"
    )
    expr_sw8 = (
        "S_W_prot_bioenergy8 = if("
        + treatment
        + "==2 && ("
        + opts["soilcmp_map"]
        + "==4 || "
        + opts["soilcmp_map"]
        + "==5),analysis_surface*(yield_pix*%f*0+yield_pix*%f),if("
        + treatment
        + "==2 && ("
        + opts["soilcmp_map"]
        + "<4 || "
        + opts["soilcmp_map"]
        + ">5), analysis_surface*(yield_pix*%f+yield_pix*2.1)))"
    )
    ...
    run_command(
        "r.mapcalc",
        overwrite=ow,
        expression=opts["output_sw_map"]
        + " = max(S_W_prot_bioenergy_FF, S_W_prot_bioenergy_T)",
    )
...
def avoided_CO2_emission(opts, flgs):
    ...
    run_command(
        "r.slope.aspect", overwrite=ow, elevation=opts["dtm2"], slope="slope_deg__"
    )
...
def fire_risk_reduction(opts, flgs):
    ...
    run_command(
        "r.mapcalc",
        overwrite=ow,
        expression=opts["output_fr_map"]
        + " = if(analysis_surface==1,"
        + opts["firerisk_map"]
        + "*0.7, "
        + opts["firerisk_map"]
        + "*1)",
...
def recreational_improvement(opts, flgs):
    ...
    run_command(
        "r.mapcalc",
        overwrite=ow,
        expression=opts["output_tot_re_map"]
        + " = (ewres()*nsres()/10000)*"
        + opts["touristic_map"],
    )
    run_command(
        "r.mapcalc",
        overwrite=ow,
        expression=opts["output_imp_re_map"]
        + " = if(analysis_surface==1,(ewres()*nsres()/10000)*"
        + opts["touristic_map"],
    )
...
def tev(opts, flgs):
    ...
    run_command(
        "r.mapcalc", overwrite=ow, expression="tev_map=(tev1/tev_area)*ewres()*nsres()"
    )

    TEV2 = "tev_area"

    if base:
        formula_tev = "%s=if(%s>0, %s-%f*%s/%s*%s)" % (
            res,
            expl,
            TEV2,
            impact,
