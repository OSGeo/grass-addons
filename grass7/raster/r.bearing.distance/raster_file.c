  ...
  if (G_find_raster (map_name, search_mapset) == NULL)
    {
      ...
      sprintf (message, "Can't find %s for input ", map_name);
      return -1;
    }
  ...
  if ((map_fd = Rast_open_old (map_name, mapset)) < 0)
    {
      ...
      sprintf (message, "Can't open %s for input ", map_name);
      return map_fd;
    }
			 ...
			 int overwrite, char* message)
{
  ...
  if ((map_fd = Rast_open_new (map_name, cell_type)) < 0)
    sprintf (message, "Can't create raster map %s@%s ",
	     map_name, mapset);
...
void Print_raster_buf (void* in_buf, 
		       RASTER_MAP_TYPE in_type,
		       FILE* stream)
{
  int row, col, nrows, ncols;
      ...
      for (col = 0; col < ncols; col ++)
	{
	  switch (in_type)
	    ...
	    case FCELL_TYPE:
	      ...
	      break;
	    case DCELL_TYPE:
	      fprintf (stream, "row=%d col=%d dcell=%lf ", nrows-1-row, col,
		       Rast_get_d_value (in_buf_ptr, in_type));
	      in_buf_ptr = G_incr_void_ptr (in_buf_ptr,
					    Rast_cell_size(in_type));	  
	    }
