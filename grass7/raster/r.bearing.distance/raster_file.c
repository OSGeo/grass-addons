...
void *Allocate_raster_buf_with_null(RASTER_MAP_TYPE buf_type)
{
    ...
    for (row = nrows - 1; row >= 0; row--)
        for (col = 0; col < ncols; col++) {
            ...
            map_buf_ptr = G_incr_void_ptr(map_buf_ptr,
                                          Rast_cell_size(buf_type));
        }
    return map_buf;
}
...
void Copy_raster_buf(void *in_buf, void *out_buf,
                     RASTER_MAP_TYPE in_type, RASTER_MAP_TYPE out_type)
{
        ...
        for (col = 0; col < ncols; col++) {
            ...
            case CELL_TYPE:
                Rast_set_c_value(out_buf_ptr,
                                 Rast_get_c_value
                                 (in_buf_ptr, in_type), out_type);
                in_buf_ptr = G_incr_void_ptr(in_buf_ptr,
                                             Rast_cell_size(in_type));
                out_buf_ptr = G_incr_void_ptr(out_buf_ptr,
                                              Rast_cell_size(out_type));
                break;
            case FCELL_TYPE:
                       ...
                       RASTER_MAP_TYPE * map_type, char *message)
{
    ...
    if (G_find_raster(map_name, search_mapset) == NULL) {
        /* message passed to G_fatal_error in main.c */
...
int Open_raster_outfile(char *map_name, char *mapset,
                        ...
                        int overwrite, char *message)
{
    ...
    if ((map_fd = Rast_open_new(map_name, cell_type)) < 0)
        /* message passed to G_fatal_error or G_warning in main.c */
        sprintf(message, _("Can't create raster map <%s@%s> "),
                map_name, mapset);
...
void Print_raster_buf_row_col(void *in_buf,
                              RASTER_MAP_TYPE in_type, FILE * stream)
{
    int row, col, nrows, ncols;
    void *in_buf_ptr;
    ...
    for (row = nrows - 1; row >= 0; row--) {
        for (col = 0; col < ncols; col++) {
            switch (in_type) {
            case CELL_TYPE:
                fprintf(stream, "row=%d col=%d cell=%d ", nrows - 1 - row,
                        col, Rast_get_c_value(in_buf_ptr, in_type));
                in_buf_ptr =
                    G_incr_void_ptr(in_buf_ptr, Rast_cell_size(in_type));
                break;
            case FCELL_TYPE:
                fprintf(stream, "row=%d col=%d fcell=%f ", nrows - 1 - row,
                        col, Rast_get_f_value(in_buf_ptr, in_type));
                in_buf_ptr =
                    G_incr_void_ptr(in_buf_ptr, Rast_cell_size(in_type));
                break;
            case DCELL_TYPE:
...
void Read_raster_infile(void *map_buf, int map_fd,
                        RASTER_MAP_TYPE buf_type, RASTER_MAP_TYPE map_type)
{
    int row, col, nrows, ncols;
        ...
        case CELL_TYPE:
            for (col = 0; col < ncols; col++) {
                Rast_set_c_value(map_buf_ptr,
                                 Rast_get_c_value
                                 (row_buf_ptr, map_type), buf_type);
                map_buf_ptr = G_incr_void_ptr(map_buf_ptr,
                                              Rast_cell_size(buf_type));
                row_buf_ptr = G_incr_void_ptr(row_buf_ptr,
...
void Set_buffer_null_c_row_col(void *out_buf,
                               RASTER_MAP_TYPE out_buf_type, int row, int col)
{
    ...
    ptr = out_buf + (row * Rast_window_cols() * Rast_cell_size(out_buf_type))
        + (col * Rast_cell_size(out_buf_type));
    Rast__set_null_value(ptr, 1, 0, out_buf_type);
}
...
void Set_buffer_null_d_row_col(void *out_buf,
                               RASTER_MAP_TYPE out_buf_type, int row, int col)
{
    void *ptr;
...
void Set_buffer_null_f_row_col(void *out_buf,
                               RASTER_MAP_TYPE out_buf_type, int row, int col)
{
    void *ptr;
...
void Write_raster_outfile(void *map_buf, int map_fd,
                          RASTER_MAP_TYPE buf_type, RASTER_MAP_TYPE map_type)
{
    int row, col, nrows, ncols;
