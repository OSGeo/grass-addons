...
/***********************************************************************/
  ...
  module->description = _("Find the bearing and/or straight-line distance from all non-null cells to the specified point.");
  ...
  input->key_desc = "name";
  input->description= _("Raster map containing non-null cells for which bearing is to be computed");
  ...
  ref_bearing->description= _("Bearing in degrees from N (for calculation of bearing difference map)");
  ...
  csv_seg->required = NO;
  csv_seg->description = "Output plain text  CSV file for segment counts";

  csv_ax = G_define_option ();
  csv_ax->key = "csv_ax";
  csv_ax->type = TYPE_STRING;
  csv_ax->required = NO;
  csv_ax->description = "Output plain text CSV file for axial difference counts";

  reverse = G_define_flag ();
  reverse->key = 'r';
  reverse->description = _("Reverse bearing (i.e. bearing towards point location) ");

  axial = G_define_flag ();
  axial->key = 'a';
  axial->description = _("Bearing difference is axial (0 - 90 degrees) rather than clockwise) ");

  axial_signed = G_define_flag ();
  axial_signed->key = 'b';
  axial_signed->description = _("Bearing difference is axial and signed (0 - +/-90 degrees) rather than clockwise) ");

  square = G_define_flag ();
  square->key = 's';
  square->description = _("Segments are clockwise from zero rather than centred on zero ");
...
/***********************************************************************/
  ...
  if (do_bearing || do_segments)
		  ...
		  if (! (Rast_is_null_value(&data, input_buf_cell_type)))
		      ...
		      if (do_relative_bearing)
			{
			  if (axial->answer)
				...
				relative_bearing = calc_azimuth_axial_diff_signed
				  (reference_bearing, bearing);
			      else
				relative_bearing = calc_azimuth_clockwise_diff
				  (reference_bearing, bearing);
			    }
			  Set_buffer_value_d_row_col (output_buf, relative_bearing, 
						      output_buf_cell_type, row, col);
			  if ((axial->answer || axial_signed->answer) && csv_ax->answer != NULL)
			    {
			...
			Set_buffer_value_d_row_col (output_buf, bearing, 
		      ...
		      if (do_segments)
			{
			  if (do_relative_bearing && (! axial->answer) && (! axial_signed->answer))
			    segment = calc_segment (relative_bearing);
			  else
			    segment = calc_segment (calc_azimuth_clockwise_diff (reference_bearing, bearing));
			  Set_buffer_value_c_row_col (segment_buf, segment, 
						      segment_buf_cell_type, row, col);
			  cell_counts [segment] ++;
			}
		    }
      ...
      else
	{
      ...
      if (do_segments)
	{
	  segment_fd = Open_raster_outfile (segment_map->answer, current_mapset, 
	  ...
	  if (segment_fd < 0)
	  ...
	  else
	    {
	      Write_raster_outfile (segment_buf, segment_fd, 
				    segment_buf_cell_type, segment_map_cell_type);
...
/***********************************************************************/
  ...
  if (do_distance)
	...
	for (i=0; i<=n_segments; i++)
	  {
	    G_message (_("| %7d | %14ld | %7.1f |\n"), i, cell_counts [i], 100.0/cell_counts [0]*cell_counts [i]);
	  }	
      }
	...
	if (csv_seg->answer != NULL)
	  {
		...
		for (i=0; i<=n_segments; i++)
		  {
		    fprintf (csv_seg_str, ",%f", 100.0/cell_counts [0]*cell_counts [i]);
		  }
		fprintf (csv_seg_str, "\n");
		fflush (csv_seg_str);
	...
	if (csv_ax->answer != NULL)
	  {
		...
		double sum_all = axial_counts[SUMPOS] + axial_counts[SUMNEG] + 0;
		...
		fprintf (csv_ax_str, ",%ld", n_all);
		fprintf (csv_ax_str, ",%ld", (long) axial_counts[SUMPOS]);
		fprintf (csv_ax_str, ",%ld", (long) axial_counts[SUMNEG]);
		fprintf (csv_ax_str, ",%lf", sum_all);
		if (axial_counts[NPOS] > 0) 
		  fprintf (csv_ax_str, ",%lf", axial_counts[SUMPOS] / axial_counts[NPOS]);
		else
		  fprintf (csv_ax_str, ",%s", "NA");
		if (axial_counts[NNEG] > 0)
		  fprintf (csv_ax_str, ",%lf", axial_counts[SUMNEG] / axial_counts[NNEG]);
		else
		  fprintf (csv_ax_str, ",%s", "NA");
		if (n_all > 0 )
