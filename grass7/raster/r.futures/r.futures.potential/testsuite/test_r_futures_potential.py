...
class TestPotential(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.runModule('r.mapcalc', expression="urban_change = if(urban_2002 == 1, if(urban_1987 == 0, 1, null()), 0)")
        cls.runModule('r.mapcalc', expression="single_level = 1")
        cls.runModule('r.sample.category', input='urban_change', output='sampling',
                      sampled=['zipcodes', 'single_level', 'devpressure', 'slope', 'lakes_dist_km', 'streets_dist_km'],
                      npoints=[500, 100], random_seed=1)
    ...
    def test_potential_run(self):
        ...
        self.assertModule('r.futures.potential', input='sampling',
                          columns=['devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'],
                          developed_column='urban_change', subregions_column='zipcodes',
                          output=self.output)
        with open(self.output, 'r') as f:
            ...
            for line in f.readlines():
                ...
                if i == 0:
                    self.assertEqual(line, ['ID', 'Intercept', 'devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'])
                i += 1
            self.assertEqual(i, 14)

    def test_potential_run_dredge(self):
        """Test if results is in expected limits"""
        self.assertModule('r.futures.potential', flags='d', input='sampling',
                          columns=['devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'],
                          developed_column='urban_change', subregions_column='zipcodes',
                          min_variables=4, max_variables=4,
                          output=self.output)
        with open(self.output, 'r') as f:
            i = 0
            for line in f.readlines():
    ...
    def test_potential_run_single_level(self):
        ...
        self.assertModule('r.futures.potential', input='sampling',
                          columns=['devpressure', 'lakes_dist_km', 'slope', 'streets_dist_km'],
                          developed_column='urban_change', subregions_column='single_level',
                          output=self.output)
        with open(self.output, 'r') as f:
            i = 0
            for line in f.readlines():
