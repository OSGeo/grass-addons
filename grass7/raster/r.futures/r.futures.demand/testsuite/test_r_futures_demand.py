...
class TestPGA(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.runModule('r.mapcalc', expression="urban_2005 = if(ndvi_2002 <= 0.15 && isnull(lakes), 1, if(isnull(lakes), 0, null()))")
        cls.runModule('r.mapcalc', expression="urban_2006 = if(ndvi_2002 <= 0.18 && isnull(lakes), 1, if(isnull(lakes), 0, null()))")

    @classmethod
    def tearDownClass(cls):
        cls.runModule('g.remove', flags='f', type='raster',
                      name=['ndvi_2002', 'ndvi_1987', 'urban_1987',
                            'urban_2002', 'urban_2005', 'urban_2006'])
        cls.del_temp_region()
