...
int main(int argc, char *argv[])
{
  ...
  struct node *quad1_hoz_head, *quad2_hoz_head, *quad3_hoz_head, *quad4_hoz_head;
...
/***********************************************************************/
  ...
  module->description = _("Compute the skyline index and / or find the horizon cells in a raster viewshed.");
  ...
  view2->key_desc = "name";
  view2->description= _("Raster map containing second viewshed map (for skyline index)");
  ...
  hoz_inc->description= _("Raster map name for storing far horizon inclination");
  ...
  skyline->description= _("Raster map name for storing viewshed skyline index");
  ...
  if ((do_hoz_az + do_hoz_inc + do_hoz_type + do_edges + do_skyline + do_profile) < 1)
    {
      G_usage ();
  ...
  if (do_hoz_az | do_hoz_inc | do_profile | do_skyline)
      ...
      if (do_hoz_az)
	{
	  ...
	  if (hoz_az_fd < 0)
	    ...
	    { 
	      hoz_az_buf = Allocate_raster_buf_with_null (hoz_az_buf_cell_type);
...
/***********************************************************************/
      ...
      if (do_hoz_inc)
	{
	  ...
	  if (hoz_inc_fd < 0)
	    ...
	    { 
	      hoz_inc_buf = Allocate_raster_buf_with_null (hoz_inc_buf_cell_type);
...
/***********************************************************************/
      ...
      if (do_hoz_type)
	{
	  ...
	  hoz_type_fd = Open_raster_outfile (hoz_type->answer, current_mapset, 
...
/***********************************************************************/
      ...
      if (do_skyline)
	{
	  ...
	  if (view2->answer != NULL)
	    viewshed_fd = Open_raster_infile (view2->answer, view_mapset, "",
					      &viewshed_map_cell_type,
					      message);
	  else 
	    {
	      viewshed_fd = Open_raster_infile (view->answer, view_mapset, "",
						&viewshed_map_cell_type,
						message);
	      G_warning ("Computing skyline index using same viewshed maps for horizon and line-of-sight from viewshed cells - is this really what you want (see manual page)");
	    }
	  ...
	  if (viewshed_fd < 0)
	      ...
	      for (row = n_box1; row <=  s_box1; row ++) /* row origin at north */
		{
		  for (col = w_box1; col <= e_box1; col ++)
		    { 
		      inclination = Get_buffer_value_d_row_col (viewshed_buf,
								viewshed_buf_cell_type,
								row, col);
		      if (! (Rast_is_null_value(&inclination, viewshed_buf_cell_type)))
			{
			  skyline_index = compute_skyline_index (row, col, inclination,
								 &edge_head, quad1_hoz_head,
								 quad2_hoz_head,
								 quad3_hoz_head,
								 quad4_hoz_head);
			  ...
			  Set_buffer_value_d_row_col (viewshed_buf, skyline_index,
		 ...
		 compute_skyline_index_simple () */ 
	      ...
	      skyline_fd = Open_raster_outfile (skyline->answer, current_mapset, 
