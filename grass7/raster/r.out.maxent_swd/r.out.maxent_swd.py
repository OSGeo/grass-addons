...
def cleanup():
    ...
    if RasterAbstractBase("{}_MASK".format(TMP_NAME)).exist():
        gscript.verbose("Restoring old mask...")
        if RasterAbstractBase("MASK").exist():
...
def parse_bgr_input(alias_input, env_maps, alias_names):
    ...
    if alias_input:
        ...
        with open(alias_input, "r") as alias:
            parameters = [line.split(",")[1] for line in alias if "," in line]
            alias = [line.split(",")[0] for line in alias if "," in line]
    else:
        parameters = env_maps
    ...
    for param in parameters:
        ...
        param_map = RasterAbstractBase(
            param
        )  # param.split("@") if "@" in param else param)
        if not param_map.exist():
            gscript.fatal(
                _(
...
def parse_species_input(species_masks, species_names):
    ...
    if species_masks:
        for idx, species in enumerate(species_masks):
            ...
            species_map = RasterAbstractBase(*species_map_name)
