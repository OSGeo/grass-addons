...
class Test(unittest.TestCase):
    ...
    def test_diversity_criterion(self):
        ...
        self.assertTrue(
            (1 in selected_samples) ^ (2 in selected_samples)
        )  # Either b or b_bis is kept
        ...
        self.assertTrue(
            (samples == np.array([a, b, b_bis, c, d, e, f, g, h, i])).all()
        )  # Check that the original array was not modified

    def test_write_result_file(self):
        X = np.array([[11.0, 3.5, 4.7], [22.0, 4.5, 6.7]])
        ...
        data = np.array(
            [
                ...
                [1.0, 0.0, 11.0, 3.5, 4.7],
                [2.0, 1.0, 22.0, 4.5, 6.7],
            ]
        )
    ...
    def test_update(self):
        ...
        update_data = np.array([["cat", "Class"], [123, 1], [456, 2]])
        ...
        X_unlabeled = np.array([[11, 22, 33], [44, 55, 66], [77, 88, 99]])
        ID_unlabeled = np.array([123, 456, 789])

        ####
        X_train, ID_train, y_train = al.update(
            update_file, X_train, ID_train, y_train, X_unlabeled, ID_unlabeled
        )
        ####
        X_train_should_be = np.array([[1, 2, 3], [4, 5, 6], [11, 22, 33], [44, 55, 66]])
