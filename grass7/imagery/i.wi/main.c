...
int main(int argc, char *argv[]) 
{
    ...
    if (!strcasecmp(wiflag, "awei_ns") && (!(opt.nir->answer)
                || !(opt.green->answer) || !(opt.chan5->answer)) )
        G_fatal_error(_("awei_noshadow requires green, nir and chan5 maps"));

    if (!strcasecmp(wiflag, "awei_s") && (!(opt.blue->answer)
                || !(opt.nir->answer) || !(opt.green->answer) 
                || !(opt.chan5->answer) || !(opt.chan7->answer)) )
        G_fatal_error(_("awei_shadow requires blue, green, nir, chan5 and chan7 maps"));

    if (!strcasecmp(wiflag, "lswi") && (!(opt.nir->answer)
                || !(opt.chan7->answer)) )
        G_fatal_error(_("lswi requires nir and chan7 maps"));

    if (!strcasecmp(wiflag, "ndwi_mf") && (!(opt.nir->answer)
                || !(opt.green->answer)) )
	...
	for (col = 0; col < ncols; col++)
	{
		...
		if (!strcasecmp(wiflag, "awei_s")) {
                d = awei_shadow(d_bluechan, d_greenchan, d_nirchan, d_chan5chan, d_chan7chan);
                ((DCELL *) outrast)[col] = d;
                }
                /*calculate lswi                    */ 
