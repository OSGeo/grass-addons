...
def cleanup():
    if rm_region is not None and \
       gscript.find_file(name=rm_region, element='windows', mapset='.')['name']:

        gscript.run_command('g.region', region=rm_region)
        gscript.run_command('g.remove', type='region',
                            name=rm_region, flags='f', quiet=True)
...
def main():
    ...
    for inmap in inmaps:
        ...
        if rinfo['datatype'] != "CELL":
            gscript.warning("Input map <%s> is not of CELL type but %s." % (inmap, rinfo['datatype']))
            continue
        ...
        gscript.run_command('r.reclass', input=inmap, output=inmap + "_rcl", rules=tmpfile)
        gscript.try_remove(tmpfile)
        rm_rasters.append(inmap + "_rcl")

        gscript.run_command('r.clump', input=inmap + "_rcl", output=inmap + "_rcl_clump")
        ...
        clump = gscript.read_command('r.what', map=inmap + "_rcl_clump",
        ...
        if n_inner_clumps > 0:
            ...
            gscript.run_command('r.patch',
                                input="%(inmap)s_nearest,%(inmap)s_nozero" %
                                      {"inmap": inmap},
        ...
        path = os.path.join(gisenv['GISDBASE'],
        ...
        if os.path.exists(path):
            newpath = os.path.join(gisenv['GISDBASE'],
                                   gisenv['LOCATION_NAME'],
                                   gisenv['MAPSET'],
                                   "cell_misc",
                                   inmap + "_null",
