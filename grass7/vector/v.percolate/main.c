...
int main(int argc, char *argv[])
{
  ...
  thresh_inc->description = _("Amount by which distance threshold is incremented between minthresh and maxthresh");
  ...
  interval->answer = "0";
  interval->description = _("Choose interval output. E.g. interval 10 will produce output for every tenth node-pair assigned cluster membership. Zero disables");
  ...
  keep->answer = "oldest";
  keep->description = _("Rule for deciding which cluster to keep: oldest or biggest");
  keep->guisection = _("Output");
  
  exitflag = G_define_flag ();
  exitflag->key = 'e';
  exitflag->description = _("Terminate once all points are connected in one group");
...
/***********************************************************************/
  ...
  if (modulo > 0) {
    G_message(_("\nOutputing results by every %d node-pair assignment to group"), modulo);
  }
  else {
  ...
  percolate (min, inc, max, numpoints, text_file_name, group_text_file_name, modulo);

  getMinMaxNNdistances (nodeList, numpoints, numpoints, &minNNdist, &maxNNdist);
  G_message (_("Min NN distance = %f, Max NN distance = %f"), minNNdist, maxNNdist);
