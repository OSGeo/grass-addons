...
def legalize_string(string):
    """Leagalize a string: replace and remove all non-alphanumeric characters

    :param string: A string to legalize
    :type string: string
    :returns: Legalized string
    :rtype: string

    :Example:

    >>> legalize_string("Test&'")
    "test_"
    """

    normalized_string = ""
    # Normalize string
    for form in ["NFKD", "NFKC", "NFD", "NFC"]:
        normalized_string_form = (
            normalize_unicode(form, string).encode("ascii", "ignore").decode()
        )
        if len(normalized_string) < len(normalized_string_form):
            normalized_string = normalized_string_form

    normalized_string = re.sub("[^0-9a-zA-Z]+", "_", normalized_string).lower()
...
def raster_type(raster, tabulate, use_label):
    ...
    if not r_map.has_cats() and r_map.mtype != "CELL":
        ...
        if tabulate:
            ...
            for rcat in rcats:
                if type(rcat) == tuple:
                    cats.append(str(rcat[1]))
                    labels.append(rcat[0])
                    labels_normalized.append(legalize_string(rcat[0]))
                else:
                    rcat = rcat.split(" ")
                    cats.append(str(rcat[0]))
                    labels.append(rcat[1])
                    labels_normalized.append(legalize_string(rcat[1]))

            if (
                len(set(labels)) != len(set(labels_normalized))
                or len(set(labels_normalized)) != len(labels_normalized)
                and use_label
            ):
                grass.warning(
                    _(
                        "Normalized lables for map <{}> are not unique, using category in addition.".format(
                            raster
                        )
                    )
                )
            ...
            for idx, rcat in enumerate(cats):
                label = (
                    labels_normalized[idx]
                    if labels_unique
                    else "{}_{}".format(rcat, labels_normalized[idx])
                )
                rcat_dict[str(rcat)] = label if use_label else str(rcat)

        else:
            r_map.close()

    return rmap_type, rcat_dict
...
def main():
    ...
    percentile = (
        ...
        else list(map(float, options["percentile"].split(",")))
    )
        ...
        for buf in buffers:
            ...
            for rm, rmap in enumerate(raster_maps):
                ...
                if tabulate:
                    ...
                    if not t_stats[0].startswith("null"):
                        mode = t_stats[0].split(" ")[0].rstrip("%")
                    elif len(t_stats) == 1:
                        mode = "NULL"
                    else:
                        mode = t_stats[1].split(" ")[0].rstrip("%")
