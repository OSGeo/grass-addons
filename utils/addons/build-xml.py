...
def get_gui_list(g7=True):
    mlist = list()
    for m in os.listdir(
        os.path.join(ADDON_PATH, "grass%s" % ("7" if g7 else "6"), "gui", "wxpython")
    ):
...
def start_grass(g7=True):
    if g7:
        ...
        ver = "64"
    gisbase = os.environ["GISBASE"] = os.path.join(
        os.getenv("HOME"), "src/grass%s/%s" % (ver, DIST)
    )
...
def header(fd):
    ...
    fd.write(
        '<addons version="%s" revision="%s" date="%s">\n'
