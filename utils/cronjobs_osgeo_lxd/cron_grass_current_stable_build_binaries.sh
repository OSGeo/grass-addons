...
# script to build GRASS GIS new current binaries + addons + progman from the `releasebranch_8_5` branch
...
# - generates the install scripts
# - generates the pyGRASS 8 HTML manual
# - generates the user 8 HTML manuals
# - injects DuckDuckGo search field
# - copies over generated manual pages to grass-stable/manuals/ securely via atomic staging
...
GMAJOR=8
GMINOR=5
GPATCH=0  # updated for GRASS 8.5.0 release
...
# Syncing updates cleanly to prevent directory locking or mid-update downtime
rm -rf /tmp/addons
if [ -d "$TARGETHTMLDIR/addons" ]; then
...
# generate addons modules.xml file
...
############################################
# CRITICAL IMPLEMENTATION: Atomic Directory Swap to prevent 404 downtime
TARGETHTMLDIRSTABLE="$TARGETMAIN/grass-stable/manuals"
STAGING_TMP="/tmp/grass_stable_manuals_staging"

echo "Deploying to stable directory via atomic swap..."
rm -rf "$STAGING_TMP"
mkdir -p "$STAGING_TMP"

# Mirror to temporary staging site first
cp -rp "$TARGETHTMLDIR"/* "$STAGING_TMP/"

# Perform quick atomic update swap using rsync with deletion tracking
rsync -a --delete "$STAGING_TMP/" "$TARGETHTMLDIRSTABLE/"
rm -rf "$STAGING_TMP"
...
############################################
# Create explicitly named sitemaps to solve the 404 tracking issue
# Output files are targeted explicitly to ensure the server finds them
python3 "$MAINDIR/src/grass$GMAJOR-addons/utils/create_manuals_sitemap.py" --dir="/var/www/code_and_data/grass$GMAJOR$GMINOR/manuals/" --url=https://grass.osgeo.org/grass$GMAJOR$GMINOR/manuals/ --output="sitemap_manuals.xml"
python3 "$MAINDIR/src/grass$GMAJOR-addons/utils/create_manuals_sitemap.py" --dir="/var/www/code_and_data/grass$GMAJOR$GMINOR/manuals/addons/" --url=https://grass.osgeo.org/grass$GMAJOR$GMINOR/manuals/addons/ --output="sitemap_manuals_addons.xml"

python3 "$MAINDIR/src/grass$GMAJOR-addons/utils/create_manuals_sitemap.py" --dir="/var/www/code_and_data/grass-stable/manuals/" --url=https://grass.osgeo.org/grass-stable/manuals/ --output="sitemap_manuals.xml"
python3 "$MAINDIR/src/grass$GMAJOR-addons/utils/create_manuals_sitemap.py" --dir="/var/www/code_and_data/grass-stable/manuals/addons/" --url=https://grass.osgeo.org/grass-stable/manuals/addons/ --output="sitemap_manuals_addons.xml"
...
echo "Finished GRASS $VERSION $ARCH compilation securely."
exit 0
