...
DCELL calc_ETp(DCELL T, DCELL Z, DCELL u2, DCELL Rn, 
								int day, DCELL Rh, DCELL hc){
	...
	/* calculus: net radiation [MJ/m2*h] -> Rn derived from r.sun */
	
	/* calculus: soil heat flux [MJ/m2*h] */
	if (day==TRUE)
		G=0.1*Rn;
	else
		G=0.5*Rn;
	...
	ETrad = (delta/(delta+gstar))*((Rn-G)/lambda);
	...
	ETaero = (3.6/lambda)*(1/(delta+gstar))*(rho*cp/ra)*(ea-ed);
...
DCELL calc_openwaterETp(DCELL T, DCELL Z, DCELL u2, DCELL Rn,  
											int day, DCELL Rh, DCELL hc){
